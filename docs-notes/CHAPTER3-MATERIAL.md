...
> **Newest material, 2026-09-13 and 14:** §3.5.84, A11 and finding 365 (a
> capability relocated with no way to reach it, and an approval recorded that never
> applied); §3.5.85, the week checked a second time (findings 366–368);
> §3.5.86, the independent review closed item by item and the five repairs it
...
> under failure (findings 346–364). It covers a service worker holding a second,
> unguarded copy of the governance read model; escalations from the dashboard that
> could not reach a person; T63's claim made true, with a correction to §3.5.82;
> and the kill switch composed with a dashboard prompt (364).
>
...
| 7   | Real-time control: suspend/terminate within 1 second             | **Met**            | `kill-switch.ts` + `agent-terminator.ts` + `src/gateway/governance-agent-termination.ts`. Measures **confirmed termination**, not dispatch: the run-activity probe waits for signalled runs to leave the Gateway registry, and reports `dispatchMs`, `elapsedMs` and `stoppedConfirmed` separately (§3.5.10, §4.x.17). _(A caveat that the command line could lock an agent but not abort its runs went with the command line on 2026-09-07.)_ **Three failure modes found and fixed in the thirteenth pass (§4.x.20)**, each of which used to return `200 OK` while stopping nothing: a mistyped agent id (the dashboard now offers known ids and warns when the typed one matches none), a hand-written `agentMode: "off"` (dropped on load), and a call carrying neither `agentId` nor `sessionKey` (refused then whenever any agent was locked, under `kill-switch-unattributable`; **since M5 refused always**, as `agent-not-registered`, because such a call names no organisation and so no rulebook). **Blast radius completed 2026-08-25 (T6, §3.5.38):** a lockdown now reaches a cross-agent child already running, by walking the `spawnedBy` chain the host records on the session entry. Finding 96 closed without any upstream change. **A fourth failure mode of the same class was found on 2026-09-01 and is the worst of the four (finding 202, §3.5.68):** the agent id was taken raw from the request body, so a stop engaged on `Scout` for an agent called `scout` wrote a lockdown the gate did not recognise, matched no runs, and reported `stoppedConfirmed: true`, because zero aborted runs reads as "nothing was in flight". Folded now at every boundary, on read as well as write. **And two throws could escape after the lockdown had landed (195)**, reporting a stop that had _worked_ as a failure; both are guarded, with the ledger write best-effort here alone and its failure carried back rather than swallowed. **A fifth, found by composing features on 2026-09-13 (finding 364, security, §3.5.83):** a prompt started from the dashboard runs outside the Gateway's run registry, so a lockdown left it running with its escalation still answerable, and "Allow once" could run an action on a locked agent. Termination now ends the agent's prompt runs too, and confirms the stop only once they have unwound, and an allow for a locked agent is refused; on the dashboard, Lock down asks first and names the agent (§3.5.86)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| 9   | Deployable on Linux, open-source components only                 | **MET 2026-09-21** | Open-source ✔ (zero new dependencies; `git diff main..HEAD -- package.json` is empty. **Re-measured 2026-09-20: the LOCKFILE diff is not empty** - 4 lines of deprecation metadata pnpm wrote onto `@aws-sdk/core` and `crypto-js`, both already upstream. No package entry was added or removed, so the substance holds; state it that way rather than claiming an empty lockfile diff). **Built, started, deployed and demonstrated on Linux.** T33 closed 2026-08-28: bare source build on Ubuntu 24.04, installer exit 0, `scripts/governance-linux-check.mjs` 14/14, `openclaw` on PATH. The fork has been **installed and running on a VPS since 2026-09-03**, and **T2 was demonstrated there on 2026-09-06** — Kimi drove agent `jack` at `~/.npmrc`, the gate refused it, and ledger entry **#25** records the denial naming the rule that produced it. **CLOSED 2026-09-21. The re-run was done on the VPS at commit `0a7d51f1c12`** and this row is now **Met**. Governance suite on Ubuntu: **3,211 passed / 16 skipped / 0 failed across 197 files** (818 s). The demonstration rehearsal: **20/20**. The platform probe `scripts/governance-linux-check.mjs`: **14/14**, `platform=linux node=v22.23.2`. **The Windows and Linux figures reconcile exactly**: 3,206 + 21 skipped and 3,211 + 16 skipped are both **3,227 tests**, so five tests that skip on Windows run on Linux, and one extra file runs there. Nothing fails on either platform. _The sentence that follows was true until that run and is kept because it is what the row turned on:_ **What keeps this row at "partially" is one thing only: the full suite has not been re-run on Linux since.** The last Linux measurement is 2,548 passed / 133 skipped and predates T44, the command line's removal and every sweep from 2026-09-01 onward, so the number cannot be quoted as current. That re-run is what remains of **T3**. _(**Corrected 2026-09-08 (viii), and the direction is the point.** This cell read "Linux tested, not deployed … the application itself has never been built or started on Linux … Nothing has run on a VPS", and cited `scripts/linux-setup.sh`, a file renamed to `scripts/wsl-dev-setup.sh` on 2026-08-28 and superseded by `scripts/vps-install.sh`. Every one of those claims had been false for between five and eleven days. The cell's own closing sentence warns that **"the status column is the one the report quotes"** — it was written against being too **optimistic**, and then went stale in the other direction, understating the project in the one document that feeds the dissertation. A register that is only audited for flattery is audited in one direction.)_ See §4.x.9 and **T3**. **Two lint errors sat in the install path itself until 2026-09-02** (finding 221): `scripts/register-ts-resolver.mjs` began with a **UTF-8 BOM**, and `scripts/governance-linux-check.mjs`, the probe the installer runs and whose failure aborts the install, carried an unused import and a `return` inside a Promise executor. None was reachable by the documented lint command, which does not target `scripts/`. All are fixed. **This row said "Met" until 2026-08-28** and contradicted §4.x.5b in this same file, which has always read "Partially met"; §3.4 says the same. |
...
| 9   | Deployable on Linux, open-source only                | Open-source ✔, `git diff main..HEAD -- package.json` is **empty**, so still zero new dependencies (re-measured 2026-09-20: the lockfile diff is **not** empty, 4 lines of deprecation metadata on two pre-existing packages, but no package entry was added or removed). Linux: `scripts/vps-install.sh` and `governance-linux-check.mjs` are present and syntax-clean, and **the four lint errors that sat in those two install-path scripts are fixed** (finding 221), one of them a BOM that would have been read as part of the first import | **Still "partially met" until the VPS boots** |
...
1. `./scripts/vps-install.sh` completing, including its `governance-linux-check`
   run. Requirement 9.
2. The **Deployment report** panel, through the tunnel. _(This was
   `openclaw governance deployment`, over plain SSH **before** any tunnel, until
   the command line was removed on 2026-09-07. Losing the before-the-tunnel
   route is the real cost of that removal and is recorded as one.)_
...
4. A real Kimi prompt that attempts a command the policy refuses, and the
   **Audit ledger** entry recording it. Requirements 3, 5 and 7 together, with a
   live model behind them for the first time.

That last one is the only thing in this project that has never been done.

## → 3.2 Analysis of Requirements

Notes on what each requirement actually demanded once implemented.

**Corrected 2026-09-20.** This paragraph used to read "the two honest narrowings
below should be stated explicitly", which its own body had overtaken: requirement
5's narrowing was **abandoned**, and every invocation is now recorded. Neither
5 nor 7 is a narrowing any longer. What survives is that both are the
requirements an examiner will press on, because each turns on what a phrase
means: "100% of agent actions" and "within one second". State plainly what each
was read as demanding and what was measured against it.
...
| ~~`src/cli/program/register.governance.ts`~~   | ~~The `openclaw governance …` command tree~~ **removed 2026-09-07** | ~~977~~ |
...
code, which §3.5.2 tabulates separately. It also counts this codebase's comment
density, which is deliberate and high: the repairs on 2026-09-05, for instance,
added 27 lines of code against 41 of comment. A line count is therefore a
...
not quote the number.**)_

The language split below was measured at that earlier point and is kept for its shape rather than its totals:
...
sequenceDiagram
  ...
  alt rule matched
    ...
    opt allow-always
      G->>G: file a rule request for an administrator
    end
  end
```

_(The `opt` branch read `persist new rule` until 2026-09-07, finding 282. That
is the behaviour QA round 13 removed as a security defect and the 2026-09-06
restoration deliberately did not bring back: `allow-always` grants the call in
the moment and files a **proposal**, so the permanent widening happens only when
a signed-in Administrator or Root approves it and is recorded against them.)_
...
#### One surface, and what the second one was for
...
_(Until 2026-09-07 there was a second route to the same data,
`openclaw governance deployment`, and this section argued at length that it was
"the important one". The argument was sound and is preserved below because the
cost it describes is now real rather than hypothetical.)_
...
_(Superseded 2026-09-07: the command line was removed, so there are two surfaces,
the HTTP control plane and the dashboard on it, and the rule below is history.
§3.5.77 records why the third surface went.)_ The project's standing rule since
R5 was that a capability lands on all three surfaces or on none. Streaming is on the dashboard (always) and the CLI
...
it tells an attacker nothing a wrong password would not, and until 2026-09-07 the
operator's way out was `openclaw governance groups migrate --delete`, which
removed them. **Nothing removes them now.** That command went with the command
line, `deleteUnmigratedAccounts` has had no caller since, and this sentence said
the **Accounts** panel removes them until 2026-09-14, when it was checked against
the code. Only an installation that held accounts before M3 can have one.
...
be verified by matching numbers, and that half of the audit has to be done by
**reading**. _(This sentence read "remains open" for ten days after the reading
was done and the gap closed, on 2026-08-28.)_ **The clause worth carrying into
the report is not that it was closed but that it cannot stay closed**: a
coverage check performed by reading has to be repeated, and by 2026-09-07 the
plain-language register had drifted eight findings behind again.
...
gate's hot path. Each "allow always" approval that an administrator **approves**
adds one permanently — the answer at the prompt files a proposal rather than a
rule, so the growth is bounded by a second person's attention as well as by the
cap. Now capped at 1000, checked after expiry pruning so an installation full of
lapsed grants recovers by itself instead of being told it is full.
...
documented example and every rule an approved `allow-always` proposal generates
— those are the escalated resource escaped and anchored, so they are literals by
construction.
...
_(Both are closed, and the first twice over. The command's verbs are refused by a
self-protecting core denial that ships today and that nobody can switch off
(`governance policy|agent|kill|ledger|…`, `baseline-policy.ts`), and the command
tree itself was removed on 2026-09-07 (§3.5.77). A hand-written per-agent `off` is
dropped on load (finding 80), and since 2026-09-13 it is refused on every path
that sets one, an approved agent-setting request included (finding 365,
§3.5.84).)_
...
| 134 | `forgetAgentIntent` written, exported, never called; the size cap already did the work **Reversed on 2026-09-07 by finding 273**, with a caller this time: the new `forgetAgentIntent` is for the _start of a turn_ rather than for session end, and it exists to stop a falsehood rather than to bound memory. | **An exported function with no caller**, finding 113's family, fourth member    |
...
The failure mode was thought safe and was stated alongside the limit: on a live
run the field is either populated or absent, and cannot be _wrong_, because an
intent is only ever read for the session that produced it and is replaced on that
session's next turn.

_(**Wrong, and settled by T2 on 2026-09-06 — finding 273.** The run produced a
third outcome nobody had allowed for: the field was **populated with the wrong
turn's words**. `llm_output` does not fire before the same turn's tool calls; the
intent is captured in the **settle** phase, after the attempt's calls have been
judged, so the value standing at the gate belongs to the previous turn. Ledger
entry #25 — the very entry that demonstrates the gate refusing a credential read
— carried the model's refusal of a *different* file from twenty-five minutes
earlier, so it read as though the model had declined when the **gate** had. Fixed
by dropping the standing intent at the start of every turn, which converts a
wrong intent into an absent one; the honest claim now is that an intent which is
present belongs to the turn that produced the call. The reasoned argument for
"it cannot be wrong" was the thing that most needed observing.)_
...
The project's verification set was six commands and a test suite when this was
written and is seven now (finding 250 added a browser project, the only one that
can see the dashboard); neither of these defects was visible to any of them,
because both files' tests asserted the behaviours the authors had thought of. The mechanical draw is what put a
...
### 3.5.77 Removing a surface, and why subtraction is a design decision

**On 2026-09-07 the `openclaw governance` command line was removed.** Fifty-five
commands, 3,162 lines and 101 tests, deleted rather than finished. It is recorded
here as a design decision rather than as housekeeping, because the reasoning is
the transferable part and because a report that only ever describes additions is
describing something other than engineering.

#### The rule that produced it

The project had asserted, in its own code, that **a capability reaching only two
of the three surfaces is unfinished**. T34 audited that rule on 2026-08-31, found
it false of four capability groups, and softened it to _"every capability reaches
all three surfaces unless a stated reason says otherwise"_. Four stated reasons
accumulated underneath it.

What nobody asked in either pass was the prior question: **why three surfaces?**
The nine design requirements in §1.3 name the tier model, the policy engine, the
audit ledger, human-in-the-loop approval and a Linux deployment. **None of them
names a command line.** The third surface was self-imposed, and every capability
built after it had to be built three times to satisfy a rule the specification
never asked for.
...
**It could not be defended.** A panel asking "why does this exist?" would have to
be answered with "it seemed incomplete without one", which is a statement about
symmetry rather than about need. Two surfaces — an HTTP control plane and the
dashboard built on it — are one sentence: a control plane and its front end.

**It was the surface that failed in front of an operator.** On 2026-09-07
`openclaw governance login` prompted for an account, printed no password label,
did not mask the password, and refused valid credentials. That failure blocked
reading the audit ledger to answer an open question (T58); the dashboard answered
the same question immediately. The surface that broke was the one nothing
required.

**It was carrying its own backlog.** T51 existed only because two of its commands
read a per-process table and so could never see the runs an operator cared about.
While surveying it for removal, a further defect surfaced: a comment stating that
`agent cancel` "reaches the Gateway's run registry" and could stop a
dashboard-started run. It could not. That is the comment an operator reads during
an incident, deciding whether they hold a scalpel or only the kill switch.

#### The measurement that settled it

The stated concern was time: the by-hand test plan (T47) needs three people and
three machines, and there was doubt about finishing it.
...
| **Rows in the by-hand plan that touched the command line** | **2 of 158**         |

The surface accounted for roughly **one percent** of the manual testing it was
supposed to be crowding out. Its automated tests were already written and green,
so removing it _reduced_ the suite rather than saving effort — the count fell
from 2,817 to 2,716, and from 156 files to 142.

**That drop is exactly 101 tests and 14 files**, which is the check that matters:
the suite lost precisely what was archived and nothing else. A removal is the one
change where a green suite proves least, because deleting a surface deletes its
tests and everything remaining passes by construction. The arithmetic is what
turns "still green" into evidence.

#### What was lost, stated rather than glossed

**The recovery surface.** The command line was file-backed and ran in its own
process, so it could read the deployment report, verify the audit chain and list
accounts **when the Gateway was down**. The dashboard cannot: it needs the
Gateway, and the Gateway needs to be up. Documentation that argued this exact
point — that the terminal "is the surface that works before the tunnel exists" —
has been left in place with a note, because the argument was correct and the cost
is now real rather than hypothetical.

**A cross-check that had found defects.** Two independent implementations of one
decision disagreeing is how finding **202** surfaced — an emergency stop that
reported success and stopped nothing — and it contributed to **254** and **255**.
That class of defect now has one fewer way of announcing itself. The parity test
suites that encoded it are archived rather than deleted, so a restored surface
would come back with its cross-checks intact.

**Neither is a security regression**, and the reason is structural rather than
reassuring. The gate is on neither surface: it sits at `runBeforeToolCallHook`,
the single function every tool call passes through, so what an agent may do is
unchanged by removing a way for a _human_ to administer policy. This was measured
rather than asserted — `cli-removal-sweep.ts` drives the gate after the removal
and confirms both an unlisted action and a core-tier denial are still refused.

#### One capability was rebuilt rather than archived

**`audit verify` was load-bearing for design requirement 8**, and it is the only
part of the surface that came back. The dashboard reports the chain's
verification — since finding 268 it shows the head, the checkpoint that agrees
with it, and the entry count — but it is the same process that holds the ledger,
and asking a system whether it has been tampered with is the shape of the
problem rather than a solution.

`scripts/verify-ledger.mjs` restores the second reader **without restoring a
surface**: no registry entry, no `--help` presence, no tiers, no session, no
write path. It is also a better witness than the command it replaces on three
counts. It imports nothing from `src/`, so it runs when the build is broken and
against files copied off the host — the arrangement that actually closes the
residual §7 caveat 4 describes. It **cannot sign in**, where the removed command
required a governance session before it would verify, which made the audit depend
on the thing under audit. And it **re-implements the hashing** rather than
importing it, so a defect in the product cannot agree with itself.

That last property has a cost worth stating, because it is a general one: **two
implementations of one algorithm drift, and a verifier that cries wolf is worse
than no verifier**, since the first false alarm teaches everyone to discount the
next real one. The mitigation is a test that runs both against real chains —
intact, one decision edited, entries truncated, checkpoint deleted — and fails if
they ever disagree. Independence bought with a pinning test is a different thing
from independence asserted.

> **And it cried wolf twice the next day, which is the strongest thing this
> section can say** (findings **286** and **287**, 2026-09-07 (iv), both fixed).
> The paragraph above named the cost before it was paid; a QA session that drove
> the script rather than reading it then paid it, twice, in the same direction:
>
> - It printed **`checkpoint: #2 AHEAD of the ledger`** on a three-entry chain
>   whose checkpoint was **behind** — the legitimate state `appendLedgerEntry`
>   creates on purpose by writing the entry first and the checkpoint second, so
>   a crash between the two leaves exactly it. Printed directly under the word
>   **INTACT**, and reading as _an entry was deleted from the end_: the single
>   conclusion this tool exists to let an operator reach correctly.
> - It accepted a signing key **below the floor the product refuses**
>   (`MIN_SUPPLIED_KEY_LENGTH`, 16 characters, enforced because a guessable key
>   defeats the whole claim). A one-character key made it HMAC every entry under
>   one byte, disagree with all of them, and report **`BROKEN at entry 1`, exit
>   1** — a misconfiguration announced as tampering, which is precisely what the
>   file's own header says its third exit code (**2, could not check**) exists to
>   prevent.
>
> **Neither was findable by reading, and the pinning test could not have caught
> either**, which is the part worth a sentence in Chapter 4. The test compares
> the two implementations against chains it _builds with the product_, so every
> fixture had a checkpoint the product had written (never behind) and a key read
> from the file (never supplied). **A parity test proves agreement on the inputs
> it constructs.** The two defects lived in the inputs it had no reason to
> construct — and the second is not a disagreement with the product at all, but
> the script being _more permissive_ than the thing it audits.
>
> The honest form of this section's claim is therefore narrower and better
> evidenced than the one above it: re-implementation is safe **where the pinning
> test reaches**, and the way to widen it is to enumerate the states the product
> can legitimately be in rather than the states its own writers produce.

#### The honest claim it leaves

The narrower claim is the more defensible one: **two operator surfaces, both
required, both tested**, rather than three of which one was optional, partly
finished, and carrying a known defect. Where a capability now exists on the HTTP
route and the dashboard alike, that is not a parity achievement — the dashboard
...
T24 let Root switch off five of the eight core denials there then were (ten today, five of them switchable), and the rule list removed a
...
### 3.5.86 An outside review closed item by item, and the five repairs it left (2026-09-13)

`Kimi_QA_1.md`, an independent review of the build against the PDF specification, had
been answered piecemeal as its points were taken into the backlog. On 2026-09-13 Kinan
asked for it to be closed properly: every point checked against the code as it stood,
anything still wrong fixed and made usable from the dashboard, and the file removed once
every point was answered. The table is in `mg/REMAINING-WORK.md` §"The independent
review, checked item by item": twenty-seven points, most of them closed weeks earlier.
Five repairs came out of the pass. They are recorded together because each is a small
instance of a larger design point.

#### Two controls with one effect, and one confirmation between them

_Stop_ in _Active agent sessions_ asked before locking an agent; _Lock down_ in the
emergency kill switch, which locks the same agent the same way, acted on one click, and
it is the one of the two that takes a typed agent id. The page's own rule is that a
confirmation belongs to the consequence, not to the widget that happens to cause it. The
repair applies that rule (`confirmThen`), and the dialog names the agent.

#### Reporting a change that genuinely widens access (bug 8)

Clash detection existed to tell an operator that a new rule adds nothing (a duplicate,
covered by a catch-all, narrower than a global rule) or does nothing (overridden by a
denial). An identical rule outliving a temporary one does something, which is precisely
why it went unreported: the detector was designed around redundancy. Kinan decided it
should be reported because of what it changes. Somebody made a grant or a restriction
temporary, and the operator adding the second rule may not know the first exists.
`extends-time-limited` is therefore a third meaning rather than a kind of redundancy, and
it has its own heading. The ledger entry for the new rule names the temporary rule it
extends, because the account that saw the notice need not be the account that later
reviews the trail. Folder grants and approved requests inherit both, since `addRule`
delegates to `addRuleChecked`, and the kind coexists with `narrower-than-global` instead
of replacing it.

#### Advice at the moment of decision, not only at authoring (the approval preview)

Warnings and clashes were computed on the create path and returned to whoever created
the rule. For a User's rule request, the create path runs inside the Administrator's
approval, after the decision, so the advice arrived when the rule was already in force
and reached nobody who could act on it. `GET rule-requests` now computes both for each
pending rule request against the policy as it stands (`withApprovalPreview`), and the
queue row shows them under _If this is approved_. Disclosure was checked rather than
assumed: a clash is computed only against global rules and rules for the request's own
agent, both of which a reader of the request can already see.

#### A false warning on the shape the product recommends

`describeRuleRisks` treated a pattern as anchored only when it ended in `$`. The
folder-grant form writes `^src(/|$)`, meaning this folder and everything below it, and a
hand-written copy of it drew an _unanchored_ warning illustrated with a
command-injection example that does not apply to paths. A warning that fires on the
product's own recommended shape trains operators to skip warnings, and the cost falls on
the true ones. For `path` rules only, a trailing `(/|$)` is now read as the end anchor it
stands for. In a command a slash is not a boundary, and a universal body such as
`^.*(/|$)` is still warned.

#### A remedy that pointed at a deleted surface

The deployment report's remedy for `deployment.core_rules_intact` told Root to run
`governance policy core-rule <id> true`, a command removed six days earlier. It was
changed to name the Policy section, and rewriting the test plan the next day found that
the Policy section offered no such control (finding 367, §3.5.85): the remedy only
became true with that fix. Finding 285's shape again: a removal audits what it deleted,
and not what referred to it.

**For Chapter 5:** closing an external review is a different act from having responded
to it. Most of its points had been closed for weeks, and the five still open sat among
them, where only reading each point against the code could find them.

**Evidence standard.** Tests: `kill-switch-confirm.test.ts` (2), `rule-conflicts.test.ts`
(5 new, 2 changed), `rule-extension-audit.test.ts` (2), `rule-warnings.test.ts` (4 new),
`governance-rule-request-preview.test.ts` (3), `rule-notices.test.ts` (5) and
`rule-request-queue-preview.test.ts` (2). **Ten mutations across the three code repairs,
all caught.** The first run of seven missed two, and neither was a surviving defect: one
expected-failure string named an assertion instead of the test's title, and one was a
real test gap, a "decided request carries no preview" test that left nothing pending, so
the route never loaded the policy and the preview was absent for the wrong reason.
Committed on 2026-09-14 as `aa578723625` and `3170e64eae0`, and the remedy text with
finding 367 in `81ff780fcc9`.

### 3.5.87 Three days checked again: a released name in three more stores, and what deletion leaves on the host (findings 369–371)

Once documents 10 to 12 were rewritten against the code on 2026-09-14, Kinan asked for
the work of the previous three days to be checked again. Two axes carried it: **the
documents' claims read against the code**, which had found 367 the day before, and
**composition of the newest fix with every other store holding state about the same
identifier**, which 366 had been, applied one level further.

#### A control whose only outcome is a refusal (369)

T4 moved per-agent posture to the Administrator floor, and the Policy section hid its
control below Administrator. The _Active agent sessions_ row kept its own copy of the
control gated on `canStop`, which a User holds for their own agents, so a User was shown
**Observe** and `policy/agent-mode` refused every press. Rewriting `GOVERNANCE.md` §5
exposed it, because the section's own sentence said the control appeared for a User.
This is the shape of findings 197, 247 and 249–250: two copies of one control, gated
separately, and only one of them moved when the rule did.

#### One identifier, four stores, one test (370)

T55 decided what an agent id carries forward after deletion: nothing. Finding 366 applied
that to the rule-request queue by checking, at approval, that the request's agent was
registered. The check answered "is this id registered?" when the decision needed "is the
agent registered under this id the one the request was about?", and the two separate the
moment the name is reused. Three stores hold questions about an agent id, and all three
behaved the same way once the id was deleted and registered again:

| Store                 | What the old question could do for the new agent                                                                      |
| --------------------- | --------------------------------------------------------------------------------------------------------------------- |
| Rule-request queue    | Be approved, writing the rule or setting onto the new agent (366's check passed, because the id was registered)       |
| Dashboard escalations | Be answered "Allow once", running the deleted agent's pending action as the new agent (T68's route checked the group) |
| Held decisions        | Be allowed, filing a rule proposal for the new agent (the route checked neither registration nor time)                |

Each was proved by a test driving the real `deprovisionAgent` and the real routes before
the repair. The repair is **one predicate, not three checks**: `registrationPredates`
compares the registration now under the id with the time the question was asked (a
request's `requestedAt`, an approval's `createdAtMs`, a held decision's `timedOutAt` less
its `waitedMs`). A registration newer than the question means the question was about
somebody else. Registration time is written only by `registerAgent`, so renaming or
re-owning an agent does not disturb it, and an unreadable time on either side answers
"predates", so no decision answerable before this check is blocked by a malformed record.
The refusal is always of the affirmative answer: a stale request can still be rejected,
a stale held decision denied, and a stale escalation waits out its timeout, which refuses
the action. Moving the held-decision answer route beside its read in
`governance-dashboard-oversight.ts` kept `governance-dashboard-api.ts` under its 700-line
limit, the subject moved whole rather than the count suppressed.

**For Chapter 5:** a decision about what an identifier carries must be applied to every
...
saying the dashboard removes them. No installation in use can hold such an account, so
the finding is recorded and the documents and the code comment corrected; building a Root
control is left as a decision. Finding 285's shape once more: a removal audits what it
deletes, and what referred to it is found later.

#### What deleting an agent leaves on the host (a candidate, not a finding)

Governance's "delete from host" (`deprovisionAgent`) calls `deleteAgentConfigEntry`, the
roster mutation, and nothing else. OpenClaw's own `agents.delete` wraps the same mutation
in a deletion journal, removes the agent's scheduled jobs
(`removeAgentJobsTransactional`), its host exec-approval policy
(`withAgentExecApprovalsRemoved`) and its session store entries
(`purgeAgentSessionStoreEntries`, upstream #65524), and by default moves its workspace,
agent directory and session transcripts to the trash. M6 composed provisioning from the
host's `createAgent` precisely because a naive write would miss the journal and the
directories, and the same argument was not applied to deletion. T55's own comment, "Here
OpenClaw has already deleted the agent", assumes the host half happened. Whether a new
...
The governance suite on the repaired tree: **3,116 passed, 21 skipped, 0 failed**, in 186
file runs. The seventeen tests over the previous figure of 3,099 are the new ones, with the
two gateway files counted under each of their three Vitest projects. The whole `ui/src`
suite: 8,204 passed, and its five failures are A10's, each confirmed by its error
(`object.stream is not a function`, under jsdom) rather than by the count. OpenClaw's
harness suites: 263 passed. **A mutation sweep reverted each protection and watched the
named test fail by its title: 8 of 8 caught.** Two of the eight were over-broad controls,
a check that refuses every question, which must break the tests asserting that the current
agent's questions stay answerable.

**A bound was measured rather than assumed.** Since 2026-09-13 the rule-request queue
works out, for every pending request, the warnings and clashes approving it would produce,
against every rule, and the dashboard reads the queue on each fifteen-second refresh, on
the Gateway's one thread. Seeded at both caps, 1,000 rules and 500 pending requests, and
read as Root five times, it answered in 34 to 43 ms. It is recorded as a measurement, not
a finding.

### 3.5.88 A capability kept by the tier that holds it, and out of its reach (A12)

T4 moved per-agent posture and escalation to the Administrator floor, and gave the User a
request in their place (§3.5.84 built the form). The User's side was checked for refusal;
the Administrator's side was never checked for reach. The Policy section drew **Use
default** on each existing escalation override, and nothing called `setAgentAsk` with a
value, so an Administrator could create an override only by recording a User-style request
and approving it, or by hand-written HTTP. Posture already had **Observe one agent**. The
standing reading of requirement #2 (§4.x.18) applies: a setting reachable only by a detour
does not let the person the requirement names configure the policy.

The control is **Escalation for one agent**, a picker of the agents the account manages and
**Ask a human** / **Deny**. Three decisions, each with a precedent:

- **One gate for both per-agent rows.** The posture row moved whole into the new module,
  `panels/policy-agent-overrides.ts`, and both rows take the section's `canEditPostures`,
  because one control drawn in two places and gated separately is findings 197, 247,
  249–250 and 369. The move also kept `policy-panels.ts` under its 700-line limit: the
  subject moved whole, the rule not suppressed.
- **Picked, not typed.** The A11 form's choice, for finding 327's reason: a typed id that
  matches nothing yields a setting nothing reads. A draft naming an agent no longer offered
  counts as no choice.
- **No confirmation**, because **Observe one agent**, the wider of the two changes, asks
  none, and neither value is irreversible.

It was tested through the rendered row with a stub client and through the mounted page,
and **a mutation sweep caught 7 of 7**: the gate ignored, the values swapped, the buttons
live with no agent chosen, a stale draft acted on, the page dropping the draft, the
section not drawing the row, and the row drawn for a User. On the final tree, with 369
and 370 beside it, the governance suite passed 3,123 tests with none failing, the browser
project 199, and the four typechecks and the full lint gate exited 0.
