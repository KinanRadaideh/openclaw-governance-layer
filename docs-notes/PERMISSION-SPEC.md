...
**With one organisation per installation, the organisation check does two jobs** (T49,
decided 2026-09-15). Between two live organisations it is separation that only the test
suites exercise, and the report does not claim it. Within the one organisation it is also
a registration check: an agent id with no registry record belongs to no organisation, so
`requireAgentInGroup` refuses it. The contract above stands as written;
`docs-notes/CHAPTER3-MATERIAL.md` §3.5.89 records the decision.
