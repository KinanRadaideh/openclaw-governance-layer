...
| 2.2.13 | Put a file in an agent's working folder, then **Delete the agent…** and **Delete the way OpenClaw does** | The row says how many folders went to `.Trash` in the Gateway account's home folder, and the file is there. An agent created afterwards with the same name gets **no** such notice                                     |
