...
| 3.2.12 | **New 2026-09-11 (T63).** Send a prompt that will take a while, then **reload the page** before it finishes and reopen the same agent | The task is still there: the conversation shows it as **Running** with a **Cancel** button, and _Active agent sessions_ lists the same task. If the only control left is the emergency stop, T63 has regressed                               |
