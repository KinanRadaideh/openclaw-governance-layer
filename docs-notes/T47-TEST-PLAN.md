...
**Checked live again on 2026-09-19, this time with a model connected** (qa-lab's mock
OpenAI server, so replies and tool calls are scripted, not a real model's), as Root, two
Administrators, a User, a User with no agents and a Viewer, two of them signed in at once in
separate browser sessions. **Pressed and passed:** 3.1.1, 3.1.2, 3.2.1 (the reply; not the
streaming, which the mock cannot show), 3.2.4, 3.2.5, 3.2.9, 3.2.12, 3.2.13, 3.6.4 (as the
Viewer, and the page's chain head matched `node scripts/verify-ledger.mjs`), 6b.1–6b.4,
6c.1, 6c.2, 6c.4, 6c.7, 6c.10, 6c.11, 6c.13, 6d.5, 6d.6, 6d.9, 6d.10 and 6d.13. It found
**377–379, all fixed** (§6e below), a decision for Kinan, **C15**, and one measurement that
bounds a claim: the kill switch's _confirmed_ stop took 2.2 s and 2.8 s on that laptop (its
signal, 1.3 ms and 6.8 ms), so **re-measure 6c.7 on the VPS and write both numbers down**.

**What that check could not do, so it is still yours**, flagged at each section below:
**screenshots** (the browser it drove had no visible window, so none exist for Chapter 4);
**three people on three machines** (it used one browser with two separate sessions);
**a real model** (T2 is still the only one); the **streaming** half of 3.2.1; **attachments**
(3.2.3); **anything on a chat channel**; **6b.5**, **6c.5**, **6c.6**, **6c.8**, **6c.12**; the
request-queue previews and deletions **6d.3, 6d.4, 6d.7, 6d.8, 6d.11 and 6d.14–6d.16**; **2.2.14**
and **3.2.14**; 3.1.6 and 3.1.7 (assignment changing while signed in); and **everything on
Linux** (T3).

**Checked live on 2026-09-18**, on an isolated Gateway driven in a browser as Root, two
Administrators and a locked-out account. That check had **no model connected and no User
account**, so it could not press anything that sends a prompt, raises an escalation or needs a
User signed in. **Those rows were the priority; the check of 2026-09-19 above pressed most of
them**, and the ones still never pressed are the priority when you run this plan. Each section that holds some says so under its heading: 2.2, 3.2, 6b, 6c and
6d. The one that needs no model at all, and matters most, is **6c.4** (the browser cache,
finding 346, security). Rows 1.2.17, 5.7 and 5.8 were added the same day.
...
| 1.2.17 | Look on an agent's row for a way to **rename** it, or to give it to **another Administrator** | **None exists yet** (**A13**, open): the server can do both and the page cannot. Write down whether you needed it, for example to hand over the agents of an Administrator who leaves; that decides whether A13 is built      |
...
| 1.8.3 | Type your Root username and confirm | A window asks how the agents are deleted, both ways explained. Choose one: everything goes, and you land on the create-the-first-account screen |
...
> **Not pressed by the live checks of 2026-09-18 or 19:** **2.2.14** (a full delete while a
> task runs). The 2026-09-19 check had a model but did not delete the agent it was using.
...
| 2.2.7  | Press Remove on an agent                                                                                 | **Two named options** with their consequences. _Remove from governance_ asks for a confirmation; _Delete the agent…_ opens a choice of two deletions, neither pre-selected, each explained                                                                                                                                                                                 |
| 2.2.8  | Choose "unregister"                                                                                      | The governance record goes; the OpenClaw agent stays, **with its rules, posture and lockdown kept** (T55)                                                                                                                                                                                                                                                                  |
| 2.2.9  | Choose "delete" on another that has a rule and a posture override                                        | Choose **Delete from OpenClaw's agent list only**. Both go (T55), Agent permissions for that id then shows nothing, and the row says the agent's files, history, scheduled tasks and approvals are still on the server                                                                                                                                                     |
...
| 2.2.12 | After 2.2.9, create an agent with the **same name**                                                      | Created, and the notice says what it picked up from the deleted agent (files in its working folder, and so on). No such notice is finding 372                                                                                                                                                                                                                              |
| 2.2.13 | Put a file in an agent's working folder, then **Delete the agent…** and **Delete the way OpenClaw does** | The row says how many folders went to `.Trash` in the Gateway account's home folder, and the file is there. An agent created afterwards with the same name gets **no** such notice, unless the deletion's row said OpenClaw left something behind, in which case the notice names that. A leftovers notice after a deletion that reported nothing left is finding 376 back |
| 2.2.14 | Start a long task on an agent and, while it runs, choose **Delete the way OpenClaw does**                | Refused, saying it is still working **and** what to do: stop it first. A refusal without the second half is finding 374                                                                                                                                                                                                                                                    |
| 2.2.15 | Press **Delete the agent…**, then **Keep this agent**                                                    | Nothing is deleted; the agent is still listed                                                                                                                                                                                                                                                                                                                              |
...
> **Pressed live on 2026-09-19** with a mock model: 3.2.1 (the reply, not its streaming), 3.2.4,
> 3.2.5, 3.2.9, 3.2.12 and 3.2.13, all passing. **Still never pressed:** the streaming half of
> 3.2.1 (the mock answers in one piece), **3.2.3** (an attachment), 3.2.8 with a real model, and
> 3.2.14. **3.2.4 on a slow machine:** the refusal reached the caller 17 seconds after it was
> recorded; the ledger had the prompt and its refusal in order.
...
| 5.7 | Kinan creates a second **Administrator** account for this row, and whoever signs in with it looks at the row for **Mohammad's** agent in _Agents in your organisation_ | The row names Mohammad as the owner and offers **nothing**: no _Remove…_, no _Allow Codex_, no _Register_. Any one of the three is finding 375 back: the server refuses all of them to an Administrator who does not own the agent |
| 5.8 | Mohammad, then Kinan, look at the same row                                                                                                                             | Mohammad, the owner, sees **Allow Codex** and **Remove…**; so does Kinan, because Root may act on any agent, which is how the agents of an Administrator who leaves are re-homed                                                   |
...
> **Pressed live on 2026-09-19:** 6b.1–6b.4, all passing (6b.2 was also pressed by Kinan by
> accident, on a card that had already expired). **Still never pressed: 6b.5**, which needs an
> Administrator cancelling a User's task from another machine. **Note for 6b.2 and 6b.4:** the
> request that appears says _requested by hitl-approval_, not by the account that pressed;
> decision C15 asks whether that should change.
...
> **Pressed live on 2026-09-19:** 6c.1, 6c.2, 6c.4, 6c.7, 6c.10, 6c.11 and 6c.13, all passing.
> **Timings to expect on a slow machine:** a card appeared 2 seconds after the escalation, but
> the escalation itself came about 30 seconds after the prompt on a cold Gateway; 6c.11's card
> closed 17 seconds after Cancel, 8 of them the Gateway recording the cancel. **Still never
> pressed: 6c.5, 6c.6, 6c.8 and 6c.12.** **6c.7, write both numbers down:** the notice reports
> the signal time and the confirmed-stop time, which measured 1.3 ms and 2,170 ms on the laptop.
> Requirement 7 says one second, and the VPS measurement is the one Chapter 4 should quote.
...
> **Pressed live on 2026-09-19:** 6d.5, 6d.6, 6d.9, 6d.10 and 6d.13, all passing, and Approve
> and Reject pressed by hand for the first time. **Still never pressed: 6d.3, 6d.4, 6d.7 (by
> `curl`), 6d.8, 6d.11 and 6d.14–6d.16.**
...
## 6e. Rows from the live check of 2026-09-19

Each row fails if its finding is back; 6e.4–6e.6 check C15 and A13, built the same day.

| #        | Who                            | Do this                                                                                                                                                                  | Expect                                                                                                                                                                                                         |
| -------- | ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **6e.1** | Kinan, at the VPS's terminal   | Let an escalation lapse unanswered, then read the Gateway log (`journalctl --user -u openclaw-gateway` or the log file)                                                  | **No** _"plugin approval follow-up reporting failed"_ line. One after every unanswered escalation is finding 377                                                                                               |
| **6e.2** | Malek, as the Viewer           | Read **What this account can do** in Identity, then Rule requests                                                                                                        | The sentence says you read the requests for your agents and for rules binding every agent, unmasked. A promise of "the rule requests queue in full" over a shorter queue is finding 378                        |
| **6e.3** | Mohammad                       | In **Request a change for one agent**, open the agent list, on an installation where OpenClaw has an agent governance never registered (`main`, if nobody registered it) | Only registered agents are offered. With `curl`, a setting request naming the unregistered one is **409**, _"… is not registered with governance …"_, and nothing is queued (finding 379)                      |
| **6e.4** | Malek, then Mohammad           | Malek gets his agent to attempt something no rule covers and presses **Always allow** on the card; Mohammad opens _Rule requests_                                        | The request reads _requested by malek, answering an escalation_, not _hitl-approval_ (C15). The same for **Would allow** on a lapsed question                                                                  |
| **6e.5** | Mohammad                       | On his own agent's row press **Edit…**, change the display name, **Save name**                                                                                           | The row shows the new name beside the same id, and a notice says what it was renamed to. Another Administrator's agent shows **no Edit…** (A13)                                                                |
| **6e.6** | Kinan, with Mohammad and Malek | **Edit…** on Mohammad's agent, choose a new owner, **Change owner**                                                                                                      | A confirmation first, saying Users and Viewers who answer to Mohammad lose the agent. After it, Malek's conversation says the agent is no longer assigned to him, and the new owner sees **Edit…** on it (A13) |
