...
| 1.2.11 | Open the **Accounts** panel on the server's dashboard                  | The same people, with their ids. **Cross-check it against the screen**: two surfaces disagreeing about who exists is the thing this row is for                                                                                                                       |
...
**New 2026-09-08: do this while Malek is signed in and watching.** Withhold
authoring from him, then restore it, and have him watch without reloading. His
rule buttons must disappear and come back within about fifteen seconds, and his
Identity panel must change with them. If nothing happens until he signs out and
in again, that is finding **305** back — and with it finding **301**, which is
the fix this one completes.
...
**New 2026-09-08, and it needs Malek on his own machine, signed in and not
touching anything.** Assign him an agent, then take it away, and have him watch
his **Your agents** section each time without reloading. Both must land within
about fifteen seconds. This is finding 305: the page used to read the account's
tier and assignments **once, at the first paint**, and never again — so
everything you do here was invisible to the person you did it to until they
...
| 2.7.9  | **New 2026-09-07 (finding 279).** Make the agent trigger an escalation by **reading** a file — not writing one — and answer **Allow always**. Approve the proposal, then ask the agent to **write that same file** | The write must still be refused or escalate. If it is allowed, 279 is back: the proposal has stopped carrying the direction, and approving a read has granted a write. Check the queue row too — it should say **(read)** in the grant itself, before the reason                                                    |
| 2.7.10 | **New 2026-09-07.** With that read grant approved, get the agent to escalate a **write** to the same file and answer Allow always                                                                                  | A **second** proposal appears. A read and a write of one file are two different grants; folding them into one would grant only whichever was asked for first                                                                                                                                                        |
| 2.7.11 | **New 2026-09-07 (finding 281, open as T60).** Answer Allow always on **more than twenty different** escalations without approving any of them                                                                     | Expect the queue to stop at **twenty** and the rest to file nothing. This is known and is a **decision, not a defect to report** — the call is still granted, nothing is widened, and the only record of the ones that did not file is a ledger entry. Note whether anything on screen told you; today nothing does |
...
| 3.1.5 | **New 2026-09-08.** Read the agent rows in **Your agents**                                                           | Each names the agent — `Scout Bot scout`, not `scout` alone. A bare id is finding 309 back                                                             |
| 3.1.6 | **New 2026-09-08. Two people.** Ask Mohammad to assign you **another** agent while you stay signed in and do nothing | It appears within about fifteen seconds, without a reload. If you have to sign out and back in, that is finding 305 back                               |
| 3.1.7 | **New 2026-09-08. Two people.** Ask Mohammad to take one **away** while you stay signed in                           | It disappears within about fifteen seconds. An agent that stays on screen with a working message box, and refuses when you use it, is finding 305 back |
...
| 3.2.9  | **New 2026-09-08.** Send a message and watch the panel **while it is answering**                                                                                                                                                | Your own message is shown straight away, above the agent's "replying…". If it vanishes, or the panel says "No messages yet" beside a live reply, that is finding 308 back                                                                    |
| 3.2.10 | **New 2026-09-08.** With several agents assigned, open the **first** one                                                                                                                                                        | The conversation appears under **that** agent's row, not below the whole list, and its id is not printed twice. Otherwise finding 307 is back                                                                                                |
| 3.2.11 | **New 2026-09-08.** Look at the width of the message box                                                                                                                                                                        | It spans the panel. A box about twenty characters wide, with the placeholder cut off, is finding 312 back                                                                                                                                    |
| 3.2.12 | **New 2026-09-11 (T63); passable only since 2026-09-12, when reloading stopped cancelling the task (finding 350).** Send a prompt that will take a while, then **reload the page** before it finishes and reopen the same agent | The task is still there: the conversation shows it as **Running** with a **Cancel** button, and _Active agent sessions_ lists the same task. If the only control left is the emergency stop, T63 has regressed                               |
| 3.2.13 | **New 2026-09-11 (T63).** Press **Cancel** on that recovered task, from _Active agent sessions_ this time                                                                                                                       | Both places change to **Stopping** together and neither Cancel can be pressed again; when it has stopped, it disappears from both. The agent is **not** locked down                                                                          |
| 3.2.14 | **New 2026-09-11 (T63).** While a task is running in this tab, try to open a different agent's conversation                                                                                                                     | A sentence tells you a task is still running here — rather than a composer that silently ignores what you type                                                                                                                               |
...
**And the half T56 added (2026-09-07 (ii)), which you also cannot reach by
hand.** The stack now carries a count of the unanswered questions it has shed,
and both the command line and the dashboard say so — the dashboard **even when
nothing is left waiting**, because a flood that was shed and then answered leaves
nothing waiting _and_ an incomplete record. Filling the stack takes 200 distinct
escalations, so this is driven by
`docs-notes/qa-sweep-2026-09-07/composition-sweep.ts`'s sibling tests rather than
by a person: six store tests and three panel tests, all run against reverted
code. **What is worth checking by hand is the opposite case**, and it costs
nothing: with a handful of escalations waiting and nothing ever shed, the pending
panel must **not** show any "this list is incomplete" notice. A warning that is
...
## 6b. Rows added 2026-09-11, from T60 and T63

Both features were built on 2026-09-11 and are tested at every step, but **no
live approval card has been pressed and no long task has been recovered by a
person** — the machine they were built on could not run a model. These rows are
where that gets proven. Record, as always, whether the outcome was **visible** and
whether a refusal **said what to do instead**.
...
| **6b.2** | Same                     | Press **Always allow**                                                                                           | The action runs **once**. A matching request appears in **Rule requests**. **No dialog appears afterwards** — the request saved, and the card had already said what the button does                           |
...
**Why there is no row for a full queue.** The limit is 40 requests plus 20 for
every account in the organisation — 120 with the four accounts §0 sets up — so
filling it by hand is not a test-plan row, for the same reason row 6.4 no longer
asks anyone to fill the pending stack. `src/governance/rule-requests.test.ts` and
`src/governance/escalation-allow-always.test.ts` drive the real queue past its
limit. **What a person checks is the other half:** that a request which _did_ save
produces no dialog at all.

**And one limit to observe rather than test.** On a chat deployment the
queue-full warning is not delivered to Discord or Telegram — see
`docs-notes/CHAT-DEPLOYMENTS.md`. If the demonstration runs through a chat
channel, do not expect it there.

## 6c. Rows added 2026-09-13, from the dashboard QA pass

Each row is a defect found and fixed in that pass (findings 346–363), or a decision built after it (T68). The row fails
if the defect is back. Row 3.2.12 and row 6b.5 could not have passed before it.

| #         | Who                                                                             | Do this                                                                                                                              | Expect                                                                                                                                                                                                                                                                                                                                                          |
| --------- | ------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **6c.1**  | Anyone with an agent, on the governance page                                    | From _Your agents_, ask the agent to do something no rule covers, with escalation on                                                 | Within a few seconds an approval card appears **on the governance page**, in a band above the sections, and not as the Control UI's pop-up (T68). **No card, followed by a row in _Awaiting your decision_ saying it timed out, is finding 347 back**                                                                                                           |
| **6c.2**  | Same                                                                            | Ask again, and leave the card unanswered until it expires                                                                            | After about two minutes the row in _Awaiting your decision_ says **timed out**. The cancelled case is row 6c.11                                                                                                                                                                                                                                                 |
| **6c.3**  | Anyone                                                                          | Press **Remove** on a rule, or any button that asks "Are you sure?", then press **Escape**                                           | The question closes and **you are still on the governance page**. Landing in chat is finding 349                                                                                                                                                                                                                                                                |
| **6c.4**  | Kinan                                                                           | Sign in, open every section, sign out. Then open the browser's developer tools → **Application** → **Cache Storage**                 | Only addresses containing `/assets/` (and the page itself) are stored. **Any address containing `governance` is finding 346 back**                                                                                                                                                                                                                              |
| **6c.5**  | Kinan                                                                           | While signed in, stop the Gateway for a minute, then start it again                                                                  | Within about 15 seconds the page says some panels could not be reloaded; pressing a button says _"Could not reach the Gateway…"_; once the Gateway is back, within a refresh that sentence is replaced by one saying it is reachable again and your last action may not have taken effect                                                                       |
...
| **6c.8**  | Kinan + a User                                                                  | With the User's conversation with an agent open, unassign that agent from them in _Accounts_                                         | Within about 15 seconds the User's conversation is replaced by a sentence saying the agent is no longer assigned to them. **A message box that is still there, and refuses, is finding 354**                                                                                                                                                                    |
| **6c.9**  | A User                                                                          | In _Rule requests_, paste a reason longer than 500 characters and submit                                                             | The field stops at 500 characters and its hint says so; the request that saves carries the whole of what the box shows. **A request that saves while the reason on screen was longer is finding 362 back**                                                                                                                                                      |
| **6c.10** | A User with an agent, an Administrator, and a Viewer, each on their own machine | The User asks the agent for something no rule covers                                                                                 | The card appears for the User and the Administrator, **and not for the Viewer**, and in nobody's Control UI pop-up. The Administrator presses **Allow once**: the card closes for both, and the _Audit ledger_ shows the approval answered by the Administrator's account. **A card on the Viewer's screen, or an answer recorded with no name, is T68 undone** |
| **6c.11** | A User with an agent                                                            | Ask for something no rule covers, and while its card is up press **Cancel** on the task in the conversation                          | Within a few seconds the card closes by itself, and _Awaiting your decision_ says **cancelled**. **A card still offering its buttons after the task stopped is finding 363 back**                                                                                                                                                                               |
| **6c.12** | Kinan + a User with an agent                                                    | The User asks for something no rule covers. While the card is up on the User's screen, change the User to a **Viewer** in _Accounts_ | Within about 15 seconds the card leaves the User's screen, along with the sections a Viewer does not see. **A card still offering Allow once to a Viewer is the stale card T68's finishing work fixed**                                                                                                                                                         |
| **6c.13** | Kinan + a User with an agent                                                    | The User asks for something no rule covers. While its card is up, Kinan engages the **kill switch** on that agent                    | Within a few seconds the card closes, and the User's conversation says the agent was stopped by the emergency kill switch. The kill switch's result reports one run aborted, not none. **A card still offering Allow once after the stop, or a result saying nothing was running, is finding 364 back**                                                         |
