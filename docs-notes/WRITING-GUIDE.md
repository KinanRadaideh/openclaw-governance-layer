...
open. As of 2026-09-13 no decision about approvals is open (T68 and T69 were both decided and built that day). **Two decisions opened on 2026-09-14** and are Kinan's: C13, what deleting an agent from the dashboard should remove on the host, and C14, whether accounts from before organisations get a way to be removed (finding 371). T3 (Linux) and T47 (the by-hand plan) can still produce findings. **Write Chapter 3 now, and leave
...
| Engineering detail          | `GOVERNANCE.md` (findings 1–134 in full, an index to 371), `mg/REMAINING-WORK.md`, `mg/REMAINING-WORK-DASHBOARD-SWEEP.md`, the two session logs | Checking a claim, finding a reproduction, recovering the reasoning behind a decision              |
...
**The newest design material**, 2026-09-12 to 2026-09-14: §3.5.83 (the dashboard
...
| ----------------------------- | ------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- |
| Findings found / fixed / open | The Findings cell of `mg/HANDOFF.md` §1's state table, checked against the registers                                | 371 / 369 / 2 (169 and 371 open; 365–371 found 2026-09-13 and 14, 369 and 370 fixed the same day) |
| Backlog items open            | Count the unstruck rows of `mg/HANDOFF.md` §6's open table                                                          | Eleven unstruck including T1, so ten open; A12, Claude's, built 2026-09-14                        |
| Governance test counts        | `node node_modules/vitest/vitest.mjs run src/governance/ src/gateway/governance-*.test.ts ui/src/pages/governance/` | 3,123 passed / 21 skipped / 187 files (2026-09-14, later)                                         |
| Linux test counts             | The same suite, run on Ubuntu                                                                                       | **Last measured before T44 — do not quote as current**                                            |
| Typechecks and lint           | `mg/HANDOFF.md` §4                                                                                                  | Four typechecks and the full lint gate 0 (2026-09-14, later)                                      |
...
| "Deleting an agent removes everything it had"                         | Governance's delete clears the id's rules, posture, overrides and lockdown (T55) and refuses old questions about it (366, 370); it removes only the agent's configuration entry on the host | On the host, OpenClaw's own delete also removes scheduled jobs, host exec approvals, session records and files; whether governance's should is decision C13 (§3.5.87)   |
| "Cancelling a task withdraws its approval", unqualified               | True for plugin approvals from 2026-09-13 (finding 363); proved by tests, not yet driven on a live Gateway or by hand                                                                       | The task, its record and its card end together; a reviewer who answered first keeps the answer (§3.5.83)                                                                |
...
| **C13** host deletion   | Chapter 3's agent lifecycle   | "Deletion clears governance's state for the agent and removes its configuration entry on the host"; not that it removes the agent's files, jobs or sessions |
| **C14** finding 371     | Chapter 5's limits            | Accounts from before organisations cannot sign in and are not removed by the dashboard; no shipped installation can hold one                                |
