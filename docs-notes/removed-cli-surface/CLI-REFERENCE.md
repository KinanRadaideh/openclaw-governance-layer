> # ⚠ This documents a surface that no longer exists
>
> **The `openclaw governance` command line was removed on 2026-09-07.** This
> file is kept, unedited below this banner, because it is the complete
> specification of what that surface did — including §2d, the list of
> capabilities that were deliberately _not_ on it and why, which is the most
> useful part if the surface is ever restored.
>
> **Nothing below is current.** Every command it describes is gone. The
> governance layer is administered through the dashboard and the HTTP control
> plane underneath it.
>
> See [`README.md`](README.md) in this folder for why it was removed, what was
...
| `governance policy set-agent-hitl-timeout <agentId> <seconds\|default>`                                                   | The same window for one agent. User tier, bounded by assignment                                                                                                                                                   |
...
| `governance accounts`                                                                                                     | **Root:** every account in your organisation, with the id `set-policy-authoring` and `agents set-owner` need. Group-scoped, exactly as `GET users` is (finding 255)                                               |
...
**A fourth was added 2026-09-04 (finding 255): the host resource view.** Found
by a mechanical parity sweep rather than by reading, which is the point of
running one. `GET system` reports CPU count, load, memory and uptime to the
dashboard and has no command, and until that date this section did not say so,
which is finding 223's shape a third time: a register promising _"the reasons
are here"_ with a reason missing. The reason is good, it was simply never
written down. See below. **The capability count is now three.**
...
**Listing them is a different question, and the answer used to be no by
accident** (finding 255). `governance accounts` was added 2026-09-04 and prints
every account in your organisation with its id, role, manager and
policy-authoring state, Root-only and group-scoped exactly as `GET users` is.

Listing does not carry the divergence cost the paragraph above describes,
because there is nothing to reimplement: no confirmation field, no password
rule, no role picker the server would refuse. It is a read.

**And its absence made this very section wrong.** The list below named
`governance set-policy-authoring` as something you can still do from here, and
that command takes an `<userId>` — an account id that **nothing on this surface
could print**. `organisation summary` reports counts and the Root's username;
`agents access` reports usernames. So a command this document offered as the
consolation for the missing ones could not be completed without opening the
dashboard. Finding 223's shape yet again, one layer in: not a capability
missing from the register, but a capability the register _claimed_ while it was
unreachable.

**What you can still do from here:** `governance accounts` lists them with
their ids, `governance set-policy-authoring` withholds or restores a User's
ability to write policy, `governance groups migrate --delete` removes accounts
that predate groups, and `governance organisation delete` removes **all** of
them at once.
...
### The host resource view

**Dashboard-only, and the reason is that the operating system already answers
it better.** `GET system` reports the platform, CPU count, load average, total
and free memory, uptime and this process's own memory. On the dashboard that is
worth a panel: an operator watching a runaway agent wants the machine's state
next to the sessions list, without leaving the page.

On a shell there is no gap to fill. `uptime`, `free -h` and `top` report the
same facts, more of them, live, and without a governance session. A command
here would be a worse version of tools the operator already has open.

**Recorded because it read as a gap until it was written down** (finding 255).
A parity sweep listed it as a capability the dashboard has and the command line
does not, which is exactly true and exactly not a defect. That is the whole
function of this section, and the entry existing is what stops the next sweep
finding it again.
...
| Mode      | Behaviour                                                                                                                                                                                                                             |
| --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `on-miss` | Pause and request human approval, routed into OpenClaw's existing approval flow. An `allow-always` answer permits that one call and **files a rule request** for an Administrator or Root to approve; it does not write a rule itself |
...
How long an escalation waits for a human before timing out, for the whole
installation. **Administrator and above.** Accepts 5–86400.
...
> **This was Root-only until 2026-09-03.** Widened at Kinan's direction: every
> other installation-wide policy setting (`set-mode`, `set-ask`) is
> Administrator, and the tier that answers an escalation is the tier that should
> be able to say how long one waits. Design doc §1.6 puts the window under Root;
> that section is preliminary design the implementation may differ from, so this
> is a recorded divergence rather than a requirement gap. `policy set-user-ask`
> stays Root, because naming a _person_ is account administration.

### `governance policy set-agent-hitl-timeout <agentId> <seconds|default>`

The same window for **one agent**, overriding the installation value. Accepts
the same range, or `default` to clear the override.

```bash
openclaw governance policy set-agent-hitl-timeout scout 1800
openclaw governance policy set-agent-hitl-timeout scout default
```

**User tier, bounded by assignment.** A User sets it for the agents assigned to
them; an Administrator for any agent in the organisation; Root inherits both.
That is the axis `set-agent-mode` and `set-agent-ask` already split on, and it
exists because "this agent waits longer" cannot be written on a number that
belongs to the whole installation.

Gated by `canManageAgent` rather than `canAuthorPolicyForAgent`: setting how
long your own agent's escalation waits is acting on a workload you are
responsible for, not changing the rules it is judged by, so a User whose policy
authoring Root has withheld keeps this (T27).

Clearing is expressed as the **absence** of a key rather than a sentinel, so an
agent with no override follows the installation value and moves with it.
...
**It says so when the list is incomplete** (T56). The stack holds a bounded
number of unanswered questions and sheds the busiest agent's oldest row to stay
within it. When it has ever shed one, this command prints a line saying how many
before the rows — and prints it **before the "nothing is waiting" line too**,
because both can be true at once: a flood that was shed and then answered leaves
an empty list and an incomplete record. The dashboard shows the same sentence
from the same count. Every escalation is in the audit ledger regardless; the
count is about your worklist, not about the record.
...
| Date       | Change                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 2026-09-04 | Added `governance accounts` (Root, group-scoped). Every account in your organisation with its **id**, which is the reason it exists: `set-policy-authoring <userId>` and `agents set-owner <agentId> <accountId>` both take an account id and **nothing on this surface printed one** — `organisation summary` gives counts and the Root's username, `agents access` gives usernames. §2d named `set-policy-authoring` as something you could still do from here while it could not be completed here, which is finding 223's shape one layer in: not a capability missing from the register, but one the register _claimed_ while it was unreachable. Found by a mechanical three-surface parity sweep (`docs-notes/qa-sweep-2026-09-04/surface-parity.mjs`) rather than by reading, which also added §2d's fourth entry, the host resource view, dashboard-only because `uptime` and `free` answer it better. **Creating, deleting, re-roling and password-resetting accounts stay dashboard-only**, for the divergence-cost reason §2d already gave; listing carries no such cost because there is nothing to reimplement (finding 255) |
