...
Section~\ref{sec:gov-requirements} states what the design was asked to do. This
section asks what each requirement demands of the design once it is read
strictly, and reports the status the system has reached against it. The
distinction matters because several of the requirements are short sentences
that carry a great deal of weight: two of them turn on what a single phrase is
taken to mean, and one of them was satisfied only after the first reading of it
was abandoned. Table~\ref{tab:gov-reqstatus} summarizes the outcome. Eight of
the nine requirements are met and the ninth, deployment on Linux, is partially
met, for reasons given in Section~\ref{sec:gov-remaining-reqs}.
...
\small
\begin{tabular}{@{}c p{6.1cm} p{3.9cm}@{}}
\toprule
\textbf{\#} & \textbf{Requirement} & \textbf{Status} \\
\midrule
1 & Node.js and TypeScript, with static type checking & Met \\
2 & Secure web dashboard with role-based access control & Met \\
3 & Default-deny model over paths, execution and network & Met \\
4 & Fine-grained, time-limited privileges & Met \\
5 & Record 100\% of actions, decisions and approvals & Met \\
6 & Tamper-evident audit logging & Met \\
7 & Suspend or terminate a session within one second & Met; measured in Section~\ref{sec:gov-latency} \\
8 & No plaintext secrets in log files & Met \\
9 & Deployable on Linux, open-source components only & Partially met \\
\bottomrule
\end{tabular}
\end{table}

Four requirements are treated individually below, because in each case reading
the requirement strictly either changed the design or changed what could
honestly be claimed of it. Requirement 3 turned on where the check is placed
rather than on what it decides. Requirement 5 turned on the meaning of
\emph{every action}, and the first implementation of it was replaced. Requirement
7 turned on what is being timed, the request to stop or the stop itself.
Requirement 8 was met by reusing an existing component rather than writing a new
one, which is a design decision and is presented as such. The remaining five
requirements are taken together in Section~\ref{sec:gov-remaining-reqs}.
