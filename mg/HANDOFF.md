...
> (no refusal's remedy had ever reached the dashboard), so **374 found, 372 fixed, 2 open**.
...
> decided and built on 2026-09-15, above.)_
>
...
>    and all but 371 fixed**, so the count now stands at **374 found, 372 fixed, 2 open** (169
>    and 371).
...
- **The C13 build was paused at one failing test, then resumed and finished (uncommitted).**
  ...
  **372–374 fixed**, so **374 found, 372 fixed, 2 open**. The record, with the checks run, is
