...
> ## Waiting on Kinan, first
>
> **Two items, both Kinan's word.** **A13** (below: rename and re-own from the dashboard, recommended
> build) and **C15**, new on 2026-09-19: a request filed by _Always allow_ or _Would allow_ says
> _requested by hitl-approval_, though since T68 the answer comes from a named account the ledger
> records. **Recommended:** attribute it to that account when the answer came from a signed-in
> governance session (`mg/REMAINING-WORK.md` §"The live QA of 2026-09-19").
>
> ## The open gap
>
> **A13: an agent cannot be renamed or given to another owner from the dashboard.** The server has both (`POST agents/rename`, `POST agents/owner`, ownership-checked with Root exempt, and audited), and no control on the page calls either, so both take hand-written HTTP. The one that matters is re-owning: when an Administrator leaves, Root is meant to re-home their agents (the reason `mayAdministerAgent` exempts Root), and today it cannot from the page.
>
> **Recommended: build it** — small, and Claude can do it alone once Kinan says so. The task, with
> what exists and what to build, is `mg/REMAINING-WORK.md` §"A13"; the sweep register lists it as
> **A13**. The alternative is to record the omission as deliberate and say in Chapter 3 that
> renaming and re-owning are API-only. Found on 2026-09-18 by driving the dashboard live.
...
> open**. It opened **C15** (top of this file) and measured the kill switch's confirmed stop above one
...
> files), taking the totals to 376 and 374 with one open that day. It also found **A13**, at the top of this file. The
...
> open, A13** (top of this file), waiting on Kinan's word. The documentation continues at 16. `FIRST-RUN`, 17. `HANDOFF`, 18.
...
- **C15, Kinan's:** who a request from _Always allow_ is attributed to. Recommended: the account.
...
- **376, fixed:** the full delete promised the next agent of the name starts with nothing, and
  ...
  reported.
- **A13, open:** rename and re-own have routes and no control (top of this file).
...
- **Documents 10–15 rewritten against the code:** `GOVERNANCE.md`,
  ...
  **Next in the order: 16. `FIRST-RUN`, 17. `HANDOFF`, 18. `SESSION-LOG-2026-09`.**
...
> **Picking up after 2026-09-13, in this order:** this file's §1 (the state table
> and the 2026-09-13 entry), then §6 and §8 (what is left, and what to do first),
> then §7's caveats 27–29. Then `mg/REMAINING-WORK-DASHBOARD-SWEEP.md` from its top
> box through §"C", and its §"The dashboard QA pass". Then
> `docs-notes/WRITING-GUIDE.md` if you are writing the report, or §4 of this file
> before you run anything. **New to the project entirely:** `docs-notes/FIRST-RUN.md`
> first, then the table below.
...
| ------------------------------------------------------- | ------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `REMAINING-WORK.md` §"The numbered backlog", **T1–T62** | The original project, plus everything added since             | **47 struck, 15 open** (47 + 15 = 62). Re-derived 2026-09-07 (iii) after T51 and T62 closed and T61 was added. T1 is open but not being done; T41 was cancelled and is struck. **If you re-derive this with a grep, T13 will lie to you:** it carries a struck row reading **DRAFTED**, for the drafting sub-task, while T13 itself — reading the answer until you can give it without notes — is open. A machine counting `~~**T13**~~` reports 45/15; the answer is 44/16. Re-derived from the rows 2026-09-05, finding 259: this cell read "T1–T53 … 39 done, 12 open" while §1 read "54 items, 42 struck … 11 open", and neither matched the table |
...
**What occupies it now, as of 2026-09-13: rebuild the VPS.** Commit and push are done:
T60, T63, the QA pass's fixes and these documents are on `personal/governance-layer`
at `0c15e73a778`. The VPS has none of it, including finding 346's security fix.
