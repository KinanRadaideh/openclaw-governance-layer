...
**Written 2026-08-19. Current as of 2026-09-08.** The single entry point
...
> five minutes, read §1's **2026-09-07 (iii)** entry alone — it is the one that
> changed what the product is. **If you are about to touch the dashboard, read
> the 2026-09-08 entry instead**: three sections were driven by hand from all
> four tiers and it is the only pass that has ever done that. If you have fifteen,
> add **§7's caveats 18, 19 and 22**: how a tamper-evidence feature's own output
> came to be taken on trust and what was done about it; the most reliable QA
> lesson this project has produced; and why deleting something is harder to
> verify than adding it.
>
> ### The four things that changed most recently
>
> 0. **The dashboard was driven by hand, section by section, from all four
>    tiers** (2026-09-08). **Five of thirteen done**: Identity, Accounts,
>    Agents in your organisation, **Your agents**, and **Active agent
>    sessions**. **Thirty findings, 296–325; twenty-nine fixed, one open as a
>    decision (T63).** §6 carries a **table of all thirteen sections and where
>    each stands** — read that if you are deciding what to do next. **Start at
>    _Agent permissions_, where one is already waiting.**
>
>    **The tier sweep is why it found what it did.** Every earlier dashboard
>    pass — 239, 240, 241–252, 264–268 — was done as **Root**, the tier every
>    string gets written for. Eight of these thirty are invisible from it.
>
>    **The four to read, and each teaches something different:**
>
>    - **296 — what a defect can cost.** A Root named `cli` bricked the
>      installation _and_ produced an administrative action that happened, was
>      reported as failed, and left no ledger entry. **T61 closed here** (297).
>    - **305 — what a sweep can miss.** The dashboard **never re-read who the
>      signed-in account is**: `refreshIdentity()` had one caller,
>      `connectedCallback`. So an assignment or an authoring permission changed
>      by Root never reached the screen of the account it was changed for —
>      **which made finding 301, one day old, half-delivered**.
>    - **319 — what a whole section can be missing.** A prompt started from
>      the dashboard never appeared in _Active agent sessions_, so design
>      requirement #2's real-time view was blank for the only way the User tier
>      can start an agent at all.
>    - **321 — what no source check can see.** The tree had stopped building,
>      and had been broken for a day, while three typechecks, both lint gates,
>      2,700 tests and four probes stayed green.
>
>    **Three things that change what you do first:**
>
>    - **Run `node scripts/build-all.mjs` before you trust anything** (321).
>      The VPS rebuild in §6 would have failed.
>    - **Run plain `oxlint` too** (323). That cell said 0 and the answer was 4,
>      three of them from the previous session. `git stash` and re-run proved
>      HEAD was clean. Do not read the cell; run the command.
>    - **T55 is decided and built**, by **Mohammad** on 2026-09-08, his first.
>      Deleting an agent clears what its id carried; unregistering keeps it.
>      **T64 is open and is the one to take next**: the clarity work these
>      sweeps produce is charged to the startup budget, and there are **60
>      bytes of headroom left**, measured 2026-09-09 rather than decremented.
>      **The reason recorded for it was wrong — finding 342.** Only `en.ts` is
>      loaded at startup; the other twenty locales are already lazy, so a
>      sentence costs one copy and not twenty-two. That makes the fix worth
>      **~170× the headroom that remains** (the governance strings are 10.5 KB
>      gzipped), and A5 on 2026-09-09 shows the cheap half of it: an
>      operator-facing sentence added for **zero bytes** by reusing one the page
>      already ships.
>
...
| **Backlog**             | **Re-derived from the rows 2026-09-09 (iii): thirteen unstruck in §6, one of which is T1 — so twelve open.** **T50, T55 and T64 closed** that day; **T63 and T64 were added** to that table having been open for a day in `mg/REMAINING-WORK-DASHBOARD-SWEEP.md` only, where T63 had also acquired a second number (T65). The count sat at fourteen through two closures and two additions: **a stable count is not evidence of a current list** (finding 343). The totals this cell used to carry — 62 items / 47 struck / 15 open — are **not re-derived here and should not be repeated**: a grep over `mg/REMAINING-WORK.md` counts prose cross-references as backlog rows and answers differently depending on the pattern. **Count from the table under §"The numbered backlog" and nowhere else** if the number is needed                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| **Findings**            | **345 found, 342 fixed, 3 open.** The three open are **169** (unexplained, never reproduced; T67 in the sweep backlog), **281** (**T60**, a decision) and **316** (**T63**, a decision). _Those last two are also written up as T66 and T65 — one item, two numbers, which is half of finding 343; quote T60 and T63._ **345** was added 2026-09-09 (iii) while checking T64 for regressions: the whole `ui/src` suite fails 27 tests across 16 files, all pre-existing at HEAD, none of them this fork's, and no command anybody runs had ever looked. Recorded rather than fixed (A10). (ii) added three, all fixed: **342**, the reason recorded for the startup budget was wrong by a factor of twenty — the twenty non-English locales are already lazily imported, so a sentence costs `en` once, and the correction is what made T64 worth doing; **343**, rows stale against _other rows in the same files_ plus two open tasks carrying a second number each; **344**, the test typecheck was red at HEAD while the row above said it was 0 — the third time a green cell in this table has described a red command, after 321 (the build) and 323 (the lint gate), and **all three were in this one table**. Before them, 09-09 (i) closed **331** and **333** and added **339-341**; (ix) added **337** and **338**. 258 closed 2026-09-08 (vi) when Mohammad decided T55 |
...
### 2026-09-09 (iii) (latest): T64 done — the page carries its own text
...
`LINUX-INSTALL.md`, `FIRST-RUN.md`) are current as of **2026-09-07 (iii)**, when
...
| **T60**     | You, then Claude                        | **Decide what "Always allow" should tell the operator, and what a full proposal queue should do** (finding 281, 2026-09-07). The button promises a permanence it no longer delivers, and all escalation proposals share one 20-slot budget for the whole organisation — past it a press grants the call and files nothing. Measured: 25 presses, 20 proposals. **A full queue never widens the policy; it only fails to propose**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| **T63**     | You, then Claude                        | **A control the server is already serving** (finding 316). `GET agent/runs` exists, is tier-gated and group-scoped, and its own comment says what it is for: _"so the dashboard can offer a cancel control for a prompt whose original tab is gone"_. `GovernanceApi.listPromptRuns` exists to call it. **No screen calls either.** Hit for real during the 09-08 sweep: a run in flight, the tab reloaded, and the only control left was the emergency kill switch — which the code beside `cancelPrompt` argues at length must not become the ordinary way out of an ordinary mistake. A **new control**, not a repair, which is why it was not taken in passing. _(Added to this table 2026-09-09; it had been open since 09-08 (iii) and was listed only in the sweep backlog, where it also had a second number, T65 — finding 343.)_                                                                                                                                                                                                                                          |
...
**What occupies it now: rebuild the VPS.** It is one command, it is in §6 under
