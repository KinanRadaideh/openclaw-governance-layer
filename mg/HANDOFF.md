...
**Written 2026-08-19. Current as of 2026-09-19.** The single entry point
...
> ## Latest, first: C15 and A13 built (2026-09-19)
>
> **Kinan decided both on 2026-09-19, as recommended, and both are built and tested.** **C15:** a
> rule request filed by answering an escalation (_Always allow_, _Would allow_) names the governance
> account that answered it, "requested by lina, answering an escalation", instead of the anonymous
> `hitl-approval`; the queue budget is unchanged. **A13:** the agent registry offers **Edit…** to the
> owner and Root: rename the display name, and, for Root, give the agent to another Administrator,
> with a confirmation saying who loses it. The record is `mg/REMAINING-WORK.md` §"C15 and A13 built";
> design §3.5.94, plain language §5.121, T47 rows 6e.4–6e.6.
>
> **Nothing is waiting on Kinan except the Chapter 3 go-ahead** and the team's items (T47 by hand,
> the VPS rebuild, T3).

> ## Start here
>
> **If you are Mohammad, or talking to him, read §0b first** — his baseline is
> `Grad_Proj___Current.pdf` and nothing after it, which is the report as it
> stood before essentially all of the implementation existed.
>
> **If you are about to write the report, start at `docs-notes/WRITING-GUIDE.md`.**
> The engineering reached a stopping point on 2026-09-11 and the project is moving
> into its documentation phase. A QA pass on 2026-09-12 and 13 then drove the
> finished features through a browser; it is committed and pushed, and it left two
> decisions, both taken and built the same day: T69 (finding 363) and T68. The writing guide maps every section of the report to the
> material that feeds it, lists every number with the command that re-derives it,
> and names the claims that must not be made.
>
> **Otherwise read §1 for the state, §6 for what is left, §7 for the caveats.** If
> you have five minutes, read §1's **2026-09-19**, **2026-09-18 and 19** and **2026-09-15** entries. If you have fifteen, add §7's caveats **5, 19, 22, 25, 26 and 27–29**.
>
> ### What changed most recently
>
> **2026-09-19 (later).** Kinan asked for it and the work below to be committed and pushed; whether that
> has happened is `git log --oneline personal/governance-layer..HEAD`, not a sentence here (finding 227). He asked for
> dashboard features to be QA'd live, the figures audited, then a commit and push. The dashboard was
> driven with a mock model for the first time, pressing most of T47's never-pressed rows, and it found
> and fixed **377** (a false warning in the Gateway log after every unanswered escalation), **378** (the
> Viewer's Identity sentence promised the whole request queue) and **379** (a setting request could be
> filed for an agent governance never registered, and never approved), so **379 found, 377 fixed, 1
> open**. It opened **C15**, decided and built later the same day (top of this file), and measured the kill switch's confirmed stop above one
> second on the laptop (caveat 5). Every figure in `docs-notes/FIGURES.md` was re-audited: fifteen
> corrected, all three forms present, the Mermaid parsed. §1's "2026-09-19" entry is the short version.
>
> **2026-09-18 and 19.** The work of 2026-09-11 to 15 was checked by
> driving the dashboard live on an isolated Gateway. It confirmed C13 end to end, the remedy on a
> refusal, 367, A12, the kill switch and the lockout, and found and fixed **375** (two registry
> controls offered to an Administrator who does not own the agent) and **376** (the full delete
> promised the next agent of the name starts with nothing, and OpenClaw can leave database side
> files), taking the totals to 376 and 374 with one open that day. It also found **A13**, built on 2026-09-19. The
> record is `mg/REMAINING-WORK.md` §"The QA over 2026-09-11 to 15, driven live"; §1's
> "2026-09-18 and 19" entry is the short version.
>
> **2026-09-15, COMMITTED AND PUSHED.** T48 answered yes; T49 decided (one organisation per
> installation); C13's leftovers proved as **finding 372**, then **decided by Kinan** and
> **built**: deleting an agent asks which of two deletions to run. Building it fixed 372 and
> found and fixed **373** (OpenClaw's own delete with files refused on Windows) and **374**
> (no refusal's remedy had ever reached the dashboard), and Kinan accepted finding 371 (C14).
> The record, with every check run, is `mg/REMAINING-WORK.md` §"C13 decided: two ways to
> delete an agent". The implementation landed in `4107a3605fe` and `48e9e56a3a5`; its record and the
> T48/T49 decisions landed in `298bf89b7fa`, followed by C14 in `d03ceaa6ab8`.
>
> **Two days of work, 2026-09-13 after the push and 2026-09-14, were committed on
> 2026-09-14** on top of `11d83e5808b`. Whether they are pushed is one command,
> `git log --oneline personal/governance-layer..HEAD`, and not a sentence here:
>
...
> - documents 10–15 rewritten, the last three days checked again, which found and fixed
>   **369–370** and left **371** open (decision C14), and **A12** built (§1's
>   "2026-09-14 (later)").
>
> **Never commit `.codex/`**, which is another agent's. **One item Claude can build is
> open, A13**, built on 2026-09-19 at Kinan's word. The documentation order finished on 2026-09-20 with 16. `FIRST-RUN`, 17. `HANDOFF` and 18.
> `SESSION-LOG-2026-09`, and two decisions are new and Kinan's: C13 and C14. _(C13 was
> decided and built on 2026-09-15, above, and C14 accepted the same day.)_
>
...
>    browser's service worker served one account's stored governance answers to the
>    next (346, security); escalations from a dashboard prompt had never reached a
>    person (347); and a closed tab cancelled the very task T63 exists to recover (350 —
>    Kinan decided tasks survive the tab). Every fix of 346–362 was re-driven on a
>    rebuilt Gateway and mutation-checked; the pass is on `personal/governance-layer`
...
>    The pass closed at 364 findings with 363 fixed; after the push, **365–374 were found
>    and all but 371 fixed** (371 accepted, C14), so the count stood at **374 found, 372 fixed,
>    1 open** (169); 375 and 376, found and fixed on 2026-09-18, moved the totals to 376 and 374, and
>    377–379, found and fixed on 2026-09-19, make it **379 found, 377 fixed, 1 open**.
>    **Six backlog items open** (T3 closed 2026-09-21, T46 on 2026-09-20; T48 and T49 answered on 2026-09-15); **no decision about
>    approvals is waiting** (T68 was built the same day: a dashboard escalation is answered
>    by the accounts that manage its agent).
>
>    **Three things that change what you do first:**
>
>    - **Read caveat 27 before writing about approvals** (T68): a dashboard escalation is
>      answered on the governance page by the accounts that manage the agent; a chat run's
>      approval is not.
>    - **Rebuild the VPS** (§8). It has none of this, including 346's security fix.
>    - **Run the whole of §4 before trusting any cell in this file**, and read §4's
>      2026-09-13 notes first: the lint gate's core shard now sits at its time cap, and
>      `build-all`'s last step times out on the Windows machine even when idle.
>
>    **Earlier:** T60 and T63 were built on 2026-09-11 from a draft another agent left
>    untested; the dashboard sweep finished all fourteen sections on 2026-09-08; T64
>    freed 9,019 startup bytes; findings 342–345 were about the record rather than the
>    product.
>
> 1. **The governance command line no longer exists.** Removed 2026-09-07 at
>    Kinan's decision — no design requirement asked for it, and it was the
...
>    `node scripts/verify-ledger.mjs`. No build, no server, no sign-in.
>    **Audited on 2026-09-07 (iv) and it had two defects, both fixed** — it
>    called a lagging checkpoint "AHEAD of the ledger" under the word INTACT,
>    and it accepted a key the product refuses and answered `BROKEN` where the
>    honest answer is "could not check". Both were **false alarms**, which its
>    own header calls worse than having no verifier. Findings 286 and 287.
>    **And the dashboard did not know it existed**: the row that recomputes the
>    chain still named `openclaw governance audit verify` for a day after that
>    command was deleted (**285**, the most consequential of the ten, because
>    that row _is_ finding 268's fix).
> 3. **The VPS still has the old build until it is rebuilt**, and therefore still
>    has the command line. See §6's "Do this before anything else".
>
> **If you want the narrative rather than the state**, `mg/SESSION-LOG-2026-09.md`
> §"2026-09-04: the day in one page" maps the seven entries that day produced.
> 2026-09-04 was the longest run of findings this project has had (241–255).
> **2026-09-05 to 2026-09-07 add six more entries and findings 256–284**;
> `mg/REMAINING-WORK.md` §"Where this file's findings stop" maps every finding
> from 241 to 284 to the section that holds it, and says which three are open.
> **2026-09-08 to 2026-09-11** add the fourteen-section sweep, T64, findings
> 342–345 and T60/T63; the session log carries each, and
> `mg/REMAINING-WORK-DASHBOARD-SWEEP.md` is the short answer to what is left.
>
> **If you are meeting the QA method rather than the product**, read §7's
> caveats 19 and 22 together. 19 is that an hour of somebody _using_ the
> dashboard has out-found a day of reading it on four separate occasions. 22 is
> its mirror: after a **removal**, every gate this project runs passes by
> construction, because deleting a surface deletes its tests — and the compiled
> surface sat in `dist/` for hours afterwards with nothing complaining.
> **Ten probe fixtures this fortnight were wrong, and six of those invented a
> defect rather than missing one.**
>
> **If you are going to the VPS**, read two things in §1: **"Updating the VPS to
> the current build"**, which is the commands that get this build onto the server
> and the steps after it, and the **VPS trip** checklist, which lists what to
> capture at each step and why each artefact matters. **The rebuild is now
> load-bearing rather than tidy**: until it runs, the server still serves the
> removed command line.
>
> ### Where things stand
>
> - **T2 IS DONE. The layer has been demonstrated.** On 2026-09-06 Kimi
>   (`kimi-for-coding`) drove agent `jack` on the VPS, was asked for `~/.npmrc`,
>   attempted the read, and **the gate refused it**. Ledger entry **#25** records
>   `read /root/.npmrc`, decision **deny**, naming the rule that did it:
>   `core-path-credential-files-env-private-keys-npmrc-netrc-re`. The chain
>   verified intact at 25 entries with the checkpoint agreeing. For eighteen days
>   this bullet said the opposite, and every other item was sequenced behind it.
>
>   **Read the attempt that failed first, because it teaches more than the one
>   that worked.** Asked for `~/.ssh/id_rsa`, the model refused _on its own_ —
>   "private keys stay private" — before making any tool call. Nothing reached
>   the gate and nothing was recorded, and **the ledger's silence was correct**.
>   A request that _sounds_ like a secret tests the model's safety training; a
>   boring-sounding file that is actually a credential tests this layer. Pick the
>   second when demonstrating, or you will screenshot the wrong thing.
>
> - **Built, verified, deployed, and now demonstrated.** Eight of nine design
>   requirements fully met. **2,780 passed and 21 skipped across 146 file runs,
>   green on Windows** (re-measured 2026-09-07 (iii), after the command line was
>   removed and the standalone ledger verifier added. The removal took it from
>   2,817 / 156 to 2,716 / 142 — a drop of **exactly** the 101 tests and 14 files
>   that went with the surface, which is the check that says nothing else went
>   with them — and T62 put 7 back in 1 file), **seven**
>   verification commands, all green, plus **twenty standing probe scripts** in
>   `docs-notes/qa-sweep-*/` that drive the layer rather than test it (counted, not
>   estimated: `ls docs-notes/qa-sweep-*/`). The seventh is new, and it is finding 250:
...
>   "no bugs left".** Sweeps on eight different axes, the VPS deployment, two
>   conflicts between sections of this file, and **an operator using the dashboard
>   on four separate days**, across 2026-09-01/07, found **one hundred and thirteen defects
>   between them, 172 through 284 inclusive (284 − 172 + 1 = 113). One hundred and
>   ten are closed; 169 stays open as an unexplained observation, 258 was open
>   as a decision and **closed 2026-09-08 when Mohammad decided T55**, and 281
>   stays open as a decision (T60).** _(This range stops at 284 on purpose.
>   285–325 came later and are counted in the state table above.)_ _(The security-relevant subset is deliberately
>   not counted here any more. It was carried as "twenty, and two more latent"
>   for several days by incrementing rather than by re-deriving, which is the
>   habit finding 227 exists to break. The register in `GOVERNANCE.md` §"Where
>   this register stops" lists the ones that cannot be skipped; count from it if
>   the number is needed.)_
>   **The last five are the ones to read if you are short of time**, because they
>   say something the other ninety-two do not: 264–268 came from Kinan _using_
>   the dashboard, and every one was **a capability the code already had and the
>   screen gave no way to reach**, three of them advertised in text while being
>   impossible to do. None was findable by reading, because the code was right.
>   **268 is the most important of them**: verifying the audit chain reported
>   "Intact, entries verified" and nothing else, so requirement 8's own output had
>   to be taken on trust. It now shows the chain head, the independent checkpoint
>   that agrees with it, and the terminal command that recomputes the same thing.
...
>   22 files and 198 tests**, which found 251 and 252 in its first minute: the
...
>   is **T47's plan**: `docs-notes/T47-TEST-PLAN.md`, 138 checks as first written, four added 2026-09-05 and **thirteen added 2026-09-07** for the night T2 landed (Root owning an agent, the agent knowing its name, the Talk panel, the empty reply, signing out, and the escalation's new "Allow always"), so 155, and **three more added 2026-09-07 (ii)** for finding 279 (approve a _read_ escalation, then try to write the same file), its de-duplication, and finding 281's queue limit — **158, counted from the rows rather than incremented**. Four added 2026-09-05** from findings 256 and 260, across
...
## 0b. Mohammad, and how to work with him

**A second operator joins from 2026-09-08: Mohammad Al-Masri**, one of the three
on the team (`PROJECT-SUMMARY.md` §1). Kinan's briefing, recorded verbatim
because the instruction is his and paraphrasing it would lose the parts that
matter:

> _"A new operator will come to you, Mohammad, who hasn't worked with you or
> previous agents before in developing this project, so his baseline knowledge
> will be different from mine; I, Kinan, have been building this over ~24
> sessions with different agents. When he comes he will identify himself. So when
> he asks questions explain things about the project, how it works and how it's
> built, from the ground up, with the understanding his knowledge is only up to
> the `____current` pdf file which is our most recent pdf work for the report
> we're writing. He will be working with you primarily to go through some tasks
> which require decisions; explain the decisions and choices in lay terms and
> from the ground up when he comes. He will also prompt you with whatever he
> needs."_

**He will identify himself.** Until he does, assume you are talking to Kinan and
§0 applies.

### What "his knowledge is only up to the pdf" means in practice

The file is `Documentation/GradProj/Grad_Proj___Current.pdf` — the authoritative
specification, and the one every requirement in this project is judged against
(`PROJECT-SUMMARY.md` §"Where everything lives"). It is also **the report as it
stood before essentially all of the implementation existed.** Everything the
repository records happened after it: the build itself, **325 defects found and
322 fixed**, **T2** (a real model refused by the gate on a real server,
2026-09-06), the **removal of the governance command line** (2026-09-07, which
changed the product from three surfaces to two), and the M-series multi-tenancy
work.

So when he describes how something works he may be describing the **design**
rather than the build, and be right about the document and wrong about the code.
That is not a mistake — it is the only source he has. **Say which of the two you
are talking about, every time**, and when they differ, name the change that moved
it rather than correcting him flatly.

### Answering him

§0's plain-language rule applies more strongly here, not less. Beyond it:

- **Name a thing before using it.** "The gate", "the ledger", "a posture" are
  this codebase's names, not the document's. He knows the concepts; not these
  words for them.
- **A finding number is not an explanation.** Say what happened, then give the
  number so he can look it up.
- **He is here mainly for decisions**, so prepare those the way the briefing
  asks: what the system does today, the options, what each costs, and a
  recommendation — in lay terms, from the ground up. **T55 and T60** in §6 are
  exactly this shape. A decision he cannot restate in his own words has not been
  made.
- **`docs-notes/QA-IN-PLAIN-TERMS.md` is the model** and the best thing to point
  him at; it was written to this standard on purpose.

### What happened when he did arrive (2026-09-08)

**He took T55, and the conversation improved the answer twice.** Recorded
because it says how to work with him, not just what was decided.

- **He rejected a proposal and was right.** The plan was to clear an agent's
  rules on deletion _and_ warn at registration. He asked the obvious question —
  if the first already clears them, what is left for the warning to warn about?
  — and working it through exposed a cost that had not been stated: the
  ...
  already knew. It became a clause on the existing confirmation instead.
- **He asked for the decision to be written out exactly before any code**, and
  that caught **finding 324**: what an agent id carries is **five** things, not
  the three anybody had measured. A decision about what a name carries was about
  to be taken on three-fifths of what a name carries.

**So: give him the options, take the pushback seriously, and write the spec
before the code.** Both improvements came from him, and neither would have come
from handing him a finished answer to approve.

**Do not hand him this file as an introduction.** It is a handoff for somebody
resuming the work and assumes the vocabulary throughout. The way in is
`docs-notes/FIRST-RUN.md`, then `PROJECT-SUMMARY.md` §1, then the relevant part
of `QA-IN-PLAIN-TERMS.md`.

---

## 1. The one-paragraph state of things

> **2026-09-21: requirement 9 is MET, T3 is closed, and Chapter 3 is three sections in.**
>
> **The Linux re-run was done**, on the VPS at commit `0a7d51f1c12`, which is what requirement 9
> had been waiting on since 2026-09-06. Kinan brought the box to HEAD, re-ran the installer and
> restarted the service, then ran all three checks:
>
> | Check                                       | Result                                                     |
> | ------------------------------------------- | ---------------------------------------------------------- |
> | Governance suite on Ubuntu                  | **3,211 passed / 16 skipped / 0 failed, 197 files, 818 s** |
> | Demonstration rehearsal                     | **20/20**                                                  |
> | Platform probe `governance-linux-check.mjs` | **14/14**, `platform=linux node=v22.23.2`                  |
>
> **The two platforms reconcile exactly.** Windows is 3,206 passed + 21 skipped and Linux is 3,211
> passed + 16 skipped: both are **3,227 tests**, so five tests that skip on Windows run on Linux and
> one extra file runs there. Nothing fails on either. That is the strongest form the claim can take,
> because it shows the difference is platform coverage rather than platform breakage.
>
> **Requirement 9 is therefore Met, all nine requirements are Met, and T3 is struck.** The
> "Verified on Linux" caution in `docs-notes/WRITING-GUIDE.md` §4 is retired.
>
> **Two things the daemon reported that are NOT requirement 9 and are still open**, both from
> `openclaw daemon restart && openclaw daemon status` on 2026-09-21:
>
> - **The connectivity probe fails.** `Connectivity probe: failed, timeout` against
>   `ws://127.0.0.1:18789`, while the same output says the runtime is `running (pid 132112, state
active, sub running, last exit 0)` and owns the port. The Gateway is up; something about the
>   probe's own credentials or config is not. **This matters for step 4 and for the demonstration,
>   because both need the dashboard.** Undiagnosed.
> - **Service config warnings.** The unit's PATH is missing `/root/.nvm/current/bin`, includes a
>   version manager, and runs Node from nvm, which `openclaw daemon status` warns can break after an
>   upgrade. It recommends `openclaw doctor --repair`. This is the same class of thing
>   `docs-notes/LINUX-INSTALL.md` §2c already documents.
>
> **Chapter 3** now has its opening, §3.1 (the nine requirements verbatim, at the supervisor's
> instruction) and §3.2's opening with Table 3.1, which gained a third column naming where each
> requirement is discussed. **The next task is the four subsections of §3.2**, and it is written up
> in `mg/HANDOFF.md` §2b.

> **2026-09-20 (later): two of Kinan's decisions taken and both built.** **C4** (T46):
> the setup wizard's banner and completion text now name the fork, say what the layer
> does and point at Settings → Governance and the first Root account; the prompts in
> between stay upstream's, which is the scope Kinan chose. **Seven strings** in
> `src/wizard/i18n/locales/en.ts`. The seventh is `findMeLater`, added after a
> reachability check: `wizard.guided.nextSteps` is printed nowhere, and the one
> next-steps call site prints `nextStepsWithoutAi` only when the operator skips AI
> setup, so without it the governance pointer missed everybody who connected a model.
> Pinned by five new tests in
> `src/wizard/i18n/index.test.ts` that all catch a revert, with two tests in
> `setup.finalize.test.ts` and one in `onboard-guided.test.ts` moved off the hardcoded
> sentence and onto the catalog. Four typechecks 0, type-aware lint 0, no new failures
> against a measured HEAD baseline. **T17**: MiKTeX installed and all fourteen kept
> figures compiled, **14 of 14 clean**, which found two defects four readings had missed
> (`docs-notes/FIGURES.md` §"2026-09-20"). **Neither is committed**; both wait on Kinan.
> One thing found and not fixed: `src/commands/onboard-guided.test.ts` has been failing
> 12 of its 23 tests at HEAD, unrecorded until today, and it is upstream's file, not the
> fork's (§4).

### Handoff state, 2026-09-20 (every cell re-derived or re-run that day)
...
| **Suite**               | **Re-measured 2026-09-20 after C15 and A13: governance suite 3,206 passed / 21 skipped / 0 failed in 196 files; the whole `ui/src` suite 8,241 passed / 5 failed (A10's five); the browser project 199 in 22 files; four typechecks 0.** Before that, **re-measured 2026-09-19 after 377–379: governance suite 3,187 passed / 21 skipped / 1 failed in 196 files, the failure a page fixture with no agent registry, fixed, that file then 12/12; the whole `ui/src` suite 8,229 passed / 6 failed (A10's five, and `app-host.test.ts`'s locale retry, which passes alone 3/3); the browser project 199 in 22 files; four typechecks 0 (`mg/REMAINING-WORK.md` §"The live QA of 2026-09-19").** Before that, **re-measured 2026-09-18/19 after the live QA's fixes: governance suite 3,176 passed / 21 skipped / 0 failed, 192 files passed and 2 skipped; the whole `ui/src` suite 8,227 passed / 5 failed (A10's five, each `object.stream is not a function`); the browser project 199 in 22 files.** Before that, **re-measured 2026-09-15 on the C13 tree: governance suite 3,169 passed / 21 skipped / 0 failed, 192 files passed and 2 skipped; the whole `ui/src` suite 8,221 passed / 5 failed (A10's five); the browser project 199 in 22 files.** Before that, **governance suite, re-measured 2026-09-14 (later) on the final tree, with 369, 370 and A12: 3,123 passed / 21 skipped / 0 failed, 187 files passed and 2 skipped** (`node node_modules/vitest/vitest.mjs run src/governance/ src/gateway/governance-*.test.ts ui/src/pages/governance/`). Earlier that day, after findings 366–368 were fixed: 3,099 / 21 / 0 across 179 files. On 2026-09-13 after the push: 3,034 / 21 / 0 across 164 files. After T68, before that work: 2,998 passed / 21 skipped / 0 failed across 157 files. Earlier that day it was 2,928 / 2 failed; the 2 were one source scan (`host-prompt-audit.test.ts`) pinned to text finding 347's fix changed, updated and passing. **The wider `ui/src/` suite, which no command in §4 runs (`node node_modules/vitest/vitest.mjs run --config test/vitest/vitest.ui.config.ts`): 8,211 passed / 5 failed across 572 files on 2026-09-14 (later)**, A12 included (8,202 / 5 earlier that day), the same five as after A10 (8,176 / 5 on 2026-09-13). Before it, 27 failed in 16 files (finding 345); 22 were fixed, and the 5 left are jsdom's `Blob.stream()`, in files the fork never touched. ui-isolated, on its own config (`test/vitest/vitest.ui-isolated.config.ts`): 403 passed, 1 skipped. The browser project: 199 passed in 22 files. The host suites in §4: 263 passed. All three re-run 2026-09-14 (later), A12 included                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| **Typechecks**          | **All four 0, re-measured 2026-09-20 after C15 and A13** (the two test typechecks failed first on a fixture that hand-listed the registry drafts, fixed), and before that 2026-09-18 after the live QA's fixes (and on 2026-09-15 and 2026-09-14 (later)): core, UI, `test/tsconfig/tsconfig.core.test.json` and `test/tsconfig/tsconfig.test.ui.json`. **The build is a separate check, and was not re-run on 2026-09-14.** **`build-all` exited 0 on 2026-09-13 (latest), every phase**, its last step reusing cached startup metadata rather than regenerating it. Before that it passed every phase, the startup budget included (316,840 B of 324,608 B), except its last, `write-cli-startup-metadata`, which times out on the Windows development machine even when idle; the measurements are in `mg/REMAINING-WORK-DASHBOARD-SWEEP.md` §"What could not be established here"                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| **Lint**                | **Re-measured 2026-09-19 after the live QA's fixes: the full gate exited 0**, with the raised cap. Before that, **re-measured 2026-09-15 on the C13 tree: the full gate exited 0**, with the raised cap, in about 31 minutes. Before that, **re-measured 2026-09-14 (later) on the final tree, 369, 370 and A12 included: the full gate exited 0**, with the raised cap, in about twenty minutes. Earlier that day the full gate and plain `oxlint` over `src` and `ui/src` exited 0 before findings 366–368 were fixed; after the fixes, plain `oxlint` and `oxfmt --check` over the fourteen files they touched 0, once one `no-map-spread` error was fixed. **Before that, the full gate exited 0 end to end on 2026-09-13 (latest)**, with the documented cap, about nineteen minutes: its first clean exit since 2026-09-07 (iii). On 2026-09-13 its first run found one type-aware error in the QA pass's own test, which was fixed; its second timed out on the core shard (exit 124, a timeout rather than a verdict: the shard needs about fifteen minutes against a 905-second cap here). The fix was confirmed through `scripts/run-oxlint.mjs` with a positive control the rule flagged. Raise the cap before the next run (§4). **T50: nothing runs the gate automatically**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
1. **The VPS is a different product from this repository** until it is rebuilt.
   It still serves the removed command line and lacks every fix since, including finding 346's security fix. One
   command, §6, "Do this before anything else".
2. **A document telling you to run `openclaw governance …` is out of date.**
   Every operator-facing one was corrected on 2026-09-07; historical narrative
   was deliberately left alone. If you find a live instruction, it is a bug in
   the docs — say so.
3. **A green suite proves less than it looks after a removal**, and this project
   has the scars: see §7 caveat 22.

---

### 2026-09-19: the dashboard driven with a model behind it (377–379, C15), and the figures re-audited

Kinan asked for dashboard features to be picked and QA'd live, anything not doable reported and
added to the T47 plan, the figures checked, and then a commit and push.

- **How:** the QA Gateway of 2026-09-18 (now `governance-gateway-qa6`, port 18825) with qa-lab's
  mock OpenAI server (`qa-mock-openai-6`), five accounts, two signed in at once on `localhost`
  and `127.0.0.1`. Outcomes read from the stores, the ledger and the Gateway log.
- **Passed:** a User's prompts, escalations reaching the right accounts only (T68), _Always allow_,
  approve and reject by hand, _Would allow_, a posture request approved (A11), cancel and kill
  switch closing a card, a task recovered after reload (T63), the concurrency cap, no Observe for a
  User, no cached governance answer after sign-out (346), and the chain verified from the Viewer's
  page and the terminal alike. `monitor` and the prompt lifecycle's order were confirmed live.
- **377, 378, 379, fixed:** a false log warning, a Viewer sentence wider than the queue, and a
  request that could never be approved.
- **C15:** who a request from _Always allow_ is attributed to. Recommended the account; Kinan agreed, and it was built the same day (§"C15 and A13 built" in `mg/REMAINING-WORK.md`).
- **Measurement:** confirmed stop 2.2–2.8 s on the laptop (caveat 5); re-measure on the VPS.
- **Not done:** screenshots, three machines, a real model, attachments, chat channels, some T47
  rows (listed at the top of `docs-notes/T47-TEST-PLAN.md`), and Linux.
- **Figures:** `docs-notes/FIGURES.md` §"2026-09-19": fifteen figures corrected, F3's Mermaid made
  to parse, F10's order fixed (and `CHAPTER3-MATERIAL.md` §3.5.17), fourteen recommended.
- **Record:** `mg/REMAINING-WORK.md` §"The live QA of 2026-09-19, with a model connected".

### 2026-09-18 and 19: the fortnight checked live (375, 376), and a gap found (A13)

**Held uncommitted until Kinan asked for a commit on 2026-09-19.** Kinan asked for the work of 2026-09-11 to 15 to be QA'd and for
the dashboard to be driven live as part of it.

- **How:** an isolated Gateway on port 18823 (launch entry `governance-gateway-qa5`, with its own
  state, governance directory and home), a rebuilt `dist`, and the dashboard driven in a browser as
  Root, two Administrators and the sign-in throttle; every outcome checked against `policy.json`,
  `agents.json`, the ledger and the disk. §4 has the two traps it hit.
- **Confirmed live:** the first-run form (205); C13 end to end, including OpenClaw's own delete
  moving folders to `.Trash` with real NTFS ids (373's fix) and the leftovers clause (372's fix);
  the remedy on a refusal (374); a core rule off and on (367); A12; the kill switch asking, locking
  and releasing; the lockout's wait (368); and deleting the organisation with its ledger kept.
- **375, fixed:** _Remove…_ and _Allow Codex_ were offered to an Administrator who does not own the
  agent, and the routes refuse them. **The first fix was wrong** (it revealed _Register_ on a
  registered agent) and only the live page showed it; the wider suite then caught an older test
  whose fixture had no owner name.
- **376, fixed:** the full delete promised the next agent of the name starts with nothing, and
  OpenClaw can leave the agent's SQLite side files; the promise is now honest and the residue is
  reported.
- **A13:** rename and re-own had routes and no control; built on 2026-09-19.
- **Not covered:** anything needing a model (A11 from a User, 369's row, T68, T63's reload); the
  QA Gateway had none connected.
- **Next:** Kinan's word on A13 and on committing; then the Chapter 3 outline.

### 2026-09-15: T48 and T49 decided, and C13 proved (372), decided and built (373, 374)

**Committed and pushed.** Kinan wants Chapter 3 written as soon as possible, and asked for the tasks
left, sorted by who can do them, with the strictly necessary ones marked.

- **T48 answered yes:** Chapter 3 is written now, and anything T3 or T47 changes afterwards
  is flagged in it.
- **T49, option (b):** one organisation per installation is the boundary, and the report
  makes no claim about separation between organisations that exist at once. Kinan asked
  whether the code for several organisations could be removed; checked against the code,
  it cannot be removed as a block, because it also keeps a deleted organisation's retained
  audit ledger apart from its successor, refuses unregistered agent ids, and enforces the
  cap. The reasoning is `docs-notes/CHAPTER3-MATERIAL.md` §3.5.89 and plain language
  §5.116; caveat 11 was corrected, since it said none of the structure is reachable in
  production.
- **C13, option (b) done: finding 372.** A probe on a real OpenClaw state directory
  (`docs-notes/qa-sweep-2026-09-15/`) proved the inheritance. Governance's delete left the
  agent's workspace file, its agent and session folders, its session database, its
  scheduled job and its approval settings, and a new agent under the same name, created by
  a different Administrator, received all of them. Medium, fixed by C13's implemented choice.
- **Two questions Kinan asked, answered in `docs-notes/CHAPTER3-MATERIAL.md` §3.5.90.**
  Option (a) would **not** remove the deleted agent's audit ledger entries: the ledger is
  governance's own file, outside everything OpenClaw's delete touches, and T55 already says
  the ledger is never touched. It would remove evidence the ledger does not hold, the full
  conversation history and the workspace files. And (a) is the option closest to
  Mohammad's T55 decision.
- **C13 decided by Kinan: both deletions, as a choice.** Deleting an agent opens a popup
  with _Delete from OpenClaw's agent list only_ and _Delete the way OpenClaw does_, each
  explained; Claude answered the eight follow-up questions with its recommendations
  (design §3.5.91).
- **The C13 build was paused at one failing test, then resumed, finished, committed and pushed.**
  The failure was OpenClaw's own `agents.delete` refusing on Windows: **finding 373**, fixed
  in the fork by keeping NTFS file ids exact. The end-to-end test then met OpenClaw's
  configuration-file guard, which now refuses with its own code and remedy, and building the
  refusals found **finding 374**: no refusal's remedy had ever reached the dashboard.
  **372–374 fixed**, and 371 accepted (C14). The record, with the checks run, is
  `mg/REMAINING-WORK.md` §"C13 decided: two ways to delete an agent".
- **Next:** the QA over the work since 2026-09-11, then the Chapter 3 outline when Kinan
  says to start writing. Not established: OpenClaw's delete with
  files on the Linux VPS (T3) or through a live Gateway.

### 2026-09-14 (later): documents 10–15, the last three days checked again (369–371), and A12 built

**Committed on 2026-09-14**, after the work beneath it had gone in first as seven commits
(`aa578723625` to `8f9408c0df0`). Kinan asked for that commit, then documents 10–12, a QA
over the last three days, documents 13–15, one task Claude could do alone and its QA, and
a commit and push, giving each the time it needed.

- **Documents 10–15 rewritten against the code:** `GOVERNANCE.md`,
  `docs-notes/QA-IN-PLAIN-TERMS.md`, `docs-notes/CHAPTER3-MATERIAL.md`,
  `docs-notes/FIGURES.md`, `docs-notes/WRITING-GUIDE.md` and `mg/PROJECT-SUMMARY.md`.
  **Next in the order: 16. `FIRST-RUN`, 17. `HANDOFF`, 18. `SESSION-LOG-2026-09`.** _(All three were done on 2026-09-20, which finished the order.)_
  `mg/PROJECT-SUMMARY.md` §6 still said the fork "has never been deployed to a VPS" and
  was "not yet demonstrated", eleven and eight days after each stopped being true.
- **The last three days checked again**, the fifteen commits from `778b769bef2` to
  `8f9408c0df0` with documents 10–12. **369**: a User was offered **Observe** on a live
  session, which the route refuses them. **370**: a question about a deleted agent, in the
  rule-request queue, the dashboard's escalations or the held decisions, could be answered
  yes for a new agent registered under the same name; one predicate,
  `registrationPredates`, now refuses the affirmative answer in all three. Both fixed.
  **371**, open and low: accounts from before organisations cannot be removed (decision
  **C14**). What "delete from host" leaves on the host is recorded as decision **C13**.
  The rule-request approval preview was measured at its caps (1,000 rules, 500 pending):
  34–43 ms.
- **A12 built:** **Escalation for one agent** on the Policy section, beside **Observe one
  agent** in `ui/src/pages/governance/panels/policy-agent-overrides.ts`, under one gate.
- **The record:** `mg/REMAINING-WORK.md` §"The QA over three days" and §"A12"; registers
  `GOVERNANCE.md` rows 369–371, plain language §5.114–§5.115, design §3.5.87–§3.5.88.
- **Verified on the final tree:** governance suite 3,123 passed, 21 skipped, 0 failed
  (187 files); whole `ui/src` suite 8,211 passed with A10's five jsdom failures;
  ui-isolated 403; browser project 199; host suites 263; the four typechecks and the full
  lint gate 0. Mutation sweeps: 8 of 8 for 369–370, 7 of 7 for A12. **Not established:**
  `build-all`, a live run on a real screen, and C13's inheritance question _(proved on
  2026-09-15: finding 372)_.

### 2026-09-14: documents 7–9 rewritten, and the week checked a second time (366–368)

**Committed on 2026-09-14 with the 2026-09-13 (after the push) work below**, in seven
commits, `aa578723625` to `8f9408c0df0`. Kinan asked for the
...
**Kinan asked for a QA check over all work done in the last week**, the 20 commits since
2026-09-05 plus 363 and T68, and for it to be committed and pushed. Three axes: the
verification set, composition across features, and the documents' claims.

- **Composition found finding 364, security.** The kill switch never stopped a dashboard
  prompt, and an escalation raised before the lock could still be allowed. Proved by a
  probe driving the Gateway's real terminator, fixed, 7 mutations caught. The other pairings
  (T68 with the service worker, with organisations, with role changes; 364 with T63) were
  right, or were fixed with T68. Record: the sweep register's §"Finding 364" and §"The
  week's QA check".
- **A10 is answered.** Finding 345's "none of the 27 is this fork's" was wrong for 19: tests
  never updated for the governance route and T52's copy, an undefined CSS token, and two
  comments a source scan reads. 22 fixed; 5 remain, jsdom's `Blob.stream()`.
- **Q4 is measured:** 50 agents, 21 accounts and 1,000 ledger entries cost every tier's
  refresh 20 to 30 ms of server reads, and render in 33 to 64 ms (Root's first mount 434 ms,
  cold).
- **The full lint gate exited 0** for the first time since 2026-09-07 (iii), and
  **`build-all` exited 0**, its last step reusing cached metadata.

**Committed in five commits** on `governance-layer`: 363, T68, 364, A10, and the documents.
The pre-commit hook re-adds staged files from the working tree, which folded 364's and
A10's hunks into the first T68 commit; the three unpushed commits were rebuilt so each holds
only its own finding, and the final tree was checked identical. Pushed to `personal`.

**Not established:** a live run of any of it on a real screen (T47 rows 6c.10 to 6c.13).

### 2026-09-13 (later still): T68 built — a dashboard escalation answered by the accounts that manage its agent

**Kinan chose (b) for T68 (sweep register C11).** An escalation raised from a dashboard
prompt had reached every Control UI holding the Gateway credential, so a Viewer could
press "Allow once". Now only the agent's assigned Users, the organisation's
Administrators and Root see it, on the governance page, and the answer is recorded under
the account that gave it.

**What was built,** in full in `mg/REMAINING-WORK-DASHBOARD-SWEEP.md` §"T68":

- one test of ownership (`governance-approval-scope.ts`);
- both Gateway authorization gates refuse such an approval to every connection except the
  approval runtime and its requester, **admin scope included**, and `approval.history`
  leaves it out;
- no forwarding to chat channels or phones;
- governance subscribes on the in-process approval bus and **claims the audience**
  (a generic `claimsAudience` on bus subscribers);
- two governance routes, `GET approvals` and `POST approvals/decide`, authorized with
  `canManageAgent` inside the organisation, answering as the Gateway's own approval
  principal and ledgered as `governance.agent.approval-answer`;
- an inline card band on the page, polled every two seconds.

**Behaviour that changed:** with no dashboard open, a dashboard escalation waits out its
timeout. **Chat runs are unchanged** (caveat 27).

**Found while finishing it, and fixed:** the card controller stopped polling when an
account could manage no agent, and so kept the cards it held. A User demoted to Viewer
kept a pressable card. The server refused the press, but the screen offered it. The
controller now clears on that change and on a 403 from its read, with a test for each
(T47 row 6c.12).

**Verified:** typechecks 0 (core, UI, test); type-aware lint over the gateway, infra,
agent-tools and governance trees 0; protocol checks, import cycles, SDK checks and doc
audit all 0 or passing; governance suite **2,998 passed, 21 skipped, 0 failed**; the
approval test set 33 files and 672 tests pass. Five files there fail only in **Windows
clean-up** of an open SQLite file, **bisected as not T68's**. Mutations: 23, 22 caught,
the one survivor a deliberately redundant check. **Not driven live**; the full lint gate
and `build-all` were run afterwards; see §"2026-09-13 (latest)".

**Kinan decided T69 (sweep register C12) and asked for it to be built.** A task
cancelled while its approval card was up had left the card pressable for up to two
minutes, and a press was announced to chat channels as an answer.

**What was built.** A new Gateway method, `plugin.approval.withdraw`, which only the
requester may call (the same device, client or connection binding `reportOutcome`
uses). The approval hook calls it the moment its run is aborted. The Gateway closes the
record as `cancelled` for `run-aborted` — a status and reason its store already had, so
**no schema change** — and publishes it through `publishAppliedApprovalResolution`, the
path every other ending takes. The card closes; a late press is refused as already
resolved; a reviewer who answered first keeps the answer. Resolved events now carry
`cancelled: true`, and the three chat renderers say _"cancelled: the run that asked for
it stopped"_ instead of _"denied"_, which also corrects how upstream's own exec run-abort
cancellation was announced. The Swift and Kotlin models were regenerated.

**Why the requester and not the Gateway.** Upstream already cancels exec approvals when a
chat run is aborted, matched by `runId`. Governance prompt runs abort through their own
controller, and plugin approvals carry no run id, so that path could never see them. The
waiter's abort is the one place Cancel, the kill switch, the prompt's time limit and a
chat abort all pass through.

**Found beside it:** T60's `plugin.approval.reportOutcome` was registered for release
train `2026.9` against the current `2026.8`, so `scripts/check-protocol-since.mjs` had
failed at HEAD since 2026-09-11 without anyone running it. Both methods now say `2026.8`,
and §4 names the script.

**Verified, each command's own exit code read:**

- **Typechecks:** core, UI and test, all 0.
- **Lint:** the type-aware wrapper over every changed file, with a positive control the
  rule flagged; plain `oxlint` over `src ui/src`, 0. The first run found the new tests
  had taken `plugin-approval.test.ts` 70 lines over the test file limit; it was split
  (requester tests and shared fixtures into their own files), not suppressed, and every
  test is still there: 33 before and after.
- **Protocol:** the Swift model check, the registry, the since-version guard and event
  coverage, all 0.
- **Import cycles** 0; the plugin-SDK API baseline and surface checks pass.
- **Suites:** the governance suite **2,930 passed, 21 skipped, 0 failed, 155 files**; the
  host-hook suites 263; the focused tests around every touched file. The hook's e2e file
  passes its approval tests; its six failures are pre-existing Windows path assertions,
  and it has to be run without the e2e config's `dist`-building setup (§4).
- **Mutations:** 11, all caught, each file restored and hash-checked.

**Not established:** the card closing on a real screen. The QA Gateway's accounts and
probes were session scratch and are gone, and a person cannot drive it by hand because
the card is a modal on every connected Control UI. `build-all` and the full lint gate
were not re-run. **Uncommitted** when written.

### 2026-09-13: the dashboard QA pass — eighteen findings, 346–363, seventeen fixed

**Kinan asked for a QA pass on T60 and T63 through the dashboard, then on the whole
dashboard, driven the way an operator would.** That meant every tier, including a
User assigned nothing, and the hard conditions: two operators at once, narrow
screens, the keyboard, long text and large lists, the Gateway stopped mid-session,
expired sessions, conflicting actions, approval queues, cancellation after a
reload, the emergency stop, and the ledger verified from outside. It ran against an
isolated Gateway with disposable accounts, driven by Playwright.

**Three findings to know about first:**

- **346, security.** The Control UI's service worker kept every governance answer
  in the browser, and served it to whichever account was signed in whenever the
  Gateway did not answer. Measured: a Viewer was served Root's accounts list;
  sign-out removed nothing; and during an outage the page presented pre-outage
  answers as a successful refresh. The worker now keeps hashed build assets only,
  and purges what older builds kept.
- **347.** An escalation raised from a dashboard prompt **had never reached a
  person**. Upstream's gate for unattended runs did not count the governance runner
  as attended, so it refused the escalation, and the refusal was recorded as a
  120-second timeout that took five seconds. **348**: every withdrawn escalation
  was recorded the same false way. Both fixed; T47 row 6c.1 checks it by hand.
- **349.** Escape on any Settings confirmation navigated to chat and unmounted the
  page.

**One decision, taken by Kinan on 2026-09-12: tasks survive the tab** (350).
Closing the tab had been cancelling the very task T63 recovers, so §3.5.82's claim
and T47 row 3.2.12 were both false. With that fixed, three smaller T63 defects
surfaced and were fixed (351–353), as did an open conversation that kept its message
box after its agent was unassigned (354). **355–361** are about what the page said
and whether it was true. Rule-clash warnings were never shown; Users were offered
refusal-only controls; the kill switch's result appeared off-screen; the approval
card truncated long actions; network errors were raw and stayed after recovery;
errors were styled as empty states; the ledger told a scoped reader nothing had been recorded; and a request's reason past 500 characters was cut without a mark (362).

**Checked and right:** the ledger verified by `scripts/verify-ledger.mjs` after a
hard stop, agreeing with the dashboard and the file; tier visibility exactly as
designed; expiry, revocation, concurrent role changes and double decisions; narrow
screens; the keyboard.

**Recorded, not fixed:** provisioning slows as agents accumulate (35 s to 77 s over
six agents); forms sit below long lists; on this machine the governance API
answers about 44 s after the Gateway reports it is listening; and four low items.
**Not established:** the approval end-to-end tests cannot launch Chromium here, and
the agent side was a local mock rather than a real model.

**Two decisions are open.** The first, from the independent review of this pass: **who may answer
an escalation raised from a dashboard prompt.** Since 347's fix, anyone whose
Control UI holds the Gateway credential sees and can answer the card, whatever
their governance tier — as for every chat run's approvals. Restricting it to the
accounts that manage the agent needs a governance identity on the approval path,
which is a design change.** The second, **363**: a task cancelled while its approval
card is up leaves the card pressable for up to two minutes, because nothing lets the
requester withdraw an approval; the fix is a new Gateway method. See the sweep
register's "Two decisions this pass leaves open".

**Verified, 2026-09-13, each command's own exit code read:**

- **The governance suite:** 2,928 passed. Its 2 failures were a source scan in
  `host-prompt-audit.test.ts` pinned to the literal `"governance"`, which 347's fix
  replaced with the shared constant. The scan now pins the constant and its value,
  and the file passes.
- **All three typechecks:** 0.
- **Host-hook contracts and upstream tests:** the host-hook contracts and the upstream
  tests beside every touched file, 263 each.
- **Lint, formatting, i18n, doc audit:** plain `oxlint` 0; `oxfmt` clean across 83
  files; the i18n gate 0 at 4,974 keys; the doc audit's count claims agree.
- **Browser, isolated UI and ledger:** the browser project 199 passed; ui-isolated 403
  passed; `verify-ledger` intact.
- **The wider `ui/src` suite:** 27 failures in 16 files, which is exactly finding 345's
  pre-existing set, none of them touched by this change.
- **The full lint gate.** It found one type-aware error, in this pass's own test
  (`unbound-method`), which is fixed. Its re-run then timed out on the core shard,
  exit 124, which is a timeout and not a verdict. So the fix was confirmed through the
  type-aware lint wrapper, alongside a positive control that the rule did flag. A plain
  `oxlint` call on one file does not run that rule, and passes trivially.
- **`build-all`:** passes every phase, the startup budget included (316,840 B), except
  `write-cli-startup-metadata`. That step times out on this machine even with nothing
  else running; the measurements are in the sweep register.
- **Mutation testing:** 68 mutations across six runs, all caught.

**How the rest of the session went, because three of its results changed what is
written above.** An independent review of the fixes raised five points. Three were
fixed: a recovered connection no longer erases the sign that a press may not have
landed, a repeated pending question is dated by its latest ending, and the service
worker purges only once it controls the tabs. One became decision T68; one is a
recorded limitation (a kill-switch stop never shows _Stopping_). **Every fix was then
re-driven on a rebuilt Gateway.** That is where 355's warning was found rendering
5,475 px off-screen (fixed), where 362 was found, and where 363 turned up while 348
was being proved. Two things went wrong and were caught: a background wrapper
reported success while `build-all` itself had exited 1, and the whole governance
suite caught a source scan that the narrower runs had missed.

**Committed and pushed 2026-09-13** to `personal/governance-layer` at `0c15e73a778`,
in three commits: T60/T63 as built (`778b769bef2`), the QA fixes (`746bfeb7bed`) and
the docs (`0c15e73a778`). **If you are picking this up:** answer T68 (T69 was answered and built later that day), rebuild
the VPS, run T47 including rows 6c.1–6c.9, and read caveats 27–29 before writing
anything about approvals.

The full record — every finding, its mechanism, its tests and its mutation check —
is `mg/REMAINING-WORK-DASHBOARD-SWEEP.md` §"The dashboard QA pass".

---

### 2026-09-11: T60 and T63 built, from a draft another agent left unfinished

**Both decided by Kinan on 2026-09-10, with another agent (Codex), which implemented
most of both and ran out of usage three times mid-work.** It left 38 modified files
and 2 new ones — uncommitted, never tested, never typechecked. **That draft was
continued rather than restarted**, after reading every line, running its tests, and
checking the parts that touch upstream OpenClaw.

**T60, "Always allow".** The escalation card now says, before the press, that the
button allows the action **once** and asks an Administrator to make it permanent.
When the request cannot be saved, the operator is told so after the press: on the
approval card's surface, through a new gateway method that lets the agent report
what its callback did, and on the dashboard's own "Would allow", whose outcome the
server returned and the panel used to discard. Approval-generated requests now get
**40 + 20 per account** in the organisation; duplicates are caught inside the file
lock; and a shrink never deletes a queued request.

**T63, the Cancel that disappeared on reload.** Both options, the conversation
first. A reopened conversation recovers this account's running task with Cancel,
and _Active agent sessions_ shows the same runs with the same Cancel. A run now
stays listed — _Stopping_, _Saving reply_ — until its reply is actually saved,
which is what lets "it vanished from the list" mean "it finished".

**What was wrong with the draft:** one core, three UI and two test-typecheck errors, 21 lint errors
(three files over the 700-line limit), 15 unformatted files, 3 failing tests and a fourth that failed only under load, stale
native-app protocol models, the dashboard discarding the outcome, a warning
dialog that would have rendered unstyled — and, the one that mattered most, **it
lost agent replies when one account sent several prompts at once**: keeping a run
until its reply was saved also kept its concurrency slot, so later prompts were
refused after their message was recorded. `qa-round12` caught it intermittently;
a saving run now keeps its row but not its slot. **All fixed.** Each over-limit file was
split along a seam this project already uses rather than squeezed.

**Checked and found sound**, because upstream code outlives this project: the schema
already accepted the draft's new field, the Telegram/Discord forwarder is idempotent
to the second resolved event, and the protocol drift is exactly the new method.

**Not established: no live approval card was pressed.** Every hop is tested; the
whole path needs a real agent reaching an escalation, which this machine cannot
produce because the Gemini key is exhausted. The VPS or T47 is where it gets proven.

**Verified:** all three typechecks 0; plain `oxlint` 0 and `oxfmt` clean across the
change set; the Swift, Kotlin and method-registry protocol checks clean; the i18n
gate 0 at 4,970 keys; and the governance suite plus the upstream tests beside every
touched file, **1,582 passed and 0 failed**. `build-all` exited 1 on its last step
only — a CLI help render timing out while a test suite and three typechecks ran
beside it — and that step **passed on its own in 93 seconds**. Everything else in the
build passed, including the startup budget.

The full record — every file group, every defect, every decision and its precedent
— is `mg/REMAINING-WORK-DASHBOARD-SWEEP.md` §"T60 and T63 built".

---

### 2026-09-09 (iii): T64 done — the page carries its own text

**Kinan's decision: split the page.** Built the same day. It is the item that had
been called "the next thing that blocks a text fix", and the block is gone.
...
**Nine kilobytes that every operator downloaded on the way to any page at all**,
for the text of one dashboard most sessions never open. Headroom went from less
than one operator-facing sentence to roughly a hundred and twenty.

**What changed, in four files.** `en-core.ts` is the English catalog minus the
governance page and is what `lib/translate.ts` imports — the startup cost.
`en-governance.ts` holds the page's 395 keys and is imported by
`governance-page.ts`, so it travels in that page's own lazy chunk. `en.ts` is
**still the whole catalog**, assembled from both, and is what the translation
pipeline imports. `translate.ts` gained `registerLocaleStrings`, and the page
calls it at **module scope** — while the lazy chunk evaluates, before the element
is defined — so no sentence is ever briefly missing and there is no `await` to
forget.

**The check that mattered.** The twenty translated locales are generated from
`en.ts`, so a key quietly leaving the catalog would take twenty translations with
it and surface as English text in a Turkish dashboard weeks later. The catalog
was compared key-for-key against the committed one: **4,956 keys before and
after, none missing, none added, no text changed.** The only difference is order
— `governance` now follows the core keys — and nothing compares against a
recorded order. The contributor i18n gate passes and reports the same 4,956.

**Finding 321's ceiling raise was handed back**, 318 → 317 KiB, because the
reason for it was exactly the text this split has moved.

**Two things deliberately not done, both recorded rather than skipped.** The
budget **baseline** was not ratcheted: the build printed the hint, and the note
at the top of that same file says baseline updates must use CI bytes via
`--startup-js-bytes`, because local zlib emits smaller streams than the Linux
builder. Setting it from a local number risks a red build on the VPS — which is
the machine T3 needs. **T3's rebuild produces the number**, so it drops then,
with a measurement instead of an invented margin (A9). And the generated locale
artefacts were not regenerated, because `ui/AGENTS.md` says they do not belong in
a source change; `catalog-fallbacks.json` therefore carries a `sourceHash` of the
old `en.ts` text, which the contributor gate does not check and the release gate
does.

**The rule it establishes**, for whoever adds the next page: a lazy page's
strings belong in a sibling `en-<page>.ts` that the page imports and registers.
The merge is top-level only — one page, one namespace — so two modules can never
co-own a key and let load order pick the winner.

---

### 2026-09-09 (ii): the small ones cleared, four decisions, and two claims that were never checked

**Three repairs, four decisions, three findings (342, 343, 344).** The three were the
items in `mg/REMAINING-WORK-DASHBOARD-SWEEP.md` §A that needed nobody's
permission, and they turned out to share one shape: **two surfaces of this
product answering the same question differently.**

- **A3.** `canManageAgent` folds an agent id through `canonicalAgentQuery`; the
  membership checks standing beside it used raw `.includes()`. So `SCOUT` was
  **manageable and unknown at the same moment**, and three call sites said so:
  the kill switch's "no agent with this id is running" warning, its
  `unregistered` guard — which meant finding 341's own repair missed the
  mistyped case — and the policy lookup, which labelled a **correct** answer as
  being about an agent that does not exist. Fixed at the owner
  (`agent-directory.ts` gained `includesAgentId`), pinned in the test file
  finding 215 created, and watched failing against the raw comparison.
- **A4.** The deployment report and the host's own security audit both answer
  "is the Gateway authenticated". With auth off, the report printed **two
  failures and two remediations for one absent credential** — seen for real on
  2026-09-08, when the QA instance had it switched off. `deployment.gateway_auth`
  is now the single row; the audit's two findings fold into it **verbatim and at
  the worse of the two severities**, so a fold can never soften a verdict. The
  rename tripwire those ids used to provide is restated against the real audit.
- **A5.** System resources dropped the load-average half of its row on Windows
  and showed "8 cores", while the Deployment report one section down says _"not
  determined here"_ for the same kind of situation. **It now says the same
  thing, in the same words, for zero startup bytes** — the string already
  exists.

**The decisions**: C3 settled as (a), leave the server's refusal text alone;
**C5 (T50) settled as (c)**, the lint gate stays manual and the hook now says so
out loud; **C2 turned out to have been decided the day before** and only the
backlog had not been told; **C1 (T64) was not taken** and was re-framed with
measurements so that it can be.

#### The one to read: finding 342

**A claim that had been repeated into three documents and was never true.**
Every statement of the startup-budget problem — including the comment inside
the script that **enforces** it — said each operator-facing sentence is
"multiplied across 22 locales and charged to startup JS".

`ui/src/i18n/lib/registry.ts` dynamic-imports all twenty non-English locales,
and `node scripts/build-all.mjs` prints them as their own chunks, none of them
among the twelve startup requests. **Only `en.ts` is static.** A sentence costs
one copy, not twenty-two.

The interesting part is the direction of the error. Overstating it made the
problem look unavoidable and the fix look cosmetic. The measurements say the
opposite:

```
startup JS gzip   325,565 B      ceiling 325,632 B      headroom 67 B
                  (325,572 B / 60 B before this pass — A5 added a sentence
                   and the bundle got *smaller*, because it reused one)
governance strings in en.ts      ~32 KB raw             ~10.5 KB gzipped
```

**The fix T64 asks for is worth about 170 times the headroom that is left.**
And its real obstacle was never written down: `scripts/control-ui-i18n.ts`
hashes the raw _text_ of `en.ts` into the locale baseline the i18n gate
compares, so splitting the file is a locale-refresh flow rather than a quiet
refactor. That is what makes it a decision instead of an afternoon.

**How it was found, which is the transferable part.** By running the command
this file already tells everyone to run first, and then **reading what it
printed rather than only its verdict.** The verdict was green. The asset list
underneath it had said the locales were lazy, every time, for as long as anyone
has been running it. Finding 321's lesson was _run the build_; this one's is
_read its output_.

#### Finding 344: the sixth verification command was red, and the table said 0

Running §4's list end to end — which is the whole reason it is a list — turned up
`tsgo -p test/tsconfig/tsconfig.core.test.json` **failing at HEAD**, before any
of this session's edits. `git stash -u` and re-run confirmed it: the commit that
added finding 340's test passed a `ledgerFilter` into `mount`, whose parameter
is `Partial<PageState>`, and that test file's local mirror of the page's state
does not declare it. An excess property, rejected by the one check that reads
the test tree.

**The row directly above it in §1 said "core, UI and tests — all 0".**

That is the **third** time a green cell in that table has described a red
command — finding **321** was the build, finding **323** was the lint gate, and
this is the test typecheck — and all three were cells in the same table. The
pattern is not carelessness about any one check; it is that a _measurement_ was
written into a document, and documents do not re-run. What each of the three
cells now says is the fix that generalises: **do not read the cell, run the
command.**

_(Fixed by declaring the field, with the reasoning left in the file.
`governance-panels.test.ts` carries the identical note about `policy`, omitted
in exactly this way for weeks in 2026-08. A local mirror of the page's state is
a copy, and copies go stale.)_

#### And finding 343, which is why §6's table changed while its total did not

Three rows were stale **against other rows in the same file**: 331 and 333 shown
open in a table forty lines above the rows recording them closed, and T55 shown
open in §6 while §1 of this same file announced it built. Two open tasks had
also been given a **second number each** — T63 is T65, T60 is T66 — in the
section whose stated purpose is that open findings be "counted rather than
remembered".

§6's table now closes T50 and T55 and adds T63 and T64. **Fourteen unstruck
before, fourteen unstruck after, and four rows different.** Findings 259 and 282
were a stale count beside current rows and the reverse; this is the third
variant, and the only check that catches it is opening the code — which is where
331's answer had been sitting for a day.
...
**Why.** No design requirement asks for a command line — §1.3 names the tiers,
the engine, the ledger, HITL and a Linux deployment, and nothing else. It was
also the surface that broke in front of an operator: `governance login` printed
no password label, did not mask the password, and refused valid credentials,
which blocked reading the ledger for T58 while the dashboard answered the same
question at once. And it was carrying T51.

**The measurement that settled it**, because the stated worry was time: the
by-hand plan T47 needs three people and three machines, and the command line
accounted for **2 of its 158 rows**. Its automated tests were already green, so
removing the surface _reduced_ the suite rather than saving effort.

**The arithmetic is the check that matters.** The suite went from 2,817 / 156 to
**2,716 / 142** — a drop of **exactly** the 101 tests and 14 files that were
archived. _(T62 then put 7 tests back in 1 file, so the figure to quote as
current is **2,723 / 143**; 2,716 is the number that proves the removal took
nothing with it.)_ A removal is the one change where a green suite proves least, because
deleting a surface deletes its tests and everything left passes by construction.
The subtraction is what turns "still green" into evidence.

**A second, adversarial pass found the thing that mattered most, and it is
finding 284.** After the first pass had reported the removal complete — sources
gone, both registries clean, the `--help` descriptor removed, three typechecks
and both lint gates green, the dedicated removal sweep 7/7 — `dist/` still held
**`register.governance-BfFo31MS.js`**, the entire compiled command tree. On any
machine running the shipped build, **including the VPS**, `openclaw governance …`
still worked.

**Deleting source does not delete the product.** Every check this project runs —
typecheck, lint, suite, and the removal sweep written for this very change —
reads the _source tree_. What a user runs is the _build_, and nothing compared
the two. Worse, this is finding **274**'s mechanism again: nothing in the build
clears `dist/`, so a plain `pnpm build` would have left the compiled command tree
orphaned there indefinitely. Only `rm -rf dist && pnpm build` removes it, which
is exactly why the rebuild command in §6 carries that `rm -rf`.

**So the VPS still has the command line until it is rebuilt**, and that rebuild
is already the first instruction in §6.

**What the sweep caught that the suite could not.**
`docs-notes/qa-sweep-2026-09-07/cli-removal-sweep.ts`, **7/7**, found a real
leftover: `core-command-descriptors.ts` still declared a `governance` descriptor,
which drives `--help`, argv routing and command suggestions. The files were gone
and the command was still **advertised** — finding 100's class, arriving through
a deletion. It also confirms all ten upstream command groups still register
(`onboard`, `daemon`, `models`, `config`, `agent`, `cron`, `doctor`, `dashboard`,
`audit`, `message`) and that **enforcement is untouched**: an unlisted action and
a core-tier denial are both still refused after the removal. The 63 upstream CLI
test files, 1,067 tests, all pass.

**What was genuinely lost, and it is not nothing.** The command line was
file-backed and ran in its own process, so it was the only way to read the
deployment report or verify the chain **when the Gateway was down** — the
documentation argued exactly that, and those passages are kept with a note
because the cost is now real rather than hypothetical. And two implementations of
one decision disagreeing is how finding **202** surfaced; that cross-check is
gone. Neither is a security regression: the gate is on neither surface.

**T51 closed by removing the question**, and one defect was recovered from it
rather than lost — a comment claiming `agent cancel` could stop a
dashboard-started run, which it could not. That is the comment an operator reads
during an incident.

**T62 raised and closed the same evening**, and it is the one thing the removal
gave back. Finding 268's repair had printed a terminal command beside the chain
verdict so the head hash could be recomputed by an independent reader — _"the
dashboard is no longer the only witness"_ — and removing the command line took
that away. Kinan chose to restore **that one capability without the surface**:
`scripts/verify-ledger.mjs`.

It is a better witness than the command it replaces. It **imports nothing from
`src/`**, so it runs with the build broken and the Gateway down, and against
files copied off the machine — which is the arrangement §7 caveat 4 says
actually closes the residual. It **cannot sign in**, where `audit verify`
required a governance session first: authenticating to a possibly-compromised
installation before auditing it makes the audit depend on the thing under audit.
And it **re-implements the hashing** rather than importing it, so a defect in
`audit-ledger.ts` cannot agree with itself.

**The cost of that independence is drift, and a verifier that cries wolf is
worse than none**, so seven tests pin it against the product on real chains —
intact, one decision edited on disk, entries deleted from the end, the checkpoint
removed. Neutering the hash check fails one; neutering the checkpoint checks
fails two. It answers **three** ways rather than two: 0 intact, 1 broken, **2
could not check**, because collapsing the third into either of the others is how
a verifier starts reporting infrastructure as tampering, or silence as safety.

**T61 added**, reported by Kinan the same evening: the dashboard answers a
mistyped password with _"Your session ended…"_. Diagnosed rather than guessed —
`run()` maps **any** 401 to `markSessionExpired()`, and the sign-in route answers
a bad password with 401 like every other route.

---

### 2026-09-07 (ii): the composition axis, and a property that was documented rather than built

**A QA session on the three days of work that preceded it, on an axis this
project had not used: the seams _between_ two new things.** Every feature landed
2026-09-05/07 has tests, and every one of those tests drives it **alone, in a
governance directory made one line earlier**. Two new standing probes
(`docs-notes/qa-sweep-2026-09-07/`) drive them against each other instead:
`composition-sweep.ts` (10 checks) and `policy-semantics-sweep.ts` (7 checks,
7 passed).

**279 is the one to read, and it is a security-relevant defect found by testing a
comment.** `proposeRuleFromEscalation` lists three properties it says it
guarantees. The third — _"it carries the access half for paths; a read that was
escalated proposes a read, not a read and a write"_ — **was false**.
`RuleRequest` had no `access` field, so the direction reached the reviewer only
inside the `reason` prose and never reached the rule, and an absent `access` on a
`PolicyRule` means **both directions**. Measured at the gate rather than reasoned
about: escalate a read, answer Allow always, approve it through the real route,
and a **write** to the same path comes back allowed — while the queue row said
"(read)" the whole time, because `describeRequest` prints the reason.

**Of the three properties that comment claims, the two with tests behind them
held and the one without did not.** The test file listing "six properties" did
not include this one. That is the transferable part: a claim in a comment is not
a property, and the ones with no test are exactly where to look.

**280 is finding 271 with the listing run properly.** 271 asked which fields
`refreshData` never overwrites and answered "the transcript". Listing this
component's **50** `@state()` fields against the **15** `endSession` cleared says
**twenty-three survived a sign-out**, and `refreshData` reloads only three of
those. The Agent permissions panel stayed open on the previous account's agent,
showing its rules and **the usernames assigned to it**; a stale `role="alert"`
announced an emergency stop the next person did not order; and the authoring and
request forms came back **filled in** with the previous account's half-typed rule
and the agent and account it named. Now **48 of 50**, with the two exceptions
named and argued rather than left to be noticed.

**The first repair covered three of the twenty-three, and was the same mistake
one layer down.** It was caught by re-running the listing _after_ fixing, not by
noticing — which is the rule worth keeping: **a fix derived from a listing is not
finished until the listing is run again against the fixed code.**

**281 is open as T60 and is a decision.** Every escalation proposal is filed under
one labelled origin, and the pending cap counts by requester, so all of them share
**a single 20-slot budget for the whole organisation**. Measured: 25 distinct
escalations answered Allow always filed **20** proposals; the other five granted
the call and filed nothing, recorded only as a ledger entry no surface reads. The
button still reads **"Always allow"**. The safe half is real and should survive
the fix: **a full queue never widens the policy, it only fails to propose.**

**282 is the documentation half, and it is the largest single sweep of stale
claims this project has recorded.** The 2026-09-06 restoration of `allow-always`
changed behaviour and **updated no document outside the code**, so six documents
still said an "allow always" answer writes a permanent rule — the behaviour QA
round 13 removed as a security defect. Among them **F3**, in both its prose and
its TikZ caption, and `CHAT-DEPLOYMENTS.md`, which is the document about the
surface the concern actually lives on.

**F3 carries the method lesson.** The 2026-09-05 figure audit read every figure
against the code and could not have caught it: the claim had been false since
long before that audit, because it described a capability that at the time **did
not exist in the code at all**. A check asking "does the code do what this figure
says?" cannot see a figure describing something the code no longer has — there is
nothing to compare against. **The complementary check is the reverse direction:
take each removed or changed feature and grep the documents for it**, which is
how three figures and nine prose claims were found, from a three-day commit list
rather than from a read.

**Also struck: "the field is populated or absent, never wrong"**, the intent
field's safety claim, still standing in three live documents after T2 produced
the third outcome nobody had allowed for (finding 273). And this file's own
**"The fifteen open"** table held fourteen rows, two of them closed, and omitted
three that were open — while the count above it was right. Finding 259 in
reverse: **deriving a number from the right source does not check the list you
print beside it, and a reader takes the list.**

**Two more sweeps and one backlog item closed.**
`policy-semantics-sweep.ts` drives the decision procedure itself — deny beats
allow whichever was written first, core beats operator, an expired rule stops
applying **at the gate** rather than at the next prune, `monitor` records where
`enforce` blocks, a per-agent posture overrides the installation's both ways,
and a locked-down agent is refused a call an explicit allow permits: **7/7**.
`systems-sweep.ts` drives five systems around the gate — sessions, the
rule-authoring guardrails, folder grants and the deployment report: **17/17**,
including that a demotion binds on a session already issued, that a grant on one
folder does not reach a **sibling whose name starts the same way**, and that the
deployment report's `overall` is the worst _non-unknown_ status so a machine
that cannot perform a check cannot turn a failure into a pass.

**Verification at the end, on a frozen tree.** **2,817 passed / 20 skipped across
156 files**, exit 0 (was 2,794 at the start; twenty-three tests added by 279, 280
and T56). Core, UI and test typechecks clean. The **full** lint gate green — and
that qualifier is the point, because plain `oxlint` was green while the full gate
was not.

_(One suite run was thrown away rather than reported. It was started and then
**edited underneath** while it ran, because T56's code landed mid-run; vitest
collects once but imports at test time, so the number would have described a
mixture of two trees. Killed and re-run on a frozen tree. A green number measured
against a tree that no longer exists is the same class of claim as a build that
succeeds without saying which code it built — finding 278, four days old and
already applicable to my own process.)_

**T50 got its first two numbers, and both came out of this session by accident.**
The full lint gate takes **about eighteen minutes** on this machine, measured
twice (05:24:15–05:42:02, then 05:55–06:14), almost all of it in twenty
per-extension `oxlint` passes —
so a pre-commit hook that runs it gets bypassed on its first bad day. And the two
gates **disagreed on a change made ten minutes earlier**: plain `oxlint` exited 0
while the full gate exited 1 on a type-aware rule, against a line plain `oxlint`
had itself demanded. "The hook is close enough" is now falsified by example.

**The gate's own tail read green while it had failed.** Twenty lines of
`[oxlint:extensions:N] finished` and no error; the failure sits at line 25 of an
82-line log and the **exit code is the only thing that says so**. §4's standing
rule — read the exit code, never a tail — caught it, which is the first time in
this record it has caught something rather than been cited afterwards.

**T56 is closed, and the decision it was waiting on did not exist.** The row said
"once Kinan says which surfaces" and named all three readers of
`listPendingDecisions`; only **two are listings**, the third being a lookup by id
that displays no worklist. The stack now carries a cumulative count of the
unanswered questions it has shed, incremented **inside the lock that sheds
them**, and both the command line and the dashboard say so in one shared
sentence. The dashboard says it **even when nothing is left waiting** — a flood
shed and then answered leaves nothing waiting _and_ an incomplete record, and
that is the reading an operator must not take.

**Two of my own probe fixtures were wrong first**, both in the direction that
manufactures a defect, and one test I wrote was **vacuous** — it asserted the
direction appears in `describeRequest`, which was already true via the reason
prose, and passed with the change reverted. Caught by running it against reverted
code rather than by reading it. Both are written up where they happened.

---

### 2026-09-07 (i): T2 done, and the four things that broke around it

**The longest session this project has had, and the one that closed its
highest-value item.** Read this entry and the opening block; the three below it
are the same night in more detail.

**T2 is done.** Kimi drove agent `jack` at `~/.npmrc` on the VPS, the gate
refused it, and entry #25 names the rule. **The attempt that failed first is
the one to learn from**: `~/.ssh/id_rsa` was refused by the _model_, before any
tool call, so nothing reached the gate. A request that sounds like a secret
tests the model; a boring-sounding credential file tests the layer.

**The ledger's silence became a diagnostic, twice.** `exec` broke on the VPS
with a missing module. Governance was ruled out in one glance — the gate records
every invocation by construction, so a refusal would be in the chain; nothing
there meant the failure happened before a tool call existed to gate. It was a
`dist/` holding pieces of two builds, and **nothing in the build ever clears it**
(finding 274). The same test is written into **T58** for the open `edit`
question, so nobody has to rediscover it.

**Five defects came from an operator using the dashboard and the sweeps around
them.** 269 (Send and Talk dead on typing; one field serving two meanings), 270
(the same shape on the emergency stop), **271** — signing out cleared three
fields where expiry cleared fifteen, so the next account in the same tab
inherited the previous one's account list, pending decisions and **agent
transcript**, and `ConversationController.forget` had never been called by
anything — and 272 (a refused agent's empty reply drew a blank line, so the one
case this layer exists to produce looked like a broken page).

**T57 and 273 are the same field from two sides**, and both are §1.6 logging
requirements. T57: a prompt was recorded on one surface only, so a task typed
into OpenClaw's own chat reached the agent with nothing naming who asked. 273:
the raw LLM intent belonged to the **previous turn**, visible in the very entry
that proves T2 — #25 carried the model's refusal of a _different file_ from
twenty-five minutes earlier, so it read as though the model declined when the
**gate** did. **The gate was right throughout; the trail describing it was wrong
in two independent ways.**

**Two documented decisions were reversed, and both had left tripwires that
worked.** Finding 83 (allow-always writing policy from an unattributable
surface) was re-opened at Kinan's direction, **its analysis re-checked and still
standing**, so the answer changed rather than the reasoning: `allow-always` now
files a **rule request** an Administrator or Root approves while signed in
(**276**). Finding 134's deleted `forgetAgentIntent` came back for 273, with a
caller. `HITL_ACTOR`'s comment ended _"do not reintroduce a writer without
...
anything, so a concurrent reader can take an empty key. Two entry points, and
the first fix covered one — **the test written for it caught that**. It fails
closed and never weakened the key, so it is availability, not tamper-evidence.

**Root may own an agent** now, with the assignment consequence handled rather
than left implicit; **an agent created through governance knows the name it was
given**; and **three live claims my own changes made false were corrected**, two
of them the VPS instructions being followed that night.

**On the testing, which is the reusable part.** Every fix was run against
reverted code before being trusted, and that step paid three times: a test
written for 271 **passed with the fix reverted** and was deleted; the T57 tests
would all have survived the hook being deleted, so **wiring guards** were added;
and one of the three ledger-key tests passes against the unfixed code and says
so in its own comment. **Every test failure produced in the session was a
fixture or assertion error and not one was a product defect.** Two reporting
mistakes of the same family: "lint exit 0" was reported twice from `$?` after a
pipeline — `tail`'s status, not the gate's, and the real result that time was a
**124 timeout** the same pipe had truncated out of the log.

### 2026-09-06 (iii): a prompt recorded wherever it comes from

**T57 built the same day it was added, at Kinan's decision: the labelled-origin
option.** A prompt that reaches an agent with no governance account behind it —
OpenClaw's own chat, the command line, a channel — is now recorded under
`HOST_PROMPT_ACTOR` (`host-prompt`), which joins `cli`, `bootstrap`,
`hitl-approval` and `unauthenticated` in `RESERVED_ACTOR_NAMES` so no real
account can produce an entry that reads as anonymous. The **channel** is named in
the entry, so an auditor learns the surface without it being dressed up as an
identity.

**It hooks at `agentCommandInternal`, the single funnel every agent turn passes
through** — the local command path and every ingress path alike — which makes it
the prompt-side equivalent of `runBeforeToolCallHook`. Governance-channel runs
are excluded (already recorded under the real account) and so are raw model runs
(machinery, not instruction).

**Both open behavioural questions were settled by precedent rather than
judgement, and one answer changed as a result.** That is the method worth
keeping: the answers were already in the codebase.

- **An agent with no group is recorded, not skipped.** The first draft returned
  silently. The gate does not: it writes to `INSTALLATION_LEDGER_GROUP` and says
  why — "requirement #5 asks for every action, and 'an unregistered agent tried
  to act' is exactly the one an operator needs". **The first draft would have
  made the case an operator most needs the one case nothing records.**
- **A failed ledger write stops the turn**, which is the gate's own answer: its
  `catch` returns `blocked: true`. **Retrying was raised and declined on the
  code's evidence**: `withFileLock` already retries lock acquisition and
  deliberately excludes the critical section's errors, because that "re-ran a
  non-idempotent append in a loop"; a failure after `appendFile` succeeds would
  append the entry twice, and a duplicate in a hash chain defeats requirement 6.

**Findings 271 and 272 came from the same sitting.** **271** is the one to read:
signing out cleared three fields where session _expiry_ cleared fifteen, so the
next account signing in **in the same tab** inherited the previous one's account
list, pending decisions and **agent conversation transcript** — the one piece
`refreshData` never reloads, so it stayed indefinitely. `ConversationController.forget`
was written for exactly this and **had never been called by anything**. **272**:
an agent turn with no text drew a blank line, and empty is the outcome when every
tool call was refused — so the one case this layer exists to produce looked like
a broken page.

**Five test failures that day were fixture errors and none was a product error**,
and one of them reported a working feature as doing nothing (asserting on the
input field names `action`/`target` where the ledger stores `toolName`/`resource`).
That is finding 257's mirror image. **A test written for 271 was deleted rather
than kept** because it passed with the fix reverted, and **the T57 tests were
found wanting before they were trusted** — all called the recorder directly and
would have survived the hook being deleted, so a wiring guard was added.

### 2026-09-06 (ii): the talk panel, and one field doing two jobs

**Findings 269 and 270, both fixed, and 269 was reported by Kinan pressing
buttons that did nothing.** The Send button would not click with a message
typed; the Talk button emptied the id box and closed the conversation instead of
opening it; and re-picking the agent from the dropdown opened it without Talk
being pressed at all.

**All three are one cause.** Two inputs in `agent-panels.ts` assigned their
keystroke to `props.promptDraft` / `props.conversationAgentId` instead of routing
it through `props.onDraft`. `agentPanelProps()` rebuilds that object every
render, so the value landed on a snapshot that was discarded and no re-render was
requested — which left each button holding the `?disabled` it had been
**rendered** with. Neither could ever come alive by typing.

**And one field was doing two jobs**: `conversationAgentId` was both "what the
operator typed" and "which conversation is open", which is why the box filled
itself in, and why the chooser's Talk button — labelled "Talk" in every state —
was calling a **toggle** and therefore closing the panel. The handler had a line
trying to defeat exactly that (`props.conversationAgentId = ""`, commented
"Force a fetch"), and that write went to the discarded snapshot like the others.
**A comment describing an intention the line could not carry out.**

Fixed by giving the draft its own field beside `killAgentId`, routing both
inputs through `onDraft`, and splitting `showConversation` (always opens) out of
`openConversation` (toggles, for the rows whose label changes).

**270 came from sweeping the class rather than from the report**: the same
`props.x = ...` shape on the **emergency stop**, where the clear-after-stop left
the agent id in the box after a stop had been engaged. Harmless to the stop and
misleading on the one control where "did that work?" is the question. The
whole-`ui/` sweep for the pattern now returns nothing.

**Why the suite missed it, which is the part to keep.** Thirteen tests covered
this panel and **not one typed anything** — every one handed the component a
pre-filled draft and asserted what rendered from it, so all thirteen tested the
template and none tested the input. Findings 206, 221 and 224 again. The three
tests added were **run against the unfixed code first: 3 failed, 27 passed**,
reproducing the operator's three symptoms exactly, then 30 passed against the
fix. A test never seen to fail is a claim, not evidence.

**T57 was added at Kinan's direction** and is the other thing to read: a prompt
was recorded in the ledger from **one** surface only. Measured, not assumed —
`ADMIN_ACTIONS.agentPrompt` had exactly one writer, the dashboard's prompt route.
**Built the same day; see the 2026-09-06 (iii) entry above.** Left in the present
tense until 2026-09-07, in a dated entry a reader could take as current.
A task typed into OpenClaw's own chat, or the CLI, or arriving on a channel,
reaches the agent with no entry naming who asked for it. **Tool calls are
unaffected and still governed**, so a refusal still refuses; what is missing is
the instruction and its author. The decision that comes first is what actor an
ungoverned prompt carries, because no governance account exists on those
surfaces.

**Also on 2026-09-06: the rehearsal ran on Linux for the first time, 20/20**, on
the VPS at `/opt/openclaw-governance`, and Kimi is connected and verified
(`Ready · 8159 ms`). **The model that actually connected is `kimi-for-coding`,
not the `kimi-k2` every document names** — record what ran, not what was planned.

### 2026-09-06 (i): an operator's second sitting, and T53 closed

**Findings 264–268, all fixed, and every one came from Kinan using the dashboard
rather than from anybody reading it.** That is now the fourth separate occasion
where the most useful findings of a day came from operating the system, and it
remains the cheapest QA this project has.

**The pattern across all five is one sentence, and the codebase already had a
name for it: the capability existed and the affordance did not.**

- **264.** The agents panel says _"or register an agent OpenClaw already has"_,
  the row for an unregistered agent carries a **Register** button, and a test has
  asserted that button since M4. **None of it was reachable**: the listing's
  fallback was `knownAgentIds(policy, liveSessions)`, so a host agent appeared
  only once somebody had written a rule about it or it happened to be running.
  The host roster is now part of the fallback, read at the route and passed in,
  because `src/governance/` must not import `src/config/`.
- **265.** `provisionAgent` takes a `model`, the route forwards it, the API
  client declares it, and **the form was the only link that did not ask**. The
  comment recording the identical fix for the owner picker sits four lines above
  the field that had the same gap.
- **266.** _"Optional, derived from the name"_ and _"Optional, OpenClaw chooses
  one"_ say what is optional without saying what the field **is**. Kinan asked
  what the third box was; it is the workspace.
- **267.** "Agent to talk to" told an Administrator to _type an id_ while the
  page held the full list and rendered it two sections above.
- **268.** The ledger, on three counts, and the third is the one to read.

**Read 268 before the viva, because it is about requirement 8.** Verifying the
chain reported _"Intact, entries verified"_ and nothing else — a verdict an
operator can only take on trust, and **a tamper-evidence feature whose output
must be trusted is missing the half that matters.** `verifyLedgerChain` now
returns the facts it already established on the way through: the chain head it
terminated at, the **independent checkpoint** that agrees with it, and whether
the entries are signed with this installation's key. The panel shows them, with
one sentence on how verification works and the terminal command that recomputes
the same thing — so the dashboard is no longer the only witness.

The other two ledger points: an entry read _"registered to account
user-1788466851277-8255cb2c"_ on the one surface whose job is saying who did
what, and now names the owner with the id kept alongside; and the red badge
column had **no label at all**, which is finding 103's repair one column over.

**T53 is closed** (2026-09-05), so the only backlog item that was mine alone is
done. `governance-page.ts` went from 735 code lines to 619 and the `max-lines`
suppression is **deleted rather than moved**. The exception's own reasoning said
every remaining candidate reads twenty or more private fields; that was true of
the prop builders and untrue of the conversation cluster, which it never
assessed.

**Two guards were added on the quietest paths.** **262**: the deployment report
now asserts the gate is armed, because `isUnconfiguredTestRun()` disables it when
a `VITEST` variable is present and `OPENCLAW_GOVERNANCE_DIR` is not — and the
latter is normally unset in production, so the whole guard rests on the former.
In that state the gate enforces nothing, refuses nothing and **writes no ledger
entry saying so**. **263**: the dashboard's hand-copied deployment types are
pinned type-level.

**Three sweeps, and the answers Kinan asked for did not change.** The M-series
was swept whole (16/16): several devices can hold several tiers in one
organisation, there is no one-account-per-device limit — the limit is one account
per browser cookie jar — and Codex still works per agent, with the backend switch
installation-wide by construction. Five days of fixes were **re-attacked rather
than re-read** (10/10), and the 2026-09-06 changes have their own sweep (7/7).

**T17's mechanical half is done.** All 21 figures carry all three forms; F18 is a
CUT cross-reference and correctly carries none. **F21's TikZ would not have
compiled** — five line breaks had lost a backslash — and **F12 claimed an
installation may hold more than one organisation**, which the 2026-08-30 cap made
false. The same correction had already been applied to F2, and the figure the
claim actually belongs to was missed.

**The week's honest total on fixture errors: six, two of which invented a defect
rather than missing one.** The worst reported the self-protecting core tier as
broken; the probe had asked about a tool called `read_file`, which does not exist
in OpenClaw, and `resource-extraction.ts` carries a comment saying an early
version of that registry guessed the same name. **Had the red been trusted, this
file would now record a critical regression that does not exist.**

### 2026-09-05 (ii): contention measured for the first time, and a cap that could be aimed

**Findings 260 and 261, both fixed.** Two more axes after the lifecycle one, and
between them they say something about this project's method that is worth more
than either finding.

**260 is the one to read, and it is finding 225 one store over.** The
pending-decision stack is per organisation while its rows are per agent, and it
shed the **oldest row globally** when it hit `MAX_PENDING_UNDECIDED`. So one
agent asking 210 distinct questions left 200 rows and **none belonging to the
other agent** — including a question an operator was meant to answer. The
resource string is whatever path or command the agent touched, so a flood of
distinct questions is the ordinary case rather than a contrived one, and the
`sameQuestion` collapsing that defends against a _wedged_ agent does nothing
against a varying one.

Repaired in 225's exact shape: **keep the bound, change which record is shed.**
`shedToUndecidedCap` now drops the busiest agent's own oldest row, so a flood
consumes its own quota before anybody else's. The victim-selection rule had never
been argued for — both cap comments argue for the caps _existing_, which was
never in dispute, and neither says why the globally-oldest row is the right one
to lose.

**How serious, stated exactly: the ledger keeps the escalation independently**,
so eviction costs the operator's worklist and not the audit record. **The drop is
still silent, and that half is open as T56**, because making it visible touches
all three readers of `listPendingDecisions` and a surface change across three
consumers is a different size of change from a victim-selection rule.

**The contention sweep found no defect and is worth keeping anyway.**
`file-lock.ts` opens by saying it exists because the CLI and the Gateway are
separate processes writing the same files — and **that claim had never been
measured**, because `file-lock.test.ts` drives contention with `Promise.all`
inside one process, which exercises the promise queue rather than the OS-level
exclusion. Four real child processes fighting over the ledger, the policy
document and the account store: **10/10.** The chain verifies, nothing is lost,
no sequence number is issued twice, no rule vanishes, one process wins a
contested username. With the lock reduced to a pass-through the same probe
reports a broken chain, ten duplicate sequence numbers and **35 of 60 authored
rules gone** — so the green is a measurement.

**261 came out of answering a question rather than looking for a defect.** Asked
what the password rules are, the answer needed two files, because the dashboard
keeps a **hand-copy** of `MIN_PASSWORD_LENGTH` and **nothing asserted the two
agreed**. Raise the server minimum and the form keeps advertising 8, producing
exactly the after-the-fact refusal the copy exists to prevent. Four lines of
contract test, in the same arrangement the avatar limit already uses.

**The method note, and it is the most useful thing here.** Across the day's
three sweeps, **five checks had to be repaired before they measured anything** —
two in the lifecycle sweep, one in the concurrency sweep, one in the bounds
sweep, and one test in the suite. Four were checks that could not fail. The
fifth was worse and is the one to remember: it asserted the ledger did not hold
an escalation, and would have been reported as a **product defect**, when what it
actually measured was that the probe had called the store directly instead of
driving the gate. **A probe that bypasses the production caller measures the
probe.** The pattern underneath all five: a check that asserts an _end state_,
rather than the mechanism that produced it, tends to pass — or fail — for the
wrong reason.

### 2026-09-05 (i): the lifecycle axis, and a name that outlived its owner
...
there is here — a protection Root cannot switch off that was matching nothing.

**Start with this, because it explains the rest.** The one test in this project
that can see the dashboard **had never run**. It sits inside the first of the
documented verification commands, which makes it look like coverage, but that
command runs it in jsdom where every width is zero, so it switches itself off
and the run prints `2 skipped` and passes. Running it for real needs Playwright's
Chromium **downloaded**, and that had never happened on this machine — so no
`.browser.test.ts` in the repository had ever executed here at all. That is
finding 250, it is the fifth instance of this project's oldest pattern (T25,
T19, T29, 136, 137), and it is why the previous day's fix for a nearly identical
complaint shipped half-finished: it was written _with a test_, and the test could
not run. Worse, the assertion it added measured the wrong edge — nothing off the
**screen**, while everything was falling off the **card**.

`npx playwright install chromium` is now step one of a **seventh verification
command** in §4. The browser project is 22 files / 198 tests and passes.

**Nine layout findings (241–248), and four of the seven complaints were one
defect**: a control cell nested inside a control cell inherits "do not shrink",
so the inner cluster sized to its widest line, ran past the card, and was
clipped — which is why the _last_ control in each cluster was the missing one.
**248 is the one to tell somebody about**: every destructive button on the page
had been unstyled since it was written, because the panels asked for the styling
using four spellings of a class name that does not exist, across twenty places.
Measured in Chromium, "Delete" came out byte-identical to an ordinary button. On
this page that means _Delete the organisation_ looked exactly like _Who does this
affect?_.

**253 and 254 are one idea.** A file inside the workspace has two legitimate
names — `work/secrets/key` and the full path — and which one the layer uses
depends on the _session's_ working directory, which the person writing a rule
cannot know. So a rule written the other way matched nothing, silently, while
the screen confirmed it had been written. **The repair is at the comparison, not
at the three places that write patterns**: rules are now matched against both
spellings and the canonical one is still recorded.

**Fixing it uncovered 254, which is the serious one.** The core denial guarding
"the governance directory in use" is generated as an absolute pattern, under a
comment stating the assumption it rests on: _"absolute whenever the target is
outside the workspace, which the governance directory always is."_ It is not
always. Relocate the store inside an agent's workspace — a supported setup with
its own field in the deployment report — and it matched nothing. Measured with
the fix removed and restored, the agent could read `policy.json`, `users.json`,
`audit-ledger.jsonl` and **`ledger.key`**. With that key the whole audit chain
can be forged, so this is requirement 8 as well as requirement 1. **The default
layout was never exposed**, because the static `.openclaw/governance` pattern
covers it.

**255 came from comparing the three surfaces mechanically.** Nine candidate
gaps, four false positives, four already answered in writing by
`CLI-REFERENCE.md` §2d — and one real: the command line could not **list**
accounts, and §2d's own "what you can still do from here" offered
`set-policy-authoring`, which takes an account id that **nothing on that surface
could print**. A register claiming a capability while it was unreachable, which
is finding 223 one layer in. `governance accounts` now exists.

**Three tests were caught proving nothing, all on the same day and all by the
same method.** A test asserting the 253 fix passed with the fix removed, because
the fixture refused everything by default. The probe demonstrating 254 did the
same thing an hour later and nearly went into the write-up as evidence. A test
asserting destructive buttons look different passed after one file was reverted,
because it only ever looked at one button. **Break it on purpose and check that
something goes red** is the only thing that found any of them, and it is now
applied in both directions: removing the fix turns five assertions red, and
adding the unsafe `..` spelling turns the safety assertion red.

**Measurements, all 2026-09-04, all re-derived rather than repeated:**

|                   |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ----------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Governance suite  | **2,801 across 146 file runs** (2,780 passed + 21 skipped), re-measured 2026-09-08 (vi) after T55 — +9 tests in one new file, `agent-policy-lifecycle.test.ts`, which is exactly what was added. It was 2,771/145 after findings 305–323, and 2,765 across 144 (2,745 + 20) on 2026-09-08 (ii) after findings 296–304 — +17 tests in one new file, which is exactly what was added. It was 2,745 (2,725 + 20) after findings 285–295 — the delta is +3 tests added less the 1 deleted with 290's dead helper, which is the subtraction rather than the green. It was 2,743 (2,723 + 20) after findings 279 and 280 and T56, exit 0 read directly rather than from a pipe. It was 2,814/156 before those, re-measured 2026-09-05 after T53 and findings 262 and 263; It was 2,766/151 earlier that day after 256, and 2,757/150 on 2026-09-04; each delta is exactly the tests added. The 20 skipped are by design: 15 need a layout engine, 5 are platform skips                                                                                                                                                                                                                                                           |
...
| Lifecycle sweep   | 8/8 (`docs-notes/qa-sweep-2026-09-05/lifecycle-sweep.ts`), and 5/8 before finding 256's repair                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| Agent lifecycle   | **2/5** (`agent-lifecycle-sweep.ts`), and the three are T55 rather than a regression                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| Contention sweep  | 10/10 across **four real child processes** (`concurrency-sweep.ts`), and 5/10 with the file lock neutered                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| Bounds sweep      | 11/11 (`bounds-sweep.ts`), and 10/11 before finding 260's repair                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| M-series sweep    | **16/16** (`m-series-sweep.ts`). Several devices at several tiers in one organisation; no per-device account limit; Codex per agent and per installation                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| Five-day sweep    | **10/10** (`five-day-regression-sweep.ts`), re-attacking 202, 225, 254, 256 and requirement 8 rather than re-reading them                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| Dashboard sweep   | **7/7** (`docs-notes/qa-sweep-2026-09-06/dashboard-changes-sweep.ts`), the server half of the 2026-09-06 operator findings                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| Composition sweep | **8/10** (`docs-notes/qa-sweep-2026-09-07/composition-sweep.ts`), the seams **between** the features of 2026-09-05/07 rather than each alone. The two that fail are the two open decisions: T55's reused agent id inheriting an escalation-approved grant, and T60's shared proposal budget. It found 279                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| Policy semantics  | **7/7** (`policy-semantics-sweep.ts`), the decision procedure itself against a whole document: deny beats allow either way round, core beats operator, an expired rule stops applying **at the gate** rather than at the next prune, `monitor` records where `enforce` blocks, a per-agent posture overrides the installation's in both directions, and a locked-down agent is refused a call an explicit allow permits                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| Systems sweep     | **17/17** (`systems-sweep.ts`), five systems around the gate: sessions (a demotion binds on a session already issued), the authoring guardrails (`core` refused, `baseline` coerced, a shadowed grant reported **and** confirmed inert at the gate), folder grants (an exception outside the folder refused before any write; a grant not reaching a **sibling whose name starts the same way**), and the deployment report (its summary agrees with its rows; `overall` is the worst _non-unknown_ status)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| T58/T59 probe     | **5/5** (`t58-t59-probe.ts`). `edit` **is** governed (path/write), and the gate resolves a path identically whether the file exists yet or not — so it cannot hand `write` and a later `edit` two different files, which rules out the mechanism behind T58's ENOENT. Also settles the provider-agnostic claim structurally: the gate's entire input is `{toolName, params, derivedPaths}` and `{agentId, sessionKey, nativeHarness, cwd}`, with **no model or provider field anywhere**, so it cannot behave differently by model                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| CLI removal       | **9/9** (`cli-removal-sweep.ts`), the ninth added 2026-09-07 (iv) for finding **285**: it greps `ui/src/pages/governance` and `ui/src/i18n/locales` for a live `openclaw governance …` **string**, which is the one thing no typecheck, lint rule or import graph can see, and it is what the dashboard's chain-verification row was still printing. Verified to fail by putting the old command back. The governance command line is gone from both registries, all ten upstream groups still register, enforcement is unchanged, and `cli` is still refused as an account identity. **It found finding 283 on its first run** — `core-command-descriptors.ts` still advertised `governance` in `--help` after every file backing it had been deleted — and gained an eighth check for **284**, that the **built artefact** no longer holds the compiled command tree. That check **skips rather than passes** when there is no `dist/`: an absent build is not a clean one                                                                                                                                                                                                                                               |
| Backlog           | **62 items, 47 struck, T1 not being done, 15 open (14 actually outstanding).** Re-derived from the rows 2026-09-07 (iii), after T51 and T62 closed and T61 was added. **A grep will say 46/14: T13 carries a struck row reading DRAFTED, for the drafting sub-task, while T13 itself is open**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| Defects           | **341 found, 338 fixed**, and **five open**: 169 as an unexplained observation, 281 as a decision (T60), 316 as a decision (T63), **331 as a decision and 333 as work**, both from 2026-09-08 (vii)'s dashboard pass (**326–336**, nine closed) and both carried in `mg/REMAINING-WORK-DASHBOARD-SWEEP.md`. Re-derived 2026-09-08 (vii); previously 325/322/3 at (vi); **258 closed when Mohammad decided T55**. Previously 304/301/3, re-derived 2026-09-07 (iv). **285–295 are that session's**, and not one of the eleven is an access defect: five are a surface or a register out of step with the removal — four naming something that no longer exists and **288 the mirror, declaring a live core rule removed** — two are `verify-ledger.mjs` misreporting its own confidence, one is dead code with a false rationale, two are stale counts, and 295 is the archive's rebuild instruction still chaining `rm -rf dist` behind a `pnpm` that is not on every PATH — the command that left this machine with no build at all on 2026-09-07, its lesson recorded in the session log and not in the file carrying it — 294 being this row and `PROJECT-SUMMARY.md`'s equivalent disagreeing by seven on the same day |
...
needs the three of you on three machines.
...
**Verified and pushed.** Governance suite **2,732 across 149 file runs** _(that
figure is T52's and is kept because this paragraph is about T52; the current
measurement is in §1)_, host
...
verification commands is green** — five when this was written, seven now. Those
commands **do not cover the repository**, and reading them as if they did is exactly how a false claim
...
**T1–T54**, the original project plus everything added since, and supersedes
every older list. §"The M-series" holds **M1–M6**, a multi-tenancy feature
requested 2026-08-24 and **complete since 2026-08-27**.

**Re-derived 2026-09-07 (iii) by counting the table rather than incrementing: 62
items, 47 struck (done or cancelled), 15 open, of which T1 is deprioritised and
not being done, so 14 are actually outstanding.** _(This read "57 items, 44
struck" from 2026-09-06 until 2026-09-07 (iii); it was correct when written and
five task-state changes went past it.)_ (T57 was added and built the
same day, 2026-09-06: a prompt is now recorded whatever surface it arrives on.) _(This block read "54 items,
42 struck … 11 open" until 2026-09-05, and it was wrong in **both** directions
before T55 and T56 were added: **T13 has two rows**, struck where its drafting
was recorded and open where the reading still is, and this summary had taken the
struck one while §6's table had taken the other. That is finding **259**. Before
that it read "43 tasks… 36 done, 5 open", and before that "38 done", which
summed to 45 against a list of 43. **Count the rows; do not adjust the number**,
and when two summaries of one table disagree, suspect a duplicated row before
suspecting either summary. `grep -cE '^\| ~~\*\*T[0-9]+\*\*~~' `on
the backlog table gives the struck ones, and the unstruck ones are the rest.
Other tables further down this file also have `T…` rows and are history, not the
backlog, which is what makes a naive grep over the whole file disagree.)_

| Open       | Who              | What                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| ---------- | ---------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ~~**T2**~~ | ~~You~~          | ~~One real agent driving one real tool call.~~ **DONE 2026-09-06**: Kimi at `~/.npmrc`, refused, ledger #25                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| **T17**    | You              | Redraw the figures, or decide Claude drafts them **The audit half is done (2026-09-06)**: all 21 figures compared against the code and checked for completeness. All three forms present for every one — F18 carries none and correctly so, being a CUT cross-reference. **Two defects fixed**: F21's TikZ would not have compiled (five line breaks had lost a backslash, so LaTeX read `\a`, `\backend` and three more as undefined control sequences), and F12 claimed an installation may hold more than one organisation, which the 2026-08-30 cap made false — the same correction F2 had already had, applied to the figure the claim actually belongs to. **What remains is yours**: whether to redraw them, or to have Claude draft the 21 for approval. The remaining risk is that nobody has _compiled_ them; that needs a LaTeX toolchain this machine does not have. |
| **T18**    | You              | The report                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| **T46**    | You, then Claude | The setup wizard still says "OpenClaw" and never mentions what this project is                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| **T47**    | All three        | **The by-hand test plan is written** (`docs-notes/T47-TEST-PLAN.md`, 138 checks as first written, plus **four added 2026-09-05** from findings 256 and 260,). Running it needs three people on three machines                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| **T48**    | You              | Decide whether Chapter 3 is ready to be written                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| **T49**    | You              | Decide what the multi-tenancy machinery is for, now one installation holds one organisation **Answer the Codex switch's scope in the same breath** (2026-09-05): `setCodexBackendEnabled` takes a `groupId` but writes an **installation-wide** config key, and the group id only routes the ledger entry — `readCodexBackendState()` takes no group id at all. Harmless while the cap stands, and if it is lifted, org A's Root switches Codex off for org B while org B's ledger says nothing. Measured in `docs-notes/qa-sweep-2026-09-05/codex-scope-probe.ts`; written up in this file's §"An observation about the Codex switch's scope".                                                                                                                                                                                                                                   |
| **T50**    | You, then Claude | Whether anything should enforce the full lint gate (finding 237)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| **T51**    | You, then Claude | Whether the command line should reach the Gateway's in-flight runs (finding 238)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| **T53**    | Claude           | Split `governance-page.ts`, a recorded 700-line exception rather than a silent one                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
ls ~/.openclaw/governance       # is this the fork, and has it initialised?
```

The governance directory holding `policy.json`, `users.json`,
`audit-ledger.jsonl` and a `ledger.key` is the proof the fork is what is
deployed; stock OpenClaw never creates it. _(This was
`openclaw governance policy show`, reading "Not signed in." as the pass, until
the command line was removed on 2026-09-07.)_ Then open the tunnel and confirm the
...
1. **Create an Administrator account** in the Accounts panel — optional since
   2026-09-06, when Root became an eligible owner. Do it anyway if you want the
   agent to answer to Mohammad rather than to you; skip it if you are setting
   up alone.
2. **Create an agent**, choosing its owner from the picker. Root now appears
   there as "(you, Root)" alongside any Administrators.
...
| 2   | The **Deployment report** panel, through the tunnel                                                   | The full report. Run it over plain SSH **before** opening the tunnel. That is the case it was written for                                       |
...
| 6   | Prompt the agent to do something the policy **refuses**                                               | The agent's reply **and** the **Audit ledger** panel showing the refusal                                                                        |
...
> nothing is governed. **The tell is `~/.openclaw/governance`**: this fork
> creates it on first run, and a wrong-branch install never does. _(It used to be
> that `openclaw governance policy show` existed at all; that command was removed
> on 2026-09-07, so the directory is now the check.)_
...
`pnpm ui:build` is not one of §4's verification commands.** A green
...
screen. _(At the time, `openclaw governance policy show` answering "Not signed
in" rather than "unknown command" was the proof the fork was deployed. That
command was removed on 2026-09-07; the governance directory is the check now.)_ Still to
...
create one), then **T2, the live run**. ~~T2 is still the highest-value item on
the project~~ — **done 2026-09-06**, and the sentence is kept because it was true
for eighteen days and explains why the rest was sequenced behind it.
...
two capabilities dashboard-only for reasons that are not true. The rule then
read: _every capability reaches all three surfaces unless a stated reason says
otherwise_, and the reasons lived in `CLI-REFERENCE.md` §2d.

**The rule is gone as of 2026-09-07 (iii), because the third surface is.** The
governance command line was removed at Kinan's decision — no design requirement
asked for it, it was the surface that broke in front of an operator, and it was
carrying T51. There are now **two surfaces: the HTTP control plane and the
...
nobody had measured it, it was false of four capability groups, and _writing the
reasons out before checking them_ got two of four predictions wrong. That is
about unmeasured universals, not about command lines.
...
~~**T2 remains the single highest-value item on the project**~~ — **done
2026-09-06**, and it was true from 2026-08-19 until that night.
Everything above is worth less than one real agent driving one real
tool call.
...
> **Picking this up on 2026-09-20, in this order:** `mg/NEXT-AGENT.md`, which is the
> whole handover in two pages; then this file's top block and §1's state table with its
> **2026-09-20**, **2026-09-19** and **2026-09-18 and 19** entries; then §6 and §8 (what
> is left, and what to do first); then §7's caveats **5, 27–29**. Then
> `mg/REMAINING-WORK-DASHBOARD-SWEEP.md` from its top box through §"C" (who owns what).
> Then `docs-notes/WRITING-GUIDE.md` if you are writing the report, which is the next
> thing to do, or §4 of this file before you run anything. **New to the project
> entirely:** `docs-notes/FIRST-RUN.md` first, then the table below.
...
| `mg/NEXT-AGENT.md`                     | **The handover prompt (2026-09-20).** What the project is, where it has reached, what is left and what to do next, in two pages                                                                                                                       |
| `mg/SESSION-LOG-2026-09.md`            | The September narrative, through 2026-09-20                                                                                                                                                                                                           |
...
| `GOVERNANCE.md`                        | Operator overview + the engineering defect table, findings 1–134 in full. **135–379 are indexed there rather than written out in full**. They live in `REMAINING-WORK.md`, and the index at the end of `GOVERNANCE.md` says which section holds which |
...
Operator-facing docs (`WRITING-PERMISSIONS.md`, `PERMISSION-SPEC.md`,
`ROLE-MODEL.md`, `BASELINE-RULES.md`, `CHAT-DEPLOYMENTS.md`, `FIGURES.md`,
`LINUX-INSTALL.md`, `FIRST-RUN.md`) are current as of **2026-09-07 (iii)** —
and `ROLE-MODEL.md`, `CHAT-DEPLOYMENTS.md` and `T47-TEST-PLAN.md` as of **2026-09-11**, for T60 and T63 — and `T47-TEST-PLAN.md` (rows 6c.1–6c.9), `ROLE-MODEL.md` and `FIRST-RUN.md` as of **2026-09-13**, for the QA pass — when
...
## 2b. Writing the report: where the files are, and the conventions that must hold

**Added 2026-09-20, when Chapter 3 was started. These are standing conventions,
not a record of a day's work: follow them, and add to them rather than inventing
a second scheme.**
...
| File                                    | What it is                                                                                                                                                                                                  |
| --------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `docs-notes/report/chapter3.tex`        | **Chapter 3, in LaTeX.** The file we write into. Kinan pastes it into his own `main.tex`. Outline only as of 2026-09-20                                                                                     |
| `docs-notes/report/main-reference.tex`  | **A read-only copy of Kinan's live `main.tex`**, as he supplied it on 2026-09-20, newer than `Grad_Proj___Current.pdf`. Chapters 1 and 2 verbatim; Chapter 2's body summarised, with what Chapter 3 owes it |
| `docs-notes/FIGURES.md`                 | **The figures, and the source of truth for them.** Each one in prose, Mermaid and TikZ, with the reasoning and a keep/cut recommendation                                                                    |
| `docs-notes/figures/build-figures.mjs`  | Extracts the kept figures' TikZ from `FIGURES.md` into one compilable document                                                                                                                              |
| `docs-notes/figures/report-figures.tex` | Generated by that script. **Do not hand-edit it**; edit `FIGURES.md` and re-run                                                                                                                             |
| `docs-notes/figures/report-figures.pdf` | The compiled preview, one figure per page. Git-ignored, so rebuild it rather than expecting it to be there                                                                                                  |
| `docs-notes/WRITING-GUIDE.md`           | Which material feeds which report section, every number with the command that re-derives it, and the claims the evidence will not support                                                                   |
| `docs-notes/CHAPTER3-MATERIAL.md`       | The raw material, organised by report section. Not prose                                                                                                                                                    |

**The live document is Kinan's and lives outside this repository**, under
`Documentation/`, which is git-ignored. Nothing written here reaches the report
until he pastes it in. When he sends a newer `main.tex`, replace
`main-reference.tex` wholesale and say that you did.

### THE NEXT TASK: the four subsections of Section 3.2

**This is what Kinan asked for next, on 2026-09-21, and it is the first thing to
do after reading this file.** Do not start anywhere else in the chapter.

Write these four, in this order, into `docs-notes/report/chapter3.tex`, replacing
each one's `\sectionstub`:

| Subsection                      | Requirement | What it has to say                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Material                                                                                                       |
| ------------------------------- | ----------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- |
| **3.2.1 Enforcement Point**     | 3           | Why a default-deny gate has to sit on the host's own tool-call path rather than beside it. The concrete detail worth the space: OpenClaw funnels every tool call through one function, `runBeforeToolCallHook` (`src/agents/agent-tools.before-tool-call.policy.ts`), and the gate had to go **before** an existing early return that skips policy work when no plugins are registered. Placing it after would have silently disabled governance on a default installation. That is a control which is correct in isolation and useless in the wrong position, which is the argument of the section                                                                  | `CHAPTER3-MATERIAL.md` "→ 3.2", requirement 3; §3.5.1, §3.5.25                                                 |
| **3.2.2 Audit Coverage**        | 5           | What recording one hundred percent of agent actions was first read as meaning, why that reading was **abandoned**, and what replaced it. The first implementation logged only _governed_ actions; tools the extractor did not recognize passed the gate silently. That was wrong on the point that mattered, because the unlogged calls are exactly the ones that reveal what the policy fails to cover. Every invocation is now recorded, and a call the layer could not evaluate carries the distinct decision `ungoverned` rather than being folded in with `allow`. **Do not write this as a narrowing that was disclosed**: it was a narrowing that was removed | `CHAPTER3-MATERIAL.md` "→ 3.2", requirement 5; §4.x.10 for the two costs it exposed                            |
| **3.2.3 Termination Latency**   | 7           | What is being timed. The lockdown is applied first and blocks every subsequent governed action immediately; the abort signal is dispatched in milliseconds; the **confirmed** stop waits for the host's own registry of in-flight runs to report those runs gone. Two numbers are kept distinct all the way to the dashboard, `dispatchMs` and `elapsedMs`, because collapsing them was the original defect. Carry the honest caveat: from the CLI there is no in-flight termination at all, because the run registry lives in the Gateway process, and the CLI says so rather than implying a stop                                                                  | `CHAPTER3-MATERIAL.md` "→ 3.2" requirement 7, §3.5.10, §4.x.8; **and `docs-notes/report/DIVERGENCES.md` §1.4** |
| **3.2.4 Credential Protection** | 8           | Met by reuse rather than reimplementation. OpenClaw already had a mature redactor (`src/logging/redact.ts`, about 1,100 lines, around 40 vendor token patterns, structured-field and URL/PEM handling), and the ledger calls `redactToolPayloadText` on every resource string before writing. The stronger argument is **placement**: a filter applied by each caller can be forgotten by one caller, and a redactor at the single boundary where entries are written cannot be                                                                                                                                                                                      | `CHAPTER3-MATERIAL.md` "→ 3.2", requirement 8; **and `DIVERGENCES.md` §1.2**                                   |

**Two of the four owe a divergence explanation**, and both are already written up
with their justification in `docs-notes/report/DIVERGENCES.md`: 3.2.3 owes §1.4,
the confirmed stop being slower than Chapter 1's promised sub-second, and 3.2.4
owes §1.2, redaction reusing the host's rather than the regex and entropy
analysis Chapter 2 describes. State each difference openly and give the reason.

**After those four**, the next thing is **3.2.5 Remaining Requirements**, which
covers requirements 1, 2, 4, 6 and 9 together and is where requirement 9's Linux
evidence lands. Then Section 3.3.

### House style for the report's prose

Set 2026-09-20, after Kinan compared a first outline against the two model
reports and asked for headings like theirs.

1. **Headings are noun phrases, two to four words, title case.** Not sentences,
   not questions. Both model reports read this way: "Packet Handling", "Voting
   Process", "System Architecture", "Smart Contract", "Donor Setup", "System
   Security". The argument belongs in the prose, not the heading. An earlier
   outline used headings like "What recording every action can honestly mean";
   that is the thing to avoid.
2. **American spelling, because Chapters 1 and 2 already use it**: `prioritize`,
   `utilizes`, `Data Sanitization`, `organizations`, `behavior`, `Modeling`. So
   in the report write canonicalization, sanitization, organization,
   authorization, behavior. **This repository's own notes are written in British
   English. Do not carry that into the report.**
3. **The section spine follows both model reports**, and is the spine
   `CHAPTER3-MATERIAL.md` is already keyed to: design requirements, analysis of
   requirements, analysis of constraints, different design approaches, and
   **Developed Design last**, before a short summary. Engineering standards is
   inserted before the approaches because the PSUT template marks it required
   and neither model report carries it in Chapter 3.
4. **Show Kinan both forms, every time.** Put the LaTeX that went into the file
   in the reply, and the same content again as ordinary readable prose. He reads
   the second to judge the writing and takes the first to paste. Never make him
   compile something in order to read it.
5. **Skeleton before prose.** Agree the headings, then fill them.
6. **No em dashes.** Kinan asked for this twice, on 2026-09-20, first for the
   wizard wording and then for the report. Use a colon, a semicolon, a comma or
   a full stop instead. It applies to everything written for him to read or to
   paste, not only to the report.
7. **Write every draft straight into the file as it is agreed**, not only into
   the chat. Prose goes into `docs-notes/report/chapter3.tex` in the same turn
   it is shown, replacing that section's `\sectionstub`. Kinan's instruction,
   2026-09-20: "add the text to the relevant tex file ... to make sure you keep
   up with me". The repository copy and what he has pasted must never diverge,
   because he is compiling in Overleaf from what he pasted and cannot see the
   file.
8. **One section at a time, in his order.** He picks the section; draft it,
   show it in both forms, write it into the file, then wait. Do not run ahead
   into the next section unasked.
9. **Cross-reference sections with `\ref`, never by typed number.** Our own
   sections carry `sec:gov-` labels for this. Chapter 1's sections have no
   labels yet, so references to Section 1.3, 1.4 and 1.5 are written as plain
   text; if Kinan adds labels there, switch them over. Figure references stay
   undefined and print `??` until the figures are pasted in, which is expected
   and is not an error.

### How Chapter 3 is written and shown

- **Write LaTeX, not Markdown.** Chapter 3 is drafted directly in
  `chapter3.tex` so it can be pasted into `main.tex` unchanged.
- **Show Kinan both forms.** Every time something is written, put it in the
  reply twice: once as the LaTeX that went into the file, and once as ordinary
  readable prose or Markdown. He reads the second to judge the writing and takes
  the first to paste. Do not make him compile something to read it.
- **Skeleton before prose.** Agree the headings first, then fill them.

### Differences from the preliminary design

**Kinan's instruction, 2026-09-20:** it is fine and expected for the final
implementation to differ from the preliminary design in Chapters 1 and 2.
**What is required is that Chapter 3 states each difference openly and explains
why the change was made.** A difference that is explained reads as engineering
judgement; the same difference left unmentioned reads as an oversight, and an
examiner who finds one will look for more.

The register is **`docs-notes/report/DIVERGENCES.md`**, and it carries the
justification for each, not just the fact. Five are verified so far: a keyed
HMAC-SHA256 chain rather than Chapter 2's Merkle tree; redaction reusing the
host's own at the ledger boundary rather than fresh regex and entropy analysis;
Chapter 1 describing only the conflict detector's earlier-rule-wins and not
evaluation's deny-precedence; the confirmed stop measuring 2.2-2.8 s against
Chapter 1's promised sub-second; and escalation being set per agent by an
Administrator rather than per user by Root.

**A full sweep of Chapter 1 §1.6 against the built system has not been done**,
and §2 of that file lists four more candidates noticed but unverified. Do the
sweep before Chapter 3 is called finished. Sections of `chapter3.tex` that owe
an explanation carry a `% DIVERGENCE` comment pointing at the register.

### How figures are referred to

- **The drawings never live in `chapter3.tex`.** They stay in `FIGURES.md`.
  Chapter 3 cites them and says, in a comment, where to find them.
- **Every figure label is prefixed `fig:gov-`,** and every table label
  `tab:gov-`. This is not decoration: Chapter 1 already uses
  `\label{fig:architecture}`, `tab:policy-eval`, `tab:policy-rules` and
  `tab:load_distribution`, and F1's label collided with the first of those until
  it was renamed on 2026-09-20. Keep the prefix for anything new, and check
  `main-reference.tex` before inventing a label.
- **Where a figure belongs, `chapter3.tex` carries one comment line:**

  ```latex
  % FIGURE F1 -> \ref{fig:gov-architecture} (prints as Figure 3.1)
  ```

  which says: figure F1 in `FIGURES.md`, cited in prose as
  `\ref{fig:gov-architecture}`, printing as Figure 3.1. The mapping table for
  all fourteen is in the header of `chapter3.tex`.

- **To see one**, open `docs-notes/figures/report-figures.pdf` at the page the
  table gives. **To get its TikZ**, copy it from `FIGURES.md` under
  `## F1: …` → `### TikZ form`, or lift the whole `figure` environment out of
  `report-figures.tex`.

### What `main.tex` still needs before the figures compile

Already there and correct: `tikz`, the `\usetikzlibrary` line, `pgfplots` with
`compat=1.18`, and `graphicx` (loaded by `style-PSUT-BSc-Eng`, so `\resizebox`
works). **Missing, and every figure depends on it: the shared `\tikzset` style
block** from `FIGURES.md` → "One shared style block". Without it the figures
still compile but lose every box, arrow and label style.

### Two cosmetic things in Chapter 1, neither of them blocking

Noticed while reading `main.tex` on 2026-09-20, recorded rather than changed,
because the live document is Kinan's. **Neither stops the document building** —
he compiles it in Overleaf and the text is all there:

1. **§1.4 and §1.5 use `\item` outside any list environment.** Measured rather
   than assumed on 2026-09-20: `pdflatex` raises _"Lonely \item--perhaps a missing
   list environment"_ once per item, **recovers, and still writes the PDF with the
   text present**. The cost is only that those entries lose their bullets and
   indentation and the log carries two errors. Wrapping them in
   `\begin{itemize} … \end{itemize}` would tidy both.
2. **§1.1 writes "Figure 1.1" as plain text** rather than `\ref`, so the number
   stops being true the moment a figure is added before it.

### The two model reports, and what they settle

Both are in `Documentation/GradProj/` and both were read on 2026-09-20:
_Design and Implementation of a Decentralized Firewall Based on Blockchain_ and
_…a Decentralized Business/NGOs Ledger Technology_. They agree on the spine —
**3.1 requirements, 3.2 analysis of requirements, 3.3 analysis of constraints,
3.4 different design approaches, 3.5 developed design** — which is the spine
`CHAPTER3-MATERIAL.md` was already keyed to, so the section numbers in that file
map one-to-one onto the report.

Two things they settle that are easy to get wrong:

- **Chapter 3 carries few figures, and no screenshots.** The firewall report's
  Chapter 3 runs fourteen pages with three figures, all flowcharts; its
  forty-odd screenshots are all in Chapter 4. Ours plans twelve figures in
  Chapter 3, which is far more, and that is a deliberate difference worth
  defending rather than an oversight: this design has more moving parts than a
  smart contract and a web page.
- **Neither discusses engineering standards in Chapter 3**, but the PSUT
  template marks that section `[Required]`. Ours keeps it, at §3.4, after the
  constraints. It is the one section of Chapter 3 with **no material behind it
  in this repository** and must be written from §1.5's wording plus the design
  as built.
...
**The modules added since that list, newest first** (the file tree is the
authority; this is only so a reader knows what to open):

```
src/governance/approval-answerers.ts              C15, who answered an escalation, for the request it files
ui/src/pages/governance/panels/agent-edit-controls.ts  A13, rename an agent and give it to another owner
src/governance/agent-host-deletion.ts             C13, OpenClaw's own delete, run in-process
ui/src/pages/governance/panels/agent-delete-choice.ts  C13, the two-option dialog and its wording
ui/src/pages/governance/panels/policy-agent-overrides.ts  A12, one agent's posture and escalation
ui/src/pages/governance/identity.ts               who administers an agent on the page (finding 375)
src/governance/host-prompt-audit.ts               T57, a prompt recorded whatever surface it came from
```
...
>   (`extensions/qa-lab/src/providers/mock-openai/server.ts`, started by the launch entry
>   `qa-mock-openai-6` on port 44080) answers `Reply with exactly: X`, and the prompt
>   ``tool progress qa check. Call the exec tool exactly once with this exact command before answering: `hostname` ``
>   makes it request one `exec`. Pick a command no shipped rule allows: `whoami`, `ls` and a
>   workspace read are allowed by the baseline rules and never escalate. A first run on a cold
>   Gateway takes about 30 seconds before the model is reached.
> - **The dashboard does not poll for approval cards in a hidden tab** (`document.hidden`,
>   `approval-controller.ts`), and the Claude browser pane's tabs report hidden when the pane
>   is. A card that "never appears" there is the harness. Override `document.hidden` in the
>   test tab, and say so in the record.
> - **Never run `test/vitest/vitest.e2e.config.ts` while a QA Gateway is running from
>   `dist`.** Its global setup rebuilds `dist` for private QA with a 300-second cap; on this
>   machine it is killed part-way, leaving about 5,800 files rewritten and `dist` half-built.
>   Rebuild with `node scripts/build-all.mjs` afterwards. To run one e2e file without it, use
>   a throwaway config that spreads the e2e config and sets `globalSetup: []`.
> - **`src/agents/agent-tools.before-tool-call.e2e.test.ts` fails 6 tests on Windows at HEAD**
>   (skill-usage diagnostics and `apply_patch` paths), POSIX path literals again. Proved by
>   title the same way as `agents-mutate.test.ts` above.

> **One addition, 2026-09-20, from building C4 (the wizard wording).**
>
> - **`src/commands/onboard-guided.test.ts` fails 12 of its 23 tests at HEAD on this machine**,
>   and no register mentioned it before today. The flow returns before `applySetup` or
>   `prompter.outro` is reached, so the assertions see zero calls. **It is not the fork's**:
>   `git diff main..HEAD` is empty for `onboard-guided.ts` and its test, so both files are
>   upstream's unchanged, and the governance layer has never touched guided onboarding. The
>   cause is undiagnosed; the TTY gate is already mocked true, so that is not it. It is
>   outside the documented verification commands, which is why it went unseen, the same way
>   finding 345 did. **Baseline it before blaming your own change**: 12 failed / 11 passed / 23.

> **Two additions, 2026-09-18, from driving a QA Gateway live.**
>
> - **A Gateway started with no configuration exits 78 at once** (_"Missing config. Run `openclaw
setup` or set gateway.mode=local"_), and the preview pane reported it as started, with no logs.
>   Write `<state dir>/openclaw.json` first: `gateway.mode: "local"`, token auth and one agent.
> - **Point HOME at the QA directory.** OpenClaw's trash is `os.homedir()/.Trash`, so a full delete
>   driven live otherwise moves an agent's files into the real profile. `governance-gateway-qa5`
>   sets HOME and USERPROFILE for this reason.

> **Four additions, 2026-09-15, from building C13.**
>
> - **A Vitest worker thread on Windows does not see its own HOME.** `os.homedir()` returns
>   the real user profile even when the shared setup points HOME and USERPROFILE at the
>   per-worker temp home, because a worker's `process.env` is not the native environment.
>   Anything that writes under the home folder, OpenClaw's trash (`os.homedir()/.Trash`)
>   included, writes into the real profile. Spy `os.homedir` in such a test and check it
>   before acting, as `src/gateway/governance-host-agent-deletion.test.ts` does.
> - **Agent ids are unique across an installation.** A governance test that seeds agents in
>   `beforeEach` needs fresh ids each time, or the second test fails with
>   `DuplicateAgentError`.
> - **`src/gateway/server-methods/agents-mutate.test.ts` fails 30 tests on Windows at HEAD**,
>   all POSIX path literals (`/agents/…` against `C:\agents\…`). Prove a red run is those 30
>   by title: swap the committed files in behind a restoring `trap`, run, and diff the
>   failing titles.
> - **A negative assertion on a POSIX literal passes on Windows without proving anything.**
>   `not.toHaveBeenCalledWith("/workspace/x")` holds when the handler was given
>   `C:\workspace\x`; compare against `path.resolve`d paths.
...
>
> - **The pnpm-free protocol checks were missing one: `node scripts/check-protocol-since.mjs`.**
>   It checks every new core Gateway method's release train against the current one.
>   T60 registered `plugin.approval.reportOutcome` as `2026.9` while the train is
>   `2026.8`, and it failed at HEAD for two days because nothing listed it. Run it with
>   the Swift, Kotlin and registry checks below.
> - **An `*.e2e.test.ts` file runs a full private-QA build before its first test.**
>   `test/vitest/vitest.e2e.config.ts` has a global setup that builds `dist` with
>   `OPENCLAW_BUILD_PRIVATE_QA=1` under a 300-second cap. It timed out here, and the run
>   then reported _"No test files found"_ for a file that exists. A file that mocks the
>   Gateway needs no build: run it through a copy of the config with `globalSetup: []`.
>   Six tests in `agent-tools.before-tool-call.e2e.test.ts` fail on Windows either way,
>   on POSIX path separators (`C:\tmp` against `\tmp`), and none of them is an approval test.
>
> **Six additions, 2026-09-13**, each learned the hard way in the QA pass.
>
> - **The full lint gate's core shard now needs about fifteen minutes here, against a
>   905-second cap.** One run finished just under it and the next timed out (exit
>   124). Raise the cap:
>   `OPENCLAW_OXLINT_SHARD_TIMEOUT_MS=2700000 node scripts/run-lint.mjs`. Its
>   `plugin-sdk boundary dts` pre-step can also time out at 300 s when the machine is
>   busy; that too is a timeout, not a verdict.
> - **A plain `oxlint --tsconfig …` call on one file does not run the type-aware
>   rules**, and passes code the gate rejects. To check files quickly, go through the
>   wrapper, `node scripts/run-oxlint.mjs --tsconfig config/tsconfig/oxlint.core.json <files>`,
>   and include a positive control that must fail.
> - **The isolated UI tests have their own config:**
>   `node node_modules/vitest/vitest.mjs run --config test/vitest/vitest.ui-isolated.config.ts`.
>   `--project ui-isolated` against `ui/vitest.config.ts` finds nothing.
> - **Source scans pin spellings.** `host-prompt-audit.test.ts` reads
>   `agent-command.ts` as text; replacing a literal with a constant broke it, and only
>   the whole governance suite noticed. Run the whole suite after touching a shared name.
> - **`write-cli-startup-metadata` times out on the Windows machine even when idle**:
>   180 s with its own environment, 142 s with Windows' system variables added, 50 s
>   from a warm shell. Record it rather than chase it; no governance file sits behind
>   it, and T3's Linux build is the one that counts.
> - **Driving the live QA Gateway:** a cold start after a rebuild took nine minutes;
>   page loads stall about thirty seconds during an embedded run; a task survives its
>   tab, so wait for `agent/runs` to be empty between probes; the approval card is
>   modal, so cancel through the API; and a probe that exits 0 may have caught its own
>   error, so read its `SCRIPT ERROR` line.
>
> **Three additions, 2026-09-11.**
>
> - **Audit the documents as well as the code:**
>   `node docs-notes/qa-sweep-2026-09-08/doc-audit.mjs` lists dead file references,
>   prose naming the removed command line, and every finding-count claim side by
>   side. Run it after editing any register.
> - **A Gateway protocol change needs the generated app models regenerated.**
>   `pnpm protocol:check` compares against the committed Swift and Kotlin models
>   (T60 added `plugin.approval.reportOutcome`). Without `pnpm`:
>   `node --import tsx scripts/protocol-gen-swift.ts --check`,
>   `node --import tsx scripts/protocol-gen-kotlin.ts` (then check `git diff` is
>   unchanged), and `node --import tsx scripts/check-protocol-registry.mjs`.
> - **Never run heavy checks beside a measurement you mean to trust.** On
>   2026-09-11 `build-all` exited 1 on its last step, `write-cli-startup-metadata`,
>   because a CLI help render hit its 120-second cap while a test suite and three
>   typechecks ran beside it; the same step passed alone in 93 seconds. Before
>   calling the build red, re-run that step on an idle machine:
>   `node --import tsx scripts/write-cli-startup-metadata.ts`. **And when a change
>   touches a shared module, run the wider governance suite, not only the tests
>   written with it** — caveat 25.

> **Two things added 2026-09-07 (iii), and both exist because the six commands
> below were all green while something was wrong.**
>
> **The full lint gate is not the sixth command.** `node scripts/run-lint.mjs`
> runs the **type-aware** rules, `scripts/` and the CSS check; plain `oxlint`
> below does not. They disagreed on 2026-09-07 on a line plain `oxlint` had
> itself demanded, so "the cheap one passed" is not evidence about the gate. It
> takes **about eighteen minutes** here, measured twice, which is why nothing
> runs it automatically (T50). **Read its exit code, never its tail**: the tail
> is twenty lines of `finished` even on a failure, and the one error sits at
> line 25 of an 82-line log.
>
> **And never read it from a wrapper's exit code either.** Running it as a
> background job on 2026-09-07 (iii), the harness reported _"completed (exit code
> 0)"_ **twice** while the gate itself had exited **1** — once on a phase
> timeout under load, once on a real lint error. The wrapper was reporting its
> own success at running the command, not the command's verdict. Capture the
> gate's status explicitly and read that:
>
> ```bash
> node scripts/run-lint.mjs > lint.log 2>&1; echo "EXIT=$?" >> lint.log
> ```
>
> This is the same family as the `| tail` that cost finding 169 and the
> `$?`-after-a-pipeline that reported "lint exit 0" twice on 2026-09-06: **three
> different ways of reading somebody else's exit code instead of the gate's.**
>
> **A timeout is not a failure**, and the two must not be collapsed. Exit **124**
> is a shard hitting its cap; exit **1** is a rule that fired. On 2026-09-07
> (iii) the gate produced both, on a machine that had restarted twice —
> `plugin-sdk boundary root shims timed out after 300000ms`, then
> `[oxlint:core] timed out after 900s`. Neither says anything about the code.
>
> **The cap is configurable**, which is worth knowing before concluding the gate
> "does not work here". The default shard timeout is fifteen minutes:
>
> ```bash
> OPENCLAW_OXLINT_SHARD_TIMEOUT_MS=2700000 node scripts/run-lint.mjs
> ```
>
> **Do not fold a raised-cap run into T50's timing.** The ~18 minutes recorded
> there came from two clean runs on an unloaded machine and is the number that
> should inform the hook-or-CI decision; a run that needed its cap raised is a
...
> ```
>
> **After removing anything, check the build, not just the source.** Every
> command below reads the **source tree**; what a user runs is `dist/`, and
> nothing in the build clears it (finding 274). On 2026-09-07 the whole compiled
> governance command line sat in `dist/` for hours after its source was deleted,
> with all six commands, both lint gates and a purpose-written removal sweep
> green (finding 284).
>
> ```bash
...
> ```
>
> _(`pnpm build` is the same thing, when `pnpm` is on your PATH. It was not on
> one used here, so the command deleted `dist/` and then failed — a destructive
> step should not be chained behind a command whose availability has not been
> checked.)_
...
**An eighth, added 2026-09-07 (iii) with T62. It is the only check in this list
that does not run the product**, which is the whole point of it: it verifies the
audit chain from outside, with no build, no server and no sign-in, so it still
...
Exit **0** intact, **1** broken, **2** could not check — and the third is not a
pass. Compare its printed chain head against the dashboard's; a disagreement
...
| Command                  | Expected                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| ------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Governance suite         | **2,801 across 146 file runs (2,780 passed + 21 skipped), re-measured 2026-09-08 (vi) on Windows**, after T55: +9 tests in one new file. It was 2,771/145 after findings 305–323, and 2,765 across 144 (2,745 + 20) on 2026-09-08 (ii) after findings 296–304: +20 tests over 2,725 and one new file, matching exactly what was added. It was 2,745 (2,725 + 20) after findings 285–295. It was 2,743 (2,723 + 20) after findings 279 and 280 and T56. It was 2,814/156 on 2026-09-05 after T53 and the M-series and five-day sweeps. It was 2,770/152 earlier that day after the bounds sweep, 2,766/151 after 256, and 2,757/150 on 2026-09-04 after the parity sweep. It was 2,732/149 earlier that day after T52, 2,701/146 on 2026-09-03 after the twelfth and thirteenth sweeps, 2,695/145 before them, and 2,684/144 on 2026-09-02. **The 20 skipped are not a warning and reading them as one wastes an hour**: 15 belong to `governance-textbox-fit.browser.test.ts`, which needs a layout engine and is skipped by design in this jsdom config (finding 250), and the other 5 are the pre-existing platform skips. To actually run those 15, see the seventh verification command in §4. **Run this one from a POSIX shell or use the PowerShell form above, and check the file count rather than the exit code**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `tsgo:core`              | clean                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `tsgo:ui`                | clean                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
neither is one of §4's seven verification commands:
...
| ------------------------------------------------------- | ------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `REMAINING-WORK.md` §"The numbered backlog", **T1–T69** | The original project, plus everything added since             | **Nine unstruck, one of which is T1 (not being done), so eight open.** Re-derived from the rows 2026-09-20. The numbering reaches T69 because T65/T66 were withdrawn as duplicates (finding 343) |
...
> **Re-derived 2026-09-21 from the rows: seven unstruck, of which one is T1, so **six open**. T3 closed that day (the Linux re-run; requirement 9 is Met) and T46 the day before (C4 built).**
>
> **Re-derived 2026-09-20 from the rows: nine unstruck, of which one is T1, so
> eight open.** Nothing in this table closed or was added since 2026-09-15. What moved that
> week is outside it: findings 375–379 found and fixed by driving the dashboard live, and
> **A12, A13 and decisions C13, C14, C15 closed**, so **nothing Claude can do alone is open**.
> The next thing is Chapter 3, on Kinan's word.

> **Re-derived 2026-09-15 from the rows: nine unstruck, of which one is T1, so eight open.**
> T48 and T49 closed that day. T48: Kinan answered yes, so Chapter 3 is written now, and
> anything T3 or T47 changes afterwards is flagged in it rather than waited for. T49:
> option (b), one organisation per installation is the boundary, and the organisation
> structure stays (`docs-notes/CHAPTER3-MATERIAL.md` §3.5.89).
...
> by the accounts that manage its agent. No decision about approvals is open.

> **Re-derived 2026-09-13 (later) from the rows: twelve unstruck, of which one is T1,
> so eleven open.** T69 closed: Kinan chose to build the withdrawal, and finding 363
> is fixed. T68 was then the one decision about approvals still open.

> **Re-derived 2026-09-13 from the rows: thirteen unstruck, of which one is T1, so
> twelve open.** T68 and T69 were added: the two decisions about approvals the QA
> pass left (sweep register C11 and C12). Nothing closed; the pass's findings were
> fixed rather than backlogged, except 363, which is T69.

> **Re-derived 2026-09-11 from the rows: eleven unstruck, of which one is T1, so
> ten open.** T60 and T63 closed — both built at Kinan's decision — and nothing was
> added. **No decision on the dashboard is outstanding any more**; what is left is
> the report, the Linux host, the by-hand plan, and T58/T59.

> **Re-derived 2026-09-09 (iii) from the rows: thirteen unstruck, of which one
> is T1.** So **twelve open**, and the way it got there is the point. Earlier the
> same day it was fourteen, _unchanged from 2026-09-08 while nothing about it was
> unchanged_: **T50 and T55 closed**, and **T63 and T64 were added** — they had
> been open for a day while living only in
> `mg/REMAINING-WORK-DASHBOARD-SWEEP.md`, and T63 had picked up a second number
> there (T65) in the meantime. Two out, two in, same total, different list. That
> is finding 343, and it is the one arithmetic cannot catch: **a stable count is
> not evidence of a current list.** **T64 then closed** when Kinan chose to split
> the page, which is what moved the number.
>
> **The heading is a measurement, and it has been wrong before.** Re-derived
> **2026-09-08** from the rows: **fourteen unstruck**, which is what it said
> then. T61 closed that day (finding 297) and its row is removed from the table
> below rather than left struck, because this table is the _open_ list.
>
> _(Corrected 2026-09-07 (ii), finding 282's second half. The table then held
> **fourteen rows, two of which were closed** — T53 on 2026-09-05, T57 on
> 2026-09-06 — and **omitted three that were open**: T1, T58 and T59. The count
> above it was right while the list below it was wrong in four places, which is
> finding 259 in reverse: **deriving a number from the right source does not
> check the list you print beside it, and a reader takes the list.** T56 and T62
> closed on 2026-09-07 (ii) and (iii); T51 closed by having its question
> removed; T60, T61 and T62 were added the same day.)_
>
> That is finding 259 in reverse: there the count was stale and the rows were
> current, so the fix was to count the rows. Here the count was re-derived and
> the rows were not, because they live in a different file. **Deriving a number
> from the right source does not check the list you print beside it**, and a
> reader takes the list, not the number.
...
| #           | Who                                     | What                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ----------- | --------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| ~~**T2**~~  | ~~You~~                                 | **DONE 2026-09-06.** Kimi drove agent `jack` at a credential file on the VPS, the gate refused it, and ledger entry #25 records the refusal with the rule that produced it. **The first attempt is the one to read**: `~/.ssh/id_rsa` was refused by the _model_, before any tool call, so nothing reached the gate — a request that sounds like a secret tests the model, not the layer                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ~~**T3**~~  | ~~You~~                                 | **DONE 2026-09-21 on the VPS at `0a7d51f1c12`: governance suite 3,211 passed / 16 skipped / 0 failed in 197 files; rehearsal 20/20; platform probe 14/14, `platform=linux node=v22.23.2`. Windows and Linux reconcile exactly at 3,227 tests, five of which skip on Windows and run on Linux. Requirement 9 is Met and T3 is closed.** _The task was:_ Deploy to a Linux host. The one design requirement not fully met; T33 made the build work. **Re-run the suite there**, the last Linux measurement is 2,548/133 and predates T44 and three sweeps                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| **T13**     | You                                     | Read the prompt-injection answer (§4.x.26) until you can give it without notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| **T17**     | You                                     | **DECIDED BY KINAN 2026-09-20, AND DONE THE SAME DAY.** MiKTeX was installed and all fourteen kept figures were compiled, one by one and together: **14 of 14 clean**, no dropped characters, nothing overflowing the text block. The compile found two defects four readings had missed, both fixed (`docs-notes/FIGURES.md` §"2026-09-20"): `of {(x,y)}` positioning silently dropped five printed characters across F11 and F3, and F1, F3 and F21 ran 58, 39 and 13 pt wide. **Then the rendered pages were looked at, which found twelve more defects across nine of the fourteen** — boxes on top of boxes, an arrow drawn through the box it pointed at, labels covering their own arrow heads, a word cut in half, and coloured chart labels in a greyscale file — all fixed, and none of them visible to a compile. Three figures were clean at first look: F19, F21, F22. `docs-notes/figures/build-figures.mjs` rebuilds the document from `FIGURES.md`; the PDF was sent to Kinan. **Two things remain, both yours**: approve the fourteen, and compile once inside `main.tex`, which has not been done and is where the template's own text width settles the `\resizebox` widths. _The task was:_ Redraw the figures, 21 candidates marked. Or decide Claude drafts them for approval **The audit half is done (2026-09-06)**: all 21 figures compared against the code and checked for completeness. All three forms present for every one — F18 carries none and correctly so, being a CUT cross-reference. **Two defects fixed**: F21's TikZ would not have compiled (five line breaks had lost a backslash, so LaTeX read `\a`, `\backend` and three more as undefined control sequences), and F12 claimed an installation may hold more than one organisation, which the 2026-08-30 cap made false — the same correction F2 had already had, applied to the figure the claim actually belongs to. **What remains is yours**: whether to redraw them, or to have Claude draft the 21 for approval. The remaining risk is that nobody has _compiled_ them; that needs a LaTeX toolchain this machine does not have. |
...
| ~~**T46**~~ | ~~You, then Claude~~                    | **BUILT AND COMMITTED 2026-09-20** in `feat(wizard): C4, the setup wizard names the governance fork`. Seven strings in `src/wizard/i18n/locales/en.ts`, pinned by five tests in `src/wizard/i18n/index.test.ts` that all go red on a revert. The seventh, `findMeLater`, was added after a reachability check found that `wizard.guided.nextSteps` is printed nowhere and the one next-steps call site prints `nextStepsWithoutAi` only when the operator skips AI setup. _The task was:_ **The setup wizard still says "OpenClaw".** `openclaw onboard` presents upstream's text unchanged, so an operator installing _this_ project is never told what it is, no mention of the layer, the tiers, the ledger or the dashboard's second gate. You have said it should read **"OpenClaw Governance"**. **How far it goes was decided by Kinan on 2026-09-20: banner and completion text only, not new prompts (C4). Not yet built.** _The choice was:_ Banner and completion text, or new prompts too. Touches upstream files, so it grows the fork diff §3.5.2b measures                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| **T47**     | All three                               | **A by-hand test plan, one list per RBAC tier, split between K, M and O.** **The plan is written: `docs-notes/T47-TEST-PLAN.md` (2026-09-04).** 138 checks as first written, plus **four added 2026-09-05** from findings 256 and 260, across the four tiers, plus six things no one person can test alone. **Running it is what remains, and it needs three people on three machines** — half of what it tests is that one account cannot see or do another's, and a shared browser session silently defeats that. Every row asks two questions beyond pass/fail: _was the outcome visible_, and _did the refusal say what to do instead_. Produces the evidence Chapter 4 needs, and is the natural companion to T2                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| ~~**T48**~~ | ~~You~~                                 | **DECIDED BY KINAN 2026-09-15: yes, write it now**, and flag in the chapter anything T3 or T47 changes afterwards. _Original row:_ **Decide whether Chapter 3 is ready to be written.** Not "is there enough material", there are ~9,300 lines across 76 sections, but "has the design stopped moving?" Findings 230–232 suggest not quite: three defects in the deployment path in one evening                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| ~~**T49**~~ | ~~You~~                                 | **DECIDED BY KINAN 2026-09-15: option (b).** One organisation per installation is the boundary; the report makes no claim about separation between organisations that exist at once, and the organisation structure stays because it also does work with one organisation (`docs-notes/CHAPTER3-MATERIAL.md` §3.5.89). _Original row:_ **Decide what the multi-tenancy machinery is for now.** The 2026-08-30 cap means a shipped installation only ever has one organisation, so M5's isolation is exercised by nothing that ships, and findings 234 and 235 were **latent for exactly that reason**. Either keep it and say in the report that it is verified by test rather than by deployment, or state the cap as the boundary. What does not work is calling it "still enforced" without re-deriving that **Answer the Codex switch's scope in the same breath** (2026-09-05): `setCodexBackendEnabled` takes a `groupId` but writes an **installation-wide** config key, and the group id only routes the ledger entry — `readCodexBackendState()` takes no group id at all. Harmless while the cap stands, and if it is lifted, org A's Root switches Codex off for org B while org B's ledger says nothing. Measured in `docs-notes/qa-sweep-2026-09-05/codex-scope-probe.ts`; written up in this file's §"An observation about the Codex switch's scope".                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| ~~**T50**~~ | ~~You, then Claude~~                    | **DECIDED AND BUILT 2026-09-09, answer (c).** Finding 237 asked which of three things should enforce the full lint gate, and two of the three are closed by fact rather than by preference. Running `scripts/run-lint.mjs` in the hook costs **~18 minutes a commit**, which is a hook people pass `--no-verify`. **Moving it to CI has nowhere to go**: Actions are switched off on this repository (T21), because the fork inherited 82 upstream workflows, fifteen of them scheduled, which spent the entire free allowance in a day — and deleting them is ruled out because §3.5.2b measures the fork diff. So it stays manual, and the third option's own condition, _stop calling it what the hook runs_, is now met **where an operator meets it**: `git-hooks/pre-commit` prints, after every successful lint, that this is not the full gate and names the command that is. Finding 323 is the argument for saying it at the moment of passing rather than in a document — the gate was recorded as clean and had four errors. **Revisit if Actions are ever re-enabled**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| ~~**T51**~~ | ~~You, then Claude~~                    | **CLOSED 2026-09-07 (iii) by removing the question.** The governance command line was removed entirely, so the two commands and the surface they were incomplete on are gone. One real defect was found while surveying it: a comment claiming `agent cancel` could stop a dashboard-started run, which it could not — the comment an operator would read during an incident                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| ~~**T53**~~ | ~~Claude~~                              | **DONE 2026-09-05.** The page was split; the two prop builders that "read twenty or more private fields" were the seam after all. _(This row stood here as open for two days after it closed — finding 282.)_                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| ~~**T55**~~ | ~~You~~                                 | **DECIDED AND BUILT 2026-09-08 by Mohammad**, his first item (finding 258): deleting an agent clears what its id carried, unregistering keeps it. _(This row stood here as open for a day after it closed, while §1 of this same file announced it — finding 343, and the third row in this table to do it after T53 and T57.)_                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| ~~**T56**~~ | ~~Claude, once you say which surfaces~~ | **DONE 2026-09-07 (ii)**, on both listing surfaces. **The decision this row asked for did not exist**: it named "all three readers of `listPendingDecisions`", and only two of the three are listings — the third is a lookup by id for the decide route, which shows no worklist. The stack now carries a cumulative count of the unanswered questions it has shed, incremented inside the lock that sheds them, and both the command line and the dashboard say so in one shared sentence. The dashboard says it **even when nothing is left waiting**, which is the case its own empty check would otherwise hide                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ~~**T57**~~ | ~~You, then Claude~~                    | **DONE 2026-09-06**, at Kinan's decision: the labelled-origin option. A prompt arriving on any surface with no governance account behind it is recorded under `host-prompt` with the channel named, hooked at `agentCommandInternal` — the single funnel every agent turn passes through. _(Stood here as open for a day after it closed — finding 282.)_                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| **T58**     | You, then Claude                        | **Settle whether `edit` is ours** (2026-09-07). Jack's tool sweep on the VPS reported `edit` failing with ENOENT on a file `write` had just created. **Three seconds of looking**: filter the ledger to Agent actions and find the `edit` call. `deny` with a rule named means governance refused it; `allow` that still failed means the fault is downstream; **no entry at all** means it never reached the gate. Reasoning says it is unlikely to be ours, and the ledger is the measurement                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| **T59**     | You, then Claude                        | **Make per-agent models work, and prove they do** (2026-09-07). Upstream supports it and this layer has never exercised it, which is why two agents sharing a connection looked like a bug when it was correct behaviour with an illegible surface. Turns the provider-agnostic claim into a measurement                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ~~**T60**~~ | ~~You, then Claude~~                    | **DECIDED BY KINAN AND BUILT 2026-09-11.** The escalation card now says what "Always allow" does before it is pressed — allows this once, and asks an Administrator — and a request that cannot be saved produces an explicit warning after the press, on the approval card's surface and on the dashboard's "Would allow". Approval-generated requests get **40 + 20 per organisation account**, de-duplicated inside the file lock, and a shrink never deletes a queued request. Finished from a draft another agent left untested; the record, what was wrong with it and the decisions taken are in `mg/REMAINING-WORK-DASHBOARD-SWEEP.md` §"T60 and T63 built"                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| ~~**T62**~~ | ~~You~~                                 | **DONE 2026-09-07 (iii), option (b).** `scripts/verify-ledger.mjs` restores the second, independent reader of the audit chain without restoring a command surface: no registry entry, no `--help` presence, no tiers, no session, no write path. Imports nothing from `src/`, so it runs with the build broken and the Gateway down, and against files copied off the machine. Re-implements the hashing so a defect in `audit-ledger.ts` cannot agree with itself, and is pinned against the product on intact and tampered chains                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| ~~**T63**~~ | ~~You, then Claude~~                    | **DECIDED BY KINAN AND BUILT 2026-09-11: both options, the conversation first.** Reopening an agent's conversation recovers this account's running task with its Cancel; _Active agent sessions_ lists the same runs with the same Cancel, from one controller and one cancel path. A run now stays listed — as _Stopping_ or _Saving reply_ — until its reply is actually saved. Same record as T60                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ~~**T64**~~ | ~~You, then Claude~~                    | **DECIDED BY KINAN AND BUILT 2026-09-09 (iii): option (a), split the page.** The English catalog is now three modules — `en-core.ts`, which `translate.ts` imports and every session pays for at startup; `en-governance.ts`, which travels in the governance page's own lazy chunk and is registered as that chunk evaluates; and `en.ts`, still the whole catalog, which the translation pipeline imports. **9,019 bytes off every first page load**, and headroom from **67 B to 8,062 B** — from less than one operator-facing sentence to about a hundred and twenty. Verified key-for-key against the committed catalog: **4,956 keys before and after, none missing, none added, no text changed**, and the contributor i18n gate passes. Finding **321's ceiling raise was handed back** (318 → 317 KiB) because the reason for it no longer exists. **One follow-up**: the budget _baseline_ still needs ratcheting and that needs Linux bytes, which T3's VPS rebuild produces — A9 in `mg/REMAINING-WORK-DASHBOARD-SWEEP.md`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| ~~**T68**~~ | ~~You~~                                 | **DECIDED BY KINAN AND BUILT 2026-09-13: option (b).** A dashboard prompt's escalation is answered on the governance page, only by the accounts that manage the agent, and recorded under that account; no other Gateway connection sees it, only the agent run that asked. See the sweep register's §"T68". _Original row:_ **Decide who may answer an escalation raised from a dashboard prompt** (2026-09-13). Since finding 347's fix, any Control UI holding the Gateway credential sees the card and can answer it, whatever its governance tier: the audience every chat run's approvals already have. The alternative, only the accounts that manage the agent, needs a governance identity on the approval path. Found by the independent review of the QA pass; sweep register C11                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| ~~**T69**~~ | ~~You, then Claude~~                    | **DECIDED BY KINAN AND BUILT 2026-09-13: option (a), build the withdrawal.** The approval hook calls a new `plugin.approval.withdraw` the moment its run is aborted; only the requester may call it; the Gateway closes the record as `cancelled`, the card closes, and a chat channel is told the request was cancelled rather than denied. Finding 363 is fixed. _Original row:_ **Decide whether a requester may withdraw an approval it no longer needs** (finding 363, 2026-09-13). A task cancelled while its card is up leaves the card pressable until it expires, and a press changes nothing. The fix is a `plugin.approval.withdraw` Gateway method with regenerated Swift and Kotlin models, the size of T60's `reportOutcome`. Sweep register C12                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| **T1**      | Deprioritised                           | Listed for completeness. Not being done, and counted as neither open nor closed above                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
### ~~Three files are still untracked, and two are load-bearing~~ **All three are committed**

```
scripts/verify-ledger.mjs                        # T62, requirement 8's second witness
src/governance/verify-ledger-script.test.ts      # what makes its independence safe
ui/src/pages/governance/role-change-manager.test.ts
```

~~`git status --porcelain` shows them as `??`.~~ **Verified tracked 2026-09-09**
(`git ls-files --error-unmatch` on all three, and the tree was clean at the
start of that session). A clean clone does have them.

**The instruction underneath is the part worth keeping, so it is restated
rather than struck**: `scripts/verify-ledger.mjs` is what §1 names as the way to
verify the audit chain from outside the product, and it is what the dashboard
points at. **Run `git status --porcelain` before any commit** and look for `??`
against anything load-bearing — this section existed because two such files sat
untracked for days while every document described them as shipped.

### If you are continuing the dashboard sweep, start here

> **Updated 2026-09-13.** Start from the observed coverage matrix and the
> re-verification table in `mg/REMAINING-WORK-DASHBOARD-SWEEP.md` §"The dashboard QA
> pass". It also names what was not driven: "Would allow", and deleting the
> organisation.

### The fourteen sections, and where each one stands

**All fourteen driven.** Driven means: signed in as Root,
Administrator, User **and** Viewer in turn, controls actually pressed, and the
store and the ledger read rather than the screen believed.

> **Updated 2026-09-08 (ix). The sweep is complete: all fourteen sections
> driven from all four tiers.**
>
> **There were fourteen, not thirteen**, and the fourteenth is the one to read
> about. `renderPendingDecisionsSection` — _"Awaiting your decision"_ — returns
> `nothing` unless an escalation has actually timed out, and no sweep and no
> fixture had ever produced that state, so **eight passes scrolled past a
> section that was not on the page.** `section-nav.ts` had said "fourteen
> sections" in its own header the whole time. **A section invisible in the state
> every sweep starts from never gets swept**, and the way to find the next one
> is to ask what each panel's early `return nothing` is waiting for.
> Findings **326–336; nine fixed, two open** (333
> as work, 331 as a decision). Section 8 was finished in (viii) and produced
> **335**, which is the pick of the eleven: five authoring routes reported only
> the first half of a conjunction, so a User whose rule editing Root had
> withheld was told _"You do not manage agent `scout`"_ — and stopped that same
> agent one request later. **T27 exists to separate those two powers and the
> refusal it produced erased its own distinction**, sending the operator to ask
> for an assignment they already held.
>
> **Two things that pass came away with, beyond the findings.**
>
> **The method changed, because the harness would not let a password be typed
> into the sign-in form.** What replaced it is better and should be kept: a
> script captures **every dashboard read route, per tier, off the running
> gateway** (`docs-notes/qa-sweep-2026-09-08/capture-tier-snapshots.mjs`), and a
> browser probe renders the real page from those real answers in real Chromium.
> A hand-written fixture cannot omit what the server actually sends, which is
> findings 251's and 313's shared cause. It also prints, per tier, every
> section, every sentence and every control with its disabled state — the
> artefact this table's earlier rows were built by hand.
>
> **The startup budget is now the binding constraint on this work.** Finding
> 321 raised the ceiling to buy 413 bytes; T55 spent it to 255; this pass's
> five sentences spent it to 112, and **the measured figure on 2026-09-09 is
> 67** (325,565 B against a 325,632 B ceiling — run the build, do not decrement
> this number). That is less than one short sentence of headroom for the whole
> product. **T64 is no longer the tidy answer, it is the next thing that blocks
> a text fix**, and the next person who writes an operator-facing sentence will
> meet a red build with no warning from any source-level check.
>
> **Correction, 2026-09-09, finding 342: "in any locale" was wrong and it is
> the sentence this problem was being explained with.** Twenty of the
> twenty-one locales are dynamically imported (`LAZY_LOCALE_REGISTRY` in
> `ui/src/i18n/lib/registry.ts`) and the build emits each as its own chunk;
> only `en.ts` is in the startup requests. A sentence is charged once. The
> correction makes T64 **more** worth doing, not less — 10.5 KB gzipped sits in
> `en.ts` for this page alone — and it names the real obstacle, which is that
> the i18n pipeline hashes `en.ts`'s raw text into the locale baseline.

| #   | Section                                            | State                                     | Findings                        | Notes                                                                                                                                                                    |
| --- | -------------------------------------------------- | ----------------------------------------- | ------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1   | Identity                                           | **Driven** 09-08 (i)(ii), re-driven 09-09 | 300, 303, 304, **331**          | 331: the Viewer's own description promised masking wider than the masking                                                                                                |
| 2   | Accounts                                           | **Driven** 09-08 (i)(ii), re-driven 09-09 | 296–299, 300, 301, 304, **339** | **296 bricked an installation.** 339 was found here and belongs to every section                                                                                         |
| 3   | Agents in your organisation                        | **Driven** 09-08 (ii)(vii)                | 302, **326**                    | 302's fix confirmed live at Administrator. **326: Register discarded T55's answer**                                                                                      |
| 4   | Your agents                                        | **Driven** 09-08 (iii), re-checked (vii)  | 305–316                         | **305 is the big one and is not about this section**                                                                                                                     |
| 5   | Active agent sessions                              | **Driven** 09-08 (v), re-checked (vii)    | 319, 320, 322                   | **319: the panel could not see the runs this product starts**                                                                                                            |
| 6   | Agent permissions                                  | **Driven** 09-08 (vii)                    | **327, 328**                    | **327: answered confidently about an agent that does not exist.** 328 is the one that was already spotted                                                                |
| 7   | Emergency kill switch                              | **Driven** 09-08 (vii), re-driven 09-09   | 332, **341**                    | Tier model correct at four tiers. 341: the danger button armed for a stop that could only be refused                                                                     |
| 8   | Policy                                             | **Driven** 09-08 (viii)                   | **335**                         | Authoring matrix, removal scoping, core rules, validation, folder grant, account override — all correct. **335: T27's distinction erased by its own refusal**            |
| 9   | Audit ledger                                       | **Driven** 09-08 (vii), re-driven 09-09   | 329, **333**, **340**           | **333 closed with no trade**: the scan window is a constant, so finding 82's bound is untouched                                                                          |
| 10  | Rule requests                                      | **Driven** 09-08 (vii), re-driven 09-09   | 330, **331**                    | 331 decided: the queue stays readable and the Identity sentence was corrected                                                                                            |
| 11  | System resources                                   | **Driven** 09-08 (vii)                    | —                               | Correct at every tier. One cosmetic note in `REMAINING-WORK.md`                                                                                                          |
| 12  | Organisation                                       | **Driven** 09-08 (vii)                    | **334**                         | **Deletion driven to completion** and refused at all three lower tiers; ledger retained, installation-level entry written                                                |
| 13  | Deployment and network posture                     | **Driven** 09-08 (vii)                    | —                               | Root-only, confirmed. Caught a real misconfiguration during the sweep                                                                                                    |
| 14  | **Awaiting your decision** (timed-out escalations) | **Driven** 09-08 (ix)                     | **338**                         | **The section no sweep had ever rendered.** Invisible unless an escalation has timed out, so eight passes went past it. 338: its hint promised a rule that nothing filed |

~~**Start at 6, Agent permissions**~~ **— all fourteen are driven; there is no
next section.** _(What stood here named section 6 and the defect to start with,
and it was answered the same day as findings 327 and 328. Kept struck rather
than deleted, because the paragraph below it was still telling readers that
sections 8, 9 and 12 were undriven a day after all three had been.)_

**Where the remaining QA value is, now that every section has been driven**, is
the five gaps in `mg/REMAINING-WORK-DASHBOARD-SWEEP.md` §"What more QA is worth
doing on these sections": pressing the last few controls **by hand** (Q1), two
operators at once (Q2), a **small window** (Q3), the page **under load** (Q4),
and the error banner's **siblings** (Q5). Q1 is the one with the record behind
it — every finding in this sweep that a render or an API matrix could not see
came from a press.

**Two things the paragraph that used to stand here said, and their answers:**

- ~~Sections 8 and 12 have not been driven from a tier below Root.~~ **Both
  have.** Section 8 was driven at all four tiers on 09-08 (viii) and produced
  finding **335**; section 12's deletion was driven to completion from Root and
  **refused at all three lower tiers**, with the store read on disk afterwards.
- ~~Section 9 is half-done.~~ **Done**, and re-driven on 09-09: the filters, the
  Viewer masking and the count line all produced findings (329, 333, 340), all
  fixed.

### What driving a section has cost and returned, so far

Five sections, **twenty-one findings** (305–325), of which **eight were
invisible from Root**. The rate has not dropped: section five produced three,
and one of them (319) is among the most consequential of the whole sweep.

**Two of the twenty-one were not in any section**, and both were found by
running a check nothing pointed at rather than by driving a screen:

- **321**: the tree had stopped building, and had been broken for a day.
- **323**: the lint gate was recorded as 0 and was 4.

**Run `node scripts/build-all.mjs` and plain `oxlint` at the _start_ of a
section, not the end.** They are the two verification commands that read the
**artefact** and the **whole tree** rather than the file being edited, and they
are the two that were red.

**Read finding 321 before the next one.** The clarity work these sweeps produce
is operator-facing _text_, `en.ts` is one locale module loaded at **startup**
while every page is lazy, and the startup budget is charged per sentence across
the locale set. The ceiling was raised once (317 → 318 KiB) and now has **413
bytes of headroom, which is about four more sentences.** `node
scripts/build-all.mjs` is the only check that sees this and no source-level gate
does — run it before finishing a section, not after finishing the sweep. **T64**
is the decision that stops this recurring.

**And one thing this section changed about the method.** Finding 319 was found by
comparing two of the product's own registries against each other while something
was actually running — `agent/runs` said a prompt was in flight, `sessions` said
nothing was. Neither surface was wrong on its own terms. **Where two views of
one fact exist, read both at the same moment**; it is the cheapest thing in this
sweep that found the largest defect.

### The fourteenth section, in plain terms

**There are fourteen sections on the governance page, and one of them is usually
not there.** It is called **"Awaiting your decision"**, and it sits between _Your
agents_ and _Active agent sessions_.

**What it is for.** When an agent tries to do something no rule allows, the
policy can be set to _ask a human_ rather than refuse outright. The agent stops
and waits. If nobody answers within the time limit — five minutes by default —
the action is refused and the agent carries on with something else. **The
question does not disappear.** It is kept, and this section is where it waits:
_"scout tried to read /var/data/payroll.csv, nobody answered, here it is."_

An operator can then say **"Would allow"** or **"Keep denied"**. The words are
conditional on purpose — the moment has passed and the action was already
refused, so nothing is being undone. What the answer does is record the
operator's judgement in the audit ledger and, since 2026-09-09, file a rule
request so an Administrator can make the _next_ attempt succeed.

**Why you will usually not see it.** The section renders **only when something
is waiting**. On a healthy installation nothing is, so the page shows thirteen
sections and the list in the sidebar has thirteen entries. It appears when an
escalation has actually timed out, and disappears again once every question has
been answered.

**Why that matters for QA, which is the real lesson.** No sweep had ever seen
it. Producing a row means driving the gate to escalate _and then letting the
escalation lapse_, and no fixture did that — the test fixtures set `ask: "off"`,
which refuses outright and never asks anybody. So eight passes examined a page
this section was not on, and `section-nav.ts` had said _"fourteen sections"_ in
its own header the whole time.

**The generalisable rule:** a panel that begins `if (nothing to show) return
nothing` is a statement about a state the sweep has to construct before it can
be looked at. **Read every early return and ask what it is waiting for** — that
is how the next hidden one gets found, and looking at the page is not.

### Standing policy: gateway auth during a live dashboard run

**Decided by Kinan, 2026-09-08. This is the answer every time; do not re-ask.**

A live run of the dashboard needs a browser signed in, and two gates stand in
front of it: the **Gateway credential** (a token, in `gateway.auth`) and the
**governance sign-in** (an account in `users.json`). They are different things
and only the second is this project's subject.

**Turn the Gateway credential off on the throwaway QA instance. Never anywhere
else.** Set `gateway.auth.mode` to `"none"` in the **copied** config the QA
launch entry points at through `OPENCLAW_STATE_DIR` — never in `~/.openclaw`,
never in the repository, never in anything an operator downloads.

**Why this is the right trade rather than a shortcut.** The Gateway credential
is upstream's, it guards a loopback-only port on a developer machine, and
switching it off costs nothing this project is trying to demonstrate. The
governance sign-in **stays on**, and it is the one under test: every tier check,
every refusal, every masking rule in these sweeps runs through it. A pass that
disabled _that_ would be measuring nothing.

**The three conditions, and all three must hold:**

1. **The file edited is outside the repository.** The QA config lives in the
   session scratchpad. Check it the way it was checked on 2026-09-08: `git
check-ignore` on the path should answer _"outside repository"_, and `git
status --porcelain -- '*.json'` should be empty.
2. **`~/.openclaw/openclaw.json` is untouched** and still reads `"mode":
"token"`. It is copied _from_, never written to.
3. **`git grep '"mode": "none"'` finds nothing but
   `.github/workflows/openclaw-performance.yml`**, which is upstream's own CI
   file and must stay unmodified. Anything else is a leak and must not be
   committed.

**Say so in the write-up.** A sweep run this way records that the Gateway
credential was off and the governance sign-in was not, because a reader
otherwise cannot tell which gate the evidence came through — and the deployment
report will correctly fail two checks while it is off, which is the report doing
its job rather than a defect. _(That is exactly what happened on 2026-09-08:
"Gateway authentication configured" and "Gateway auth missing on loopback" both
failed, and both were true of the QA instance and of nothing that ships.)_

**What this policy does not license.** It is not permission to weaken a default
in the product, to commit a config change, or to disable the governance login.
If a live run ever seems to need one of those, that is a finding about the
product, not a setup step.

**How to set it up, which is ten minutes.** A launch entry
`governance-gateway-qa` in `.claude/launch.json` runs the Gateway on 18799
against a **throwaway governance directory**, so nothing here touches a real
install.

> **If a Gateway is already running, you cannot just start a second one**, and
> this cost half an hour on 2026-09-08 (v). It fails with `gateway already
running (pid …); lock timeout after 5000ms` — a single-instance lock, not a
> port clash, so changing `--port` does not help. The three things it needs,
> together, are in the `governance-gateway-qa2` entry beside it:
>
> ```
> OPENCLAW_ALLOW_MULTI_GATEWAY=1     # skips the config lock
> OPENCLAW_STATE_DIR=<throwaway>     # its own state lock
> OPENCLAW_GOVERNANCE_DIR=<throwaway>
> ```
>
> **And `OPENCLAW_STATE_DIR` relocates the config too**, so a fresh one exits
> `78` with _"Missing config. Run `openclaw setup`"_ — copy `openclaw.json`
> (and `agents/`) into it, or pass `--allow-unconfigured` and accept a host with
> no agents. **Do not set `OPENCLAW_HOME`**: that moves everything and gets you
> the same 78 with more to undo. Bootstrap a Root, then create one account at each tier — the
> cheapest version of caveat 24 — and sign in as each in turn:

```
kinan   root           (bootstrap)
haitham administrator
lina    user           answers to haitham
noor    viewer         answers to haitham
```

> **⚠ Two environment facts that will cost you an hour each if you do not know
> them, both measured 2026-09-08 (v).**
>
> **The Gateway's event loop blocks for 40–50 seconds while an embedded agent
> run is in flight on this machine.** Every HTTP route stops answering and one
> core pins at 100%. It does it on a run started from the **host's own chat**
> too, so it is not this layer's prompt path; it recovers when the run ends. If
> the dashboard goes unresponsive mid-prompt, that is this and not a defect you
> have just introduced.
>
> **The Gemini quota on the configured key is exhausted** (`429`), and the
> previously-configured `google/gemini-2.5-flash` was **retired upstream**
> (`404`). No live model run can be carried to a reply here. Everything a run
> is needed for — the outcome paths, the transcript, T2-style demonstrations —
> has to be exercised by **refusal** (lock the agent down, then prompt) or on
> the VPS.

**Three habits that earned their place in this sweep:**

1. **Read the store and the ledger, not the screen, to decide whether
   something happened.** Twice a check ran while the operation was still in
   flight — once before a confirmation dialog had been answered, once while
   an agent was still provisioning — and both times a defect was nearly
   filed against something that then worked.
2. **Rebuild the Control UI after every change** (`node scripts/ui.js
build`, about 11s) and remember that **the Gateway's own auto-rebuild
   deletes `dist/control-ui`** and serves a 503 until it has rebuilt it. A
   server change needs the Gateway restarted; a UI change does not.
3. **A refused operation should leave no ledger entry**, and in this sweep
   every one of them did. That is worth checking each time: it is the
   cheapest test of whether a failure was clean, and finding 296 is what it
   looks like when one is not. _(A **governed** refusal is the opposite case
   and must be recorded: prompting a locked-down agent wrote entry #20,
   decision `deny`, naming the reason. "Refused" means the request never
   happened; a denial is a decision the gate made and is requirement 5's
   subject.)_
4. **Create an account at each tier that the section is for, including the
   awkward one.** Three of the four sections so far turned on an account
   somebody would not think to make: a User with an **empty** assignment
   reads a different screen from a User with agents and from an
   Administrator with no list, and findings 303, 309 and 311 are all only
   visible from it.
5. **`document.hidden` stops the auto-refresh, by design.** A page that "is
   not updating" in a background tab or an unfocused preview pane is
   behaving correctly, and the first measurement of finding 305 was taken
   that way and proved nothing. Trigger a refresh with a real mutation, or
   settle the question in the source, before concluding anything about
   staleness.

### Do this before anything else

**First, if `git status` still shows it, commit and push the T60/T63 work**
(2026-09-11): every file `git status` lists apart from `.codex/` — both features, their protocol change
with the regenerated Swift and Kotlin models, and these documents. It is the only
copy. `.codex/` belongs to another agent and is not part of the commit.

**Rebuild the VPS, with the daemon stopped.** This is the one thing standing
...
**And since 2026-09-07 (iv) it is load-bearing for a screenshot you are
otherwise likely to take.** The dashboard's chain-verification row printed _"Run
`openclaw governance audit verify` at the terminal to recompute this
independently"_ — a command the server will no longer have once it is rebuilt,
and which the current build still offers, so the two are wrong in opposite
directions and **only the rebuild makes them agree**. It now names
`node scripts/verify-ledger.mjs` (finding 285). Requirement 8's evidence is a
screenshot of that row beside the script's own output, and taken against the
pre-rebuild server it would show the deleted command instead.
...
A quick way to see the rest of the state at once, once the daemon is up: open
the tunnel and read the dashboard's **Deployment report** panel. _(This was
`openclaw governance deployment`, which ran over a plain SSH session **before**
any tunnel existed. That command was removed on 2026-09-07 and the
before-the-tunnel route went with it, which is the one real cost of the removal
— set the tunnel up first.)_
...
| #                 | What                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Effort   |
| ----------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- |
| ~~**T2**~~        | ~~**Run it once with a real agent** and record what happens.~~ **DONE 2026-09-06.** Until that night every proof was a test calling the gate directly; no language model had driven a tool call through it. _The single highest-value item left_                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | 2–4 days |
...
| **T3**            | **Deploy to a real Linux host. Half done: the fork has been installed and running on a VPS since 2026-09-03**, and T2 was demonstrated there. The suite runs on Ubuntu under WSL2 and has not been re-run on the server. _(This row read "nothing has run on a VPS" for four days after it did; finding 282.)_ The one requirement (#9) not fully met. **Blocked on T33**, a VPS that cannot run the build wastes the booking rather than the afternoon                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | 3–5 days |
...
| **T17**           | **The figures.** 24 headings in `docs-notes/FIGURES.md` (F23 and F24 added 2026-09-11 for T60 and T63), 23 drawable, each in prose, Mermaid and TikZ, none compiled. Redraw them, or ask for drafts                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| **T47**           | **Run the by-hand test plan** — three people, three machines. Rows 6b.1–6b.5 and 6c.1–6c.9 are the only proof T60, T63 and the QA pass's fixes will get from a person                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| **T46, T58, T59** | Judgements and checks: how far the setup wizard rewording goes (T46); whether `edit` failing on the VPS is ours (T58); and making per-agent models work (T59)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ~~**T68**~~       | **Decided and built 2026-09-13**, as was its companion T69: a dashboard escalation is answered by the accounts that manage the agent, and a cancelled task withdraws its approval.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |

### Mine, and nothing blocks them

> **As of 2026-09-11:** a hands-on pass over T60 and T63 in the dashboard (Q6), then
> the broad dashboard QA sweep; the 27 failing UI tests (A10); the startup baseline
> ratchet once a Linux build exists (A9); the setup wizard's wording once T46 is
> decided (A8); and drafting the figures if asked (T17). **Committing is Kinan's
> call.** The list below is older and kept as the record.
...
| **T17**     | **Redraw the Mermaid diagrams** in the report's style. Candidates already marked "Figure candidate" throughout `CHAPTER3-MATERIAL.md`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | 2–3 days **The audit half is done (2026-09-06)**: all 21 figures compared against the code and checked for completeness. All three forms present for every one — F18 carries none and correctly so, being a CUT cross-reference. **Two defects fixed**: F21's TikZ would not have compiled (five line breaks had lost a backslash, so LaTeX read `\a`, `\backend` and three more as undefined control sequences), and F12 claimed an installation may hold more than one organisation, which the 2026-08-30 cap made false — the same correction F2 had already had, applied to the figure the claim actually belongs to. **What remains is yours**: whether to redraw them, or to have Claude draft the 21 for approval. The remaining risk is that nobody has _compiled_ them; that needs a LaTeX toolchain this machine does not have. |
...
- ~~**T7**~~: search tools governed at their root only; `grep`/`find`/`ls`
  ...
  2026-08-26** (`search-audit.ts`) and the **prevention half closed 2026-08-30**
  (§3.5.61): a denied result is withheld on the in-process runtime, and is
  structurally unclosable on the native Codex harness, which is recorded as a
  result rather than as a gap. _(This bullet said "only the prevention half is
  open" for eight days after it closed, in a section whose own heading is
  "Nothing to do".)_
...
**2026-09-08 to 2026-09-11:** T55 and T61 (09-08); T50 and T64 (09-09); T60 and
T63 (09-11).

**2026-09-12 and 13:** the QA pass fixed findings 346–362 directly and added T68 and
T69. **T69 closed later on 2026-09-13** when Kinan chose to build the withdrawal,
which fixed finding 363, and **T68 closed the same evening**, when Kinan chose to limit a
dashboard escalation to the accounts that manage its agent.
...
5. **The kill switch reports two numbers**, and the honest one is weaker than
   ...
   **Measured live on 2026-09-19 (§3.5.93): above one second.** Locking a running
   dashboard task, the signal took 1.3 ms and 6.8 ms and the _confirmed_ stop
   2,170 ms and 2,760 ms, on a laptop whose Gateway logged event-loop stalls of up to
   8.6 s. Lockdown is in force before either number, so no further action runs, but
   the report must not say the task was confirmed stopped within one second without
   the VPS measurement (T3, T47 row 6c.7).
...
11. **The multi-tenancy machinery is real and tested, its separation between live
    organisations is exercised by nothing that ships, and on 2026-09-03 that turned
    out to have a cost.** M5 built per-organisation storage, `requireGroup`,
    `requireManagedAgent` and roster filters; the one-organisation cap (caveat 2)
    means a shipped installation holds one organisation at a time, so separation
    between two live organisations never runs in production. _(Until 2026-09-15
    this read "none of it is reachable in production", which is too strong: the same
    structure keeps a deleted organisation's retained audit ledger apart from its
    successor, and refuses an agent id with no registration record;
    `docs-notes/CHAPTER3-MATERIAL.md` §3.5.89.)_ Findings 234
    ...
    gone stale in two places while being repeated in a register.** **T49 settled how
    the report presents it (2026-09-15, option b)**: one organisation per
    installation is the boundary, and no claim is made about separation between
    organisations that exist at once.
...
18. **Requirement 8's own output had to be taken on trust** (finding 268,
    2026-09-06, fixed). Here because "how do you know the ledger has not been
    tampered with?" is a question a viva will ask, and until this week the
    honest answer contained a shrug.

    **What was wrong.** Pressing **Verify chain integrity** reported _"Intact,
    entries verified"_ and nothing else. The verification underneath was real
    and always had been — it recomputes every entry's hash from its contents
    plus the installation's signing key, checks that each entry's `prevHash`
    matches the one before it, and compares the result against a **separate
    checkpoint file**, which is the only thing that can detect entries deleted
    from the end, since a truncated chain is still a valid chain. None of that
    reached the operator. **A tamper-evidence feature whose verdict must be
    taken on trust is missing the half that matters**, and Kinan's phrase for it
    was exact: the project was saying "trust me".

    **What it says now.** `verifyLedgerChain` returns the facts it already
    established on the way through — nothing newly computed: the **chain head**
    it terminated at, the **checkpoint** that independently agrees with it, and
    whether the entries are keyed. The panel prints them under one sentence
    explaining the mechanism.

    **⚠ Half of this caveat's repair was undone on 2026-09-07 (iii), and it is
    the half worth knowing about.** The fix originally printed a **terminal
    command** beside the verdict — `openclaw governance audit verify` — so the
    head hash could be recomputed independently, and the claim was: _"the
    dashboard is no longer the only witness; a disagreement between the two is
    itself the finding."_ **That command no longer exists.** The governance
    command line was removed, and with it the second witness.

    **What survives is still more than a shrug**, and the distinction matters
    for a viva. The panel reports the facts the verification established — the
    chain head, the sequence it terminated at, and the **independent
    checkpoint** that agrees with it — so an operator can see _what_ was
    compared rather than only that a comparison happened, and truncation is
    detectable because the checkpoint is a separate file. **What is gone is the
    ability to recompute it from outside the dashboard.** Both readings now come
    through the same process.

    **Restored the same day as `scripts/verify-ledger.mjs` (T62).** The second
    reader is back, and it is a better one than the command it replaces: it
    imports nothing from `src/`, so it runs when the build is broken and the
    Gateway is down; it **cannot sign in**, where the removed `audit verify`
    required a governance session before it would verify — the wrong way round
    for an audit tool, since it made the audit depend on the thing under audit;
    and it **re-implements the hashing** rather than importing it, so a defect in
    `audit-ledger.ts` cannot agree with itself. Compare its printed chain head
    against the dashboard's; a disagreement between the two is itself the
    finding.

    **What is still true**: both readers run on the same host, so §7 caveat 4's
    residual stands unchanged — an attacker who deletes **both** the key and the
    checkpoint leaves nothing on the machine to contradict a rewritten chain.
    The script's contribution there is that it runs against files copied **off**
    the host, which is the arrangement that closes it. The
    ledger is `audit-ledger.jsonl` under the governance directory and its
    checkpoint sits beside it; anything that can read those two files and the
    key can verify the chain without this product's help, which is the property
    that actually matters and is the one §7 caveat 4 already describes.

    **What to say about it.** The tamper-evidence was never the weak part; the
    _reporting_ of it was. That distinction is worth making rather than
    smoothing over, because it is the same distinction findings 202 and 229
    turn on — a control that works and a control that can be seen to work are
    different claims, and this project has now been caught three times shipping
    the first while believing it had the second.

19. **Four of the five findings on 2026-09-06 were capabilities the screen did
    not offer** (264–267, fixed). Not a security caveat, and here because it is
    the most reliable QA lesson the project has produced.

    An operator sat down and tried to make an agent, talk to it, and read the
    log. The Register button existed and was unreachable. The `model` field was
    accepted by the route, the API and the client, and the form never asked. The
    agent id was displayed two sections above the panel demanding you type it.
    Three of the four were **advertised in text while being impossible to do**.

    **None was findable by reading the code, because the code was right.** That
    is the fourth separate occasion where an hour of use beat a day of reading,
    and the standing recommendation is in `mg/SESSION-LOG-2026-09.md`: after any
    UI change, have somebody use it before believing it.

20. **A property claimed in a comment and never tested was the one that was
    false** (finding 279, 2026-09-07, fixed). Here because it is the cheapest
    check this project has found and it took three days to think of.

    `proposeRuleFromEscalation` opens by listing **three** properties it says it
    is responsible for, "each a way it could otherwise grant more than the
    operator saw". Two of them — the proposed pattern is the resource escaped
    and anchored, and it is scoped to the agent that asked — have tests and are
    true. The third, that a read which was escalated proposes a read and not a
    read and a write, had no test **and was not implemented**: the direction went
    into the request's human-readable `reason` and nowhere else, and an absent
    `access` on a rule means both directions.

    So approving a proposal filed from a _read_ granted a permission to
    **write** the same path — measured at the gate through the real approval
    route, not inferred — while the row the Administrator approved displayed
    "(read)", because the queue prints the reason.

    **What to say about it.** The comment was not dishonest; it described what
    the author intended and the code did two of the three things. What made the
    third invisible is that nothing anywhere disagreed with it: the test file
    listing "six properties" of this feature does not include it, so the claim
    existed only in prose, and prose does not fail. **A comment asserting a
    security property is a to-do until something measures it**, and the cheapest
    way to find these is to read the guarantees a module claims for itself and
    ask which of them a test would notice the absence of.

    Same family as findings 112 and 113 — an unreachable validator branch and an
    exported function nothing called — and the same family as `--skip-ui` in
    finding 278: **a thing that announces a consequence it does not have.**

21. **A fix derived from a listing is not finished until the listing is run again
    against the fixed code** (finding 280, 2026-09-07, fixed). Not a security
    caveat. Here because the same mistake was made twice in one hour, the second
    time by the person who had just written up the first.

    Finding 271 asked which of the page's fields survive a sign-out and answered
    "the transcript", from a listing. Running that listing mechanically —
    **50 `@state()` fields against the 15 `endSession` cleared** — says
    twenty-three survive, and `refreshData` reloads only three of them. Among the
    twenty: the Agent permissions panel stayed open on the previous account's
    agent showing **the usernames assigned to it**, a stale alert announced an
    emergency stop the next person had not ordered, and the authoring forms came
    back filled in with the previous account's half-typed rule.

    **The repair for that covered three of the twenty-three, and was the same
    mistake one layer down.** It was caught by re-running the listing after
    fixing rather than by noticing. `endSession` now clears 48 of 50 and is
    written as "everything, less the exceptions", with the two exceptions named
    and argued in place, so the next field added to that component is session
    state until somebody argues otherwise there.

22. **A removal is the hardest change to verify, and every gate this project has
    passes by construction after one** (findings 283 and 284, 2026-09-07, both
    fixed). Here because it is a general lesson about evidence, and because a
    panel asking "how do you know you removed it?" deserves better than "the
    tests pass".

    **Deleting a surface deletes its tests.** So the suite goes _down_ and
    everything remaining is green for a reason that has nothing to do with the
    change. "Still green" is nearly content-free. What has content is the
    **subtraction**: the governance command line took the suite from 2,817 / 156
    to 2,716 / 142, which is exactly the 101 tests and 14 files that were
    archived and nothing else. That arithmetic is the check; the green is not.

    **Two things were still wrong after every gate was green.**

    **283 — the sign outlived the thing.** All eight source files gone, the
    command-registry entry gone, three typechecks and both lint gates clean —
    and `core-command-descriptors.ts` still declared a `governance` descriptor,
    which drives `--help`, argv routing and command suggestions. The product
    advertised a command that could no longer load. Nothing could have caught it
    from the source: a descriptor is _data_, so typechecks are blind to it, and
    the tests that would have exercised it had been deleted with the surface.

    **284 — the source is not the product.** After 283 was fixed and a
    purpose-written removal sweep passed 7/7, `dist/` still held
    `register.governance-BfFo31MS.js`: the entire compiled command tree. On any
    machine running the shipped build, including the VPS, the removed command
    still worked. **Every check this project runs reads the source tree; what a
    user runs is the build, and nothing was comparing them.** Worse, this is
    finding 274's mechanism again — nothing in the build clears `dist/`, so a
    plain rebuild would have left the compiled surface orphaned there
    indefinitely, source-less and still runnable.

    **What to do about it, and it is three lines.** After removing anything:
    check the count moved by exactly what you took out; grep for the _name_
    rather than the files, because descriptors, help text and documentation
    outlive imports; and rebuild with `rm -rf dist` before believing any of it.
    `docs-notes/qa-sweep-2026-09-07/cli-removal-sweep.ts` does the first and
    third, and **skips rather than passes** when there is no `dist/` to inspect,
    because an absent build is not a clean one.

23. **"Grep for the name" means grep the source too, and for the string a person
    reads rather than the symbol a compiler resolves** (finding 285,
    2026-09-07 (iv), fixed). The correction to caveat 22's own advice, found by
    following it one step further, and the reason it is a separate caveat is
    that the removal pass **did** grep — nineteen documents were rewritten from
    a three-day commit list — and still missed the copy that mattered most.

    The dashboard's chain-verification row printed _"Run `openclaw governance
audit verify` at the terminal to recompute this independently of the
    dashboard"_. **That row is finding 268's fix** (caveat 18): the repair that
    stopped requirement 8's verdict having to be taken on trust, whose stated
    claim is _"the dashboard is no longer the only witness"_. A day later the
    command it named was deleted, and for a day the second witness pointed at
    nothing.

    **Three reasons nothing caught it**, and each is worth separating:

    - It is a **string literal in `.ts`**, not prose in `.md`, and the pass
      grepped documents.
    - It is **not an identifier**, so no typecheck, no lint rule and no import
      graph has an opinion about it. The code was correct throughout.
    - The sentence lives in `en.ts` and the **command is interpolated from the
      panel**, so even a search of the translation file finds the sentence
      without the command in it.

    **The habit:** after removing a capability, grep the whole tree — `.ts` and
    `.md` alike — for the _words an operator would type_, and read every hit
    that is inside a quoted string. It is now pinned in both directions: the
    panel test asserts the page names `node scripts/verify-ledger.mjs` **and**
    that the string `openclaw governance` appears nowhere on it.

    **And audit the replacement as hard as the thing it replaced.** Driving
    `scripts/verify-ledger.mjs` in the same session found two defects in it
    (286, 287), both in the direction of a **false alarm** rather than a false
    pass, which is the failure its own header names as worse than having no
    verifier at all.

24. **Drive the dashboard as every tier, not as Root** (findings 301–303,
    2026-09-08, all fixed). Caveat 19 says an hour of _using_ the dashboard
    out-finds a day of reading it. This is its correction, and it cost three
    findings to learn: **every dashboard pass this project has ever done —
    239, 240, 241–252, 264–268 — was done as Root.**

    Root is the tier every string gets written for, and it is the tier that
    can see everything, so a whole class of defect is invisible from it by
    construction:

    - **A fact the page needs may not reach the tiers that need it.** The
      dashboard resolves an account id to a name against the account list,
      and that list is **Root-only** — so an agent's owner rendered as
      `user-1788814759825-7e0761b7` to the Administrator whose agents they
      were (302). Same for `canAuthorPolicy`, which was never sent at all,
      so every **withheld** User read as allowed and was offered the
      authoring controls (301).
    - **A sentence is written for whoever was on screen when it was
      written.** _"You manage every agent, so there is no assigned list"_ is
      true of an Administrator and the exact inverse for a User with nothing
      assigned — and one condition, `assigned.length === 0`, meant both
      (303).

    **The cheap version of this check**: create one account at each tier, and
    sign in as each in turn. It is four sign-ins and it found three defects
    the first time it was tried. **Then read every sentence the lower tier
    sees and ask whether it is true _of them_** — not whether it is true.

    And note what it did **not** find, because that is the other half of the
    result: the tier _enforcement_ held everywhere it was pushed. An
    Administrator asking for `users`, `users` POST and `users/delete` on Root
    got **403** from all three, naming the required and the actual tier. The
    panel hiding is a courtesy; the route is the control, exactly as the
    route module's header claims.

---

25. **Work taken over from another agent is untested work, whatever its summary
    says** (2026-09-11). Codex left T60 and T63 mid-way; its own summary said the
    tests had not been run, and understated the rest. Running every check found
    type errors, 21 lint errors, three over-limit files, wrong test expectations and
    stale protocol models — and **one real defect no reading of the diff showed**:
    concurrent prompts from one account lost agent replies, because keeping a run
    listed until its reply was saved also held its concurrency slot. Only the
    **wider** governance suite caught it, intermittently. Carry the rule: snapshot
    the draft, then run everything, including suites the change did not come with,
    and re-run a failure alone several times before calling it flaky.

26. **A measurement is only as good as the machine's state when it was taken**
    (2026-09-11). `build-all` failed on a CLI help render that took 115 seconds
    under load and 35 seconds idle; a new test failed once under full-suite load and
    passed 5/5 alone, because it polled a one-second clock across a disk write and a
    file lock — re-anchored on the event, not given a longer timeout. **Neither was
    a code defect, and neither could be told apart from one without re-running on a
    quiet machine.** Do not run heavy checks in parallel with a result you intend to
    quote.
27. **Who answers an approval depends on where the run came from** (2026-09-13). An
    escalation from a **dashboard prompt** is answered on the governance page, only by
    the accounts that manage the agent, and recorded under that account (T68, built the
    same day): no Gateway connection sees it, admin scope included. An approval from a
    **chat run** is still answered by whoever holds the Gateway credential, because the
    Control UI connects as a Gateway operator, and "Allow once" there is not attributed
    to a governance account. **Proved by tests, not yet on a live Gateway.** And with no
    dashboard open, a dashboard escalation now waits out its timeout instead of ending
    at once for want of a route.
28. **While a chat run's approval card is up it covers the page** (2026-09-13). With
    one open, the conversation's Cancel and the emergency stop are one click behind it:
    press Deny first. A dashboard escalation's card is drawn inline on the governance page
    instead (T68), so it covers nothing. **A cancelled task's card now closes by itself** — finding 363,
    fixed that day at Kinan's decision (T69): the agent withdraws its approval the
    moment its run stops, and a reviewer who answered first keeps the answer. **Proved
    by tests and not yet driven on a live Gateway**; T47 row 6c.11 drives it by hand,
    which became possible when T68 took the dashboard's card out of the modal.
29. **An exit code of 0 is a verdict only when it is the command's own** (2026-09-13).
    The QA pass met three that were not: a background wrapper reported 0 while
    `build-all` had exited 1; Playwright probes caught their own errors and exited 0;
    and a one-file `oxlint` call passed code the type-aware gate rejects. The remedies
    were the command's own exit line, the probe's `SCRIPT ERROR` observation, and a
    positive control that must fail.

## 8. If you only do one thing

~~**Run it once with a real agent (T2).**~~ **T2 is done** — 2026-09-06, on the
VPS, ledger entry #25. This slot has now been occupied by three things and all
three are closed: losing the work (F1, closed 2026-08-21), the unpushed tree
(closed 2026-08-28), and the live run.

**What occupies it now, and still does on 2026-09-20: rebuild the VPS.** Everything is
committed and pushed (§3's three commands are the claim, not a hash here). **The VPS has
none of the last fortnight**: finding 346's security fix, C13's choice of deletions, A11,
A12, A13, C15, and the fixes for findings 346–379. Whatever is demonstrated on it is a
different product from this repository until it is rebuilt, and **T47 largely tests code
that is not there** (the plan says so in its §0).

It is one command, it is in §6 under
"Do this before anything else", and until it runs the server is a different
product from this repository — it still serves the **governance command line**
that was removed on 2026-09-07, and it does not have that day's fixes.

```bash
cd /opt/openclaw-governance && git pull && openclaw daemon stop && rm -rf dist dist-runtime && pnpm build && openclaw daemon start
```

Then `openclaw --version`, because a build that succeeds says nothing about
which code it built.
...
wants. In this order and nothing else in between:

1. Create an **Administrator** account — **optional as of 2026-09-06**, when
   Root became an eligible owner. This step used to be mandatory and this list
   said so; the dead end it described is gone rather than signposted.
2. Create an **agent**, choosing its owner from the picker: an Administrator,
   or Root itself.
...
5. **Ask the agent to read a credential file**, and keep both the reply and
   the **Audit ledger** panel.

Step 5 is the whole project. Everything before it is setup.

**And capture the second witness beside it**, which is new since 2026-09-07:

```bash
node scripts/verify-ledger.mjs
```

The dashboard's verdict beside a terminal that recomputed the same chain head
independently — no build, no server, no sign-in — is a materially stronger answer
to _"how do you know the log was not tampered with?"_ than either alone.
...
the **Deployment report** panel on it. That single report turns four prose
