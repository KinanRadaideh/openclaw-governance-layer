...
| **Suite**               | **2,842 passed / 21 skipped across 147 files, Windows, exit 0.** Re-measured 2026-09-09 (ii); it was 2,780 / 146 after T55 on 09-08 (vi). The +62 and the +1 file are 09-09 (i)'s dashboard-sweep tests plus this pass's eleven: five on the folded gateway-auth row, four on the agent-id comparison, two on the real-audit tripwire                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| **Backlog**             | **Re-derived from the rows 2026-09-09: fourteen unstruck in §6, one of which is T1.** **T50 and T55 closed**; **T63 and T64 were added to that table**, having been open for a day in `mg/REMAINING-WORK-DASHBOARD-SWEEP.md` only — and T63 had acquired a second number there (T65) while it was out of sight. Two out, two in, same total: **a stable count is not evidence of a current list** (finding 343). The totals this cell used to carry — 62 items / 47 struck / 15 open — are **not re-derived here and should not be repeated**: a grep over `mg/REMAINING-WORK.md` counts prose cross-references as backlog rows and answers differently depending on the pattern. **Count from the table under §"The numbered backlog" and nowhere else** if the number is needed                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| **Findings**            | **344 found, 341 fixed, 3 open.** The three open are **169** (unexplained, never reproduced; T67 in the sweep backlog), **281** (**T60**, a decision) and **316** (**T63**, a decision). _Those last two are also written up as T66 and T65 — one item, two numbers, which is half of finding 343; quote T60 and T63._ Re-derived 2026-09-09 (ii), which added three, all fixed: **342**, the reason recorded for the startup budget was wrong by a factor of twenty — the twenty non-English locales are already lazily imported, so a sentence costs `en` once and not "22 locales", and the correction makes T64 worth ~170× the 60 bytes of headroom that remain rather than less; **343**, rows in the backlog files stale against _other rows in the same files_ (331 and 333 shown open after they closed, T55 shown open in §6 after §1 announced it built) plus two open tasks carrying a second number each; **344**, the **test typecheck was red at HEAD** while the row above said it was 0 — the third time a green cell in this table has described a red command, after 321 (the build) and 323 (the lint gate), and **all three were in this one table**. Before them, 2026-09-09 (i) closed **331** and **333** and added **339** (the error banner rendered off-screen, so every refusal on a fourteen-section page was invisible — measured at y = -12617), **340** (finding 329's own count line read as the size of the trail once a filter was on) and **341** (the kill switch armed for an agent it knew was unregistered). Before that, (ix) added **337** (the ledger verifier reported tampering when it merely lacked a key) and **338** (section 14's hint promised a rule that nothing filed). 258 closed 2026-09-08 (vi) when Mohammad decided T55 |
...
### 2026-09-09 (ii) (latest): the small ones cleared, four decisions, and two claims that were never checked
...
> **Re-derived 2026-09-09 from the rows: fourteen unstruck, of which one is T1.**
> The number is unchanged from 2026-09-08 and **nothing about it is unchanged**,
> which is exactly why it has to be counted rather than carried: **T50 and T55
> closed**, and **T63 and T64 were added** — they had been open for a day while
> living only in `mg/REMAINING-WORK-DASHBOARD-SWEEP.md`, and T63 had picked up a
> second number there (T65) in the meantime. Two out, two in, same total,
> different list. That is finding 343, and it is the one arithmetic cannot
> catch: **a stable count is not evidence of a current list.**
>
...
| **T64**     | You, then Claude                        | **🔴 The locale module split, and the thing that now blocks a text fix.** `translate.ts` statically imports `ui/src/i18n/locales/en.ts`, loaded at **startup**, while the governance page is lazy — so every operator-facing sentence is charged to startup JS, against **60 bytes of measured headroom** (2026-09-09). **Finding 342 corrected the reason recorded for this**: the twenty non-English locales are already lazy, so a sentence costs `en` once and not "22 locales". That makes the fix worth more, not less — the governance strings are ~32 KB of `en.ts`, **10.5 KB gzipped**, about 170× the headroom left. The runtime change is one import; the obstacle is that `scripts/control-ui-i18n.ts` hashes the **raw text** of `en.ts` into the locale baseline the i18n gate compares, so it goes through the locale-refresh flow `ui/AGENTS.md` describes rather than landing as a quiet refactor. _(Added to this table 2026-09-09; open since 09-08 and listed only in the sweep backlog — finding 343.)_                                                       |
