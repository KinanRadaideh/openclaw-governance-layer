...
> **Newest, 2026-09-19 (latest).** Kinan decided **C15** and asked for **A13**, and both are built: a
> request filed by answering an escalation names the account that answered, and agents are renamed and
> re-owned from the registry (`mg/REMAINING-WORK.md` §"C15 and A13 built").
>
> **2026-09-19 (later).** The dashboard was driven live with a mock model connected, which
> found and fixed findings 377–379 and opened decision **C15**, and every figure in `docs-notes/FIGURES.md`
> was re-audited (`mg/HANDOFF.md` §1, "2026-09-19").
>
> **2026-09-18 and 19.** The work of 2026-09-11 to 15 was
> checked by driving the dashboard live, which found and fixed findings 375 and 376 and found **A13**:
...
- `mg/REMAINING-WORK.md`. The long-form backlog, T1–T64 with T68 and T69 (T65–T67 and the A-tasks live in the sweep register), plus the M-series (M1–M6, complete). **Count the unstruck rows of `mg/HANDOFF.md` §6 rather than quoting a total from any document** — every total written into these files has gone stale at least once. `mg/REMAINING-WORK-DASHBOARD-SWEEP.md` is the short answer to "what is left, and whose is it"
...
4. An **emergency kill switch** that stops a runaway agent and reports whether it
   actually stopped.
5. A **web dashboard** over all of it, and the HTTP control plane it sits on.
   _(This read "a web dashboard **and a command-line interface**" until
   2026-09-07 (iv), a day after the command line was removed and in a list
   whose own header box says so four lines above. Finding **291**'s exact
   shape, one register over.)_
...
8. **A ledger that records the authority an action was taken under**, not merely
   who took it. _(This item was "**a login on the command line** (T5) that
   records the account and its tier and enforces the same permissions as the
   dashboard". T5's surface is gone; **the half that survives is the
   `actorRole` on the ledger entry**, which is the part requirement 8 needed
...
12. **Groups** (M3): the layer is built to hold several organisations, and **a
    ...
    409 once it exists), and since T49 (2026-09-15) that is the boundary the report
    states: separation between organisations that exist at once is exercised only
    by the tests, and is not claimed. The structure stays because it also works with
    one organisation (`docs-notes/CHAPTER3-MATERIAL.md` §3.5.89). A group is the unit a Root owns, its Root,
...
| `ui/src/pages/governance/`                                                          | The dashboard. **Split into panel modules on 2026-08-25 (T16)**: `governance-page.ts` keeps state, lifecycle and the effect primitives (**697** code lines on 2026-08-28, from 2,412; 696 at T16, 703, over the limit, under M6, back to 697 when `renderFreshness` moved to `panels/oversight-panels.ts`, finding 136. **Three lines of headroom** at that date, so the next panel added to this page would break it again — and it did, twice: T53 split the page again on 2026-09-05, and finding 323 found it back over the limit on 2026-09-08. **Do not read a line count from this document**; the lint gate is the only place it is true, which is finding 227's rule applied to the number that has needed it most), and `panels/` holds one module per panel, `policy`, `agent`, `account`, `oversight`, `session`, and the `agent-policy-lookup`. **The panels match the route modules that serve them**, so a question like "who can see the ledger?" is one route file and one panel file. Panels are pure functions of explicit props. Beside them: the typed API client, `agent-directory.ts` and the two filters (`rule-filter`, `ledger-filter`). Pure derivations, which is why their logic was always testable and the component's was not. **Six more modules were split out between 2026-09-05 and 2026-09-08 and this row did not name them until 2026-09-08 (viii)** — the map a newcomer reads was a week behind the tree it maps. The API client crossed the 700-line limit three times and each time a _subject_ moved out whole rather than the count being suppressed (T16's rule): `api.accounts.ts`, `api.policy-writes.ts`, `api.agents.ts`. The page did the same twice more: `panels/section-nav.ts` (the jump-nav, which reads the rendered DOM rather than a hand-written list so it cannot offer a Viewer a link to a section that is not on their page) and `panels/policy-agent-timeout.ts` and `panels/policy-root-settings.ts`. **`policy-agent-timeout.ts` is the one to read**: its header records why it gates on `canManageAnyAgent` and not on the authoring predicate, which is T27's distinction, and finding 335 is what that distinction looks like when a refusal forgets it |
| `ui/src/pages/governance/conversation-controller.ts`                                | The conversation's state, and since T63 (2026-09-11) the recovery of this account's running tasks after a reload: one server snapshot feeding both the conversation and _Active agent sessions_, with one cancel path                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `ui/src/pages/governance/panels/active-sessions-panel.ts`, `prompt-run-controls.ts` | _Active agent sessions_ (moved out of `agent-panels.ts` when T63 took it past 700 lines) and the Cancel control both views share                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `src/governance/prompt-runs.ts`                                                     | The in-flight prompt registry: caps, timeout, cancellation, and since T63 the `ending`/`finishing` lifecycle — a run stays listed until its reply is saved, but only executing runs hold a slot                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `src/gateway/server-methods/plugin-approval.ts`, `ui/src/app/exec-approval.ts`      | T60's report-back channel: `plugin.approval.reportOutcome`, and the Control UI's follow-up notice when an "Always allow" request could not be saved                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `ui/src/i18n/locales/en-core.ts`, `en-governance.ts`                                | T64 (2026-09-09): the English text startup loads, and the governance page's own text, which travels with that page. `en.ts` merges both for the translation pipeline only                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| ~~`src/cli/program/register.governance.ts`~~                                        | ~~The `openclaw governance ...` command tree~~ **Removed 2026-09-07.** The whole surface — six files, `governance-cli-gate.ts` and `src/governance/cli-identity.ts` — is archived with its tests and a restore procedure in `old-docs/removed-cli-surface/`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| `old-docs/removed-cli-surface/`        | The command-line surface removed 2026-09-07: every source file and test, why it went, what was lost, and a restore procedure. `CLI-REFERENCE.md` moved in with it                                                                                                                                                                                                                                                                                                               |
...
| `scripts/verify-ledger.mjs`            | Verify the audit chain from **outside** the product: no build, no server, no sign-in. The second witness requirement 8 needs (T62)                                                                                                                                                                                                                                                                                                                                              |
| `docs-notes/FIRST-RUN.md`              | **Start here if you have never seen the project (T45, 2026-09-03).** What the layer is, the one command that distinguishes this fork from stock OpenClaw, and the shortest path from a clean machine to a governed agent, with the cold-server steps in the order a new server needs them rather than as troubleshooting                                                                                                                                                        |
| `docs-notes/LINUX-INSTALL.md`          | How the fork reaches a Linux server, and the last measurements taken there                                                                                                                                                                                                                                                                                                                                                                                                      |
| `old-docs/T2-LIVE-RUN.md`              | The runbook for the live demonstration. **T2 was done on 2026-09-06**; the runbook is kept because the run is worth repeating — finding 273 was fixed after it, so the intent field in the existing evidence belongs to the wrong turn                                                                                                                                                                                                                                          |
| `docs-notes/T47-TEST-PLAN.md`          | The by-hand test plan, one list per tier plus rows needing several people; §6b, §6c and §6d added for T60/T63, the dashboard QA pass and the work after the push; 211 rows, checked against the code 2026-09-14, four C13 rows added 2026-09-15, three on 2026-09-18 and six (§6e: findings 377–379, then C15 and A13) on 2026-09-19; the rows the live checks of 2026-09-18 and 19 pressed, and those still never pressed, are listed at its top and flagged in their sections |
| `old-docs/UPSTREAM-BUG-REPORT.md`      | A bug found in OpenClaw itself (written, not yet filed)                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
A fresh install starts in `enforce`, with the shipped baseline rules already in
it (`docs-notes/BASELINE-RULES.md`). It once started in `monitor`, which ran the
full policy and recorded its verdict but let the action proceed; the independent
review flagged that as a softening of "default-deny", and the supervisor decided
it: enforce by default, and ship rules so an agent is usable on day one. `monitor`
...
`old-docs/SESSION-LOG-2026-08.md`; the round-13 backlog and the A5/A6 write-ups are in
...
| -     | Independent review (`Kimi_QA_1.md`, removed 2026-09-13; in git history)                  | Administrative actions are absent from the audit log; file paths are not canonicalised, so `workspace/../../etc/passwd` defeats a workspace rule                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
> section from all four tiers, and then through a browser under failure; **379 findings, 377 fixed, one open** (169, an unexplained observation; 371, accounts from before organisations that nothing can remove, was accepted on 2026-09-15, decision C14); **T60 and T63
...
> false "unanchored" warning on the folder-grant shape was fixed; superseded documents
> moved to `old-docs/`; and the first six reference documents in the update order were
...
>
> **2026-09-15, uncommitted:** T48 answered yes and T49 decided (one organisation per
> installation is the boundary); C13's leftovers proved (finding 372) and decided (the
> operator chooses between the list-only delete and OpenClaw's own); **built the same day**,
> which fixed 372 and found and fixed 373 and 374. `mg/HANDOFF.md` §1 "2026-09-15" and
> `mg/REMAINING-WORK.md` §"C13 decided" have the detail.
>
...
> again on 2026-09-03.** The documented command used to be
> `node node_modules/oxlint/bin/oxlint --config .oxlintrc.json src ui/src`, which
> exits `0` while missing 34 type-aware rules and all of `scripts/`.
> **Those 38 errors are now fixed and both shards exit `0`.** Two things
> replaced them, and both are about the gate rather than the code:
> `node scripts/run-lint.mjs` **could not complete on Windows**, it spawned
> stylelint through the extensionless `.bin` shim, which `spawnSync` cannot
> execute without a shell, so it died after every oxlint shard had passed and
> its CSS check had never once run (**finding 233**, fixed); and
> `git-hooks/pre-commit` **does not run it**, the hook runs `oxfmt` and plain
> `oxlint` over staged files, which is the narrow invocation finding 221 was
