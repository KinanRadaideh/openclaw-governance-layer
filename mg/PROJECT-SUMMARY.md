...
- `mg/REMAINING-WORK.md` — everything still outstanding. **§"The numbered backlog" (T1–T28) is the authoritative list**; the sections below it are history
...
13. **An agent registry** (M4): a record per agent — id, display name, group,
    ...
    agent owned by a different Administrator — though an unregistered id is
    still assignable, a deliberate limit that needs M6 to close.
...
15. **A lockdown that reaches what the locked agent started** (T6). Stopping an
    ...
    the session store.
16. A Root-only **deployment and network report** that reads the live
...
| `mg/REMAINING-WORK.md`              | The backlog. **§"The numbered backlog" (T1–T28) is authoritative**; everything beneath it is kept as history and marked as such                                  |
| `mg/SESSION-LOG-2026-08.md`         | What the August 2026 session changed, and why                                                                                                                    |
...
Eighteen rounds, **119 defects found and all fixed**, plus B1
...
- **2,151 governance tests pass across 101 files** — file _runs_, not files; 1,343 distinct tests across 75 distinct files, because the thirteen gateway files each run under three Vitest projects (measured 2026-08-24, after M4, the agent registry); `pnpm tsgo:core` and `pnpm tsgo:ui` both clean;
