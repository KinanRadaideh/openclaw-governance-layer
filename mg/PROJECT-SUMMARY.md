...
**Current as of 2026-09-07 (iii).** A single place to understand what this
...
> **Two things changed recently enough to catch you out.** The **governance
> command line was removed** on 2026-09-07 — there are two surfaces now, the HTTP
> control plane and the dashboard on it, and anything telling you to run
> `openclaw governance …` is out of date. And the **VPS still runs the previous
> build**, so it is a different product from this repository until it is rebuilt
> (`HANDOFF.md` §6, "Do this before anything else").
...
- `mg/REMAINING-WORK.md`. Everything still outstanding. **§"The numbered backlog" (T1–T62) is the authoritative list**, 47 struck and 15 open, re-derived from the rows on 2026-09-07 (iii). **Count the rows rather than quoting this line** — and note that a grep will say 48/14, because T13 carries a struck row reading _DRAFTED_ for its drafting sub-task while T13 itself is open, 5 open as of 2026-09-02, derived as 44 − 2 not-being-done − 5 open; the sections below it are history. A second backlog, §"The M-series" (M1–M6), holds the multi-tenancy feature, **complete as of 2026-08-27**
