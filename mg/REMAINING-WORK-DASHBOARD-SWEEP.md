...
> `docs-notes/report/DIVERGENCES.md`. **§A is no longer empty: A14**, a deliberate sweep of
> Chapter 1 §1.6 against the system as built, is open and is Claude's alone.
>
> **State on 2026-09-20.** Kinan took two decisions and both were built, neither committed.
> **C4** (T46): the wizard's banner and completion text name the fork, say what the layer does,
> and point at Settings → Governance and the first Root account; the prompts in between stay
> upstream's. **T17**: MiKTeX installed and all fourteen kept figures compiled, 14 of 14 clean,
> finding two defects four readings had missed; then the rendered pages were examined, which
> found twelve more across nine figures that no compile could catch. Counts are unchanged at **379 found, 377 fixed,
> 1 open**: neither of the figure defects is a defect of the governance layer, and the one other
> thing found, `src/commands/onboard-guided.test.ts` failing 12 of 23 at HEAD, is upstream's file
> and not the fork's (`mg/HANDOFF.md` §4). **C4 was the second-to-last decision in §C: only C10 is still open there**, and T17 was
> never a §C row, it is B3.
>
...
| **A14** | **Sweep Chapter 1 §1.6 (Preliminary Design) against the system as built, and register every difference with its justification.** Added 2026-09-20 at Kinan's instruction. Five differences are already recorded in `docs-notes/report/DIVERGENCES.md`, but they were found while reading §1.6 for other reasons rather than by a deliberate pass, so the list is certainly incomplete. Read §1.6 paragraph by paragraph against the code and the registers; for each difference write what Chapter 1 proposed, what was built, and **why the change was made**, because the justification is what Chapter 3 needs and the part that takes thought. §2 of that file lists four candidates already noticed and not yet verified: an administrator's answer "optionally becoming policy" against the rule request that is actually filed; the Viewer's read scope; what the network rule kind matches against; and each tier's description read line by line against `docs-notes/ROLE-MODEL.md`. **Do the same for Chapter 2** while there, which has already yielded two.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Chapter 3 has to state every difference openly; one found by an examiner instead reads as an oversight and invites a hunt for more                           | Medium                      |
...
| **B3**     | **T17 — the figures.** **Re-audited 2026-09-19 at Kinan's request**: all 23 drawable figures read against the code, fifteen corrected (F3's Mermaid did not parse; F10 drew its first two steps backwards; F24's TikZ would have printed a command name), every one now in prose, Mermaid and TikZ, and the summary table rebuilt: **fourteen recommended, twelve in Chapter 3 and two in Chapter 4** (`docs-notes/FIGURES.md` §"2026-09-19"). The Mermaid is parsed and rendered by Mermaid 11, 23 of 23. **Compiled and looked at, 2026-09-20** (T17's compile half, decided by Kinan that day): MiKTeX installed, all fourteen kept figures compiled one by one and together, **14 of 14 clean**, and then every rendered page examined. The compile found two defects four readings had missed; **looking found twelve more across nine of the fourteen**, none of them things a compile can see. All fixed, and `docs-notes/figures/build-figures.mjs` regenerates the document from `FIGURES.md`. What is left is yours: approve the fourteen (the PDF was sent), and compile once inside `main.tex`, which has not been done | Approve the drafts                                                                                                                                                                                                 |
...
| ~~**C4**~~  | **DECIDED BY KINAN 2026-09-20: banner and completion text only**, not new prompts — new prompts touch more upstream files and grow the fork diff §3.5.2b measures, for wording the demo path never reads. Not yet built. _The question was:_ **T46 — how far the setup wizard rewording goes**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Banner and completion text only, or new prompts too. Touches upstream files, so it grows the fork diff §3.5.2b measures                                                                                                                             |
