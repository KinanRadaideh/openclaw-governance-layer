...
> Kinan's, and §C, six decisions: C4, C8, C9, C10, and two added that day, **C13** (what
> "delete from host" should remove) and **C14** (finding 371). **Two findings are open:
> T67 (finding 169, never reproduced) and 371 (accounts from before organisations cannot
> be removed; low, and recommended accepted).** The week's QA and the second check of the
...
| **C13**     | **What "delete from host" should remove** (found 2026-09-14, not yet a numbered finding). Governance's `deprovisionAgent` calls only `deleteAgentConfigEntry`, the roster mutation. OpenClaw's own `agents.delete` also keeps a deletion journal, removes the agent's scheduled jobs (`removeAgentJobsTransactional`), its host exec-approval policy (`withAgentExecApprovalsRemoved`) and its session store entries (`purgeAgentSessionStoreEntries`, upstream #65524), and by default trashes its workspace, agent directory and session transcripts. Whether a new agent provisioned under the same id inherits those is **not proven**: the provisioning tests mock both host functions, and a proof needs a real OpenClaw state directory                                                                                              | (a) Match the host: compose its deletion the way M6 composes `createAgent` (upstream's logic is inline in the RPC handler, so this means extracting it or routing through the Gateway), and decide whether files go too, which cannot be undone; (b) prove the inheritance first, then decide; (c) leave it and state it in the report. **Recommended: (b)**, because the decision turns on a consequence nobody has measured |
...
and C13's inheritance question.
