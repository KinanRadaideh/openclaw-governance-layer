...
> Kinan's, and §C, three decisions: C4, C10, and **C14** (finding 371), added on 2026-09-14.
> **C13** (what "delete from host" should remove) was decided by Kinan on 2026-09-15 and
> **built by Claude the same day, uncommitted** (§"C13 decided" at the foot of this
> file). **C9 (T48) and C8 (T49) were answered by Kinan on 2026-09-15**: yes,
> Chapter 3 is written now; and (b), one organisation per installation is the boundary. **Two findings are open:
> T67 (finding 169, never reproduced) and 371 (accounts from before organisations cannot
> be removed; low, and recommended accepted).** 372 (a new agent inherits a deleted agent's
...
| **C13**     | **What "delete from host" should remove** (found 2026-09-14; **proved 2026-09-15 and numbered finding 372**). **DECIDED BY KINAN AND BUILT 2026-09-15: both deletions offered as a choice, each explained; uncommitted — §"C13 decided" at the foot of this file. Finding 372 fixed; 373 and 374 found and fixed on the way.** Governance's `deprovisionAgent` calls only `deleteAgentConfigEntry`, the roster mutation. OpenClaw's own `agents.delete` also keeps a deletion journal, removes the agent's scheduled jobs (`removeAgentJobsTransactional`), its host exec-approval policy (`withAgentExecApprovalsRemoved`) and its session store entries (`purgeAgentSessionStoreEntries`, upstream #65524), and by default trashes its workspace, agent directory and session transcripts. **Proved 2026-09-15 (option b):** on a real OpenClaw state directory, a new agent provisioned under the same id by a different Administrator inherited the workspace file, the agent and session folders, the session database, the scheduled job and the approval settings (`docs-notes/qa-sweep-2026-09-15/`; design §3.5.90) | (a) Match the host: compose its deletion the way M6 composes `createAgent` (upstream's logic is inline in the RPC handler, so this means extracting it or routing through the Gateway), and decide whether files go too, which cannot be undone; ~~(b) prove the inheritance first, then decide~~ **done 2026-09-15**; (c) leave it and state it in the report. (a) would not touch the audit ledger, and is the option closest to T55 (§3.5.90). **Recommended now: (a)**, with the files decided separately. _(Until the proof this recommended (b), because the decision turned on a consequence nobody had measured.)_ |
| **C14**     | **Finding 371 — accounts from before organisations.** They cannot sign in, and nothing removes them since `governance groups migrate --delete` went with the command line; no installation in use can hold one                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | (a) A Root-only control that lists and deletes them; (b) accept it, and say in the report that the migration exists only as a store function. **Recommended: (b)**, since the state cannot arise on anything that ships                                                                                                                                                                                                                                                                                                                                                                                                    |
...
**Built, uncommitted.** Paused mid-build at one failing end-to-end test, then resumed and
