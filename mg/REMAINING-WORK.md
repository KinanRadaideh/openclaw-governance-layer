...
below, **fifty-seven tasks, T1–T57, of which 43 are struck and 14 open** (T1 is
...
| **273–275** | §"2026-09-06 (iv)" | **From the first live run, and from sweeping around it.** **273**: `readAgentIntent` reads the model's stored narration and never clears it, so the last thing a model said sticks to every later tool call until it speaks again. Visible in the very ledger that records T2 — entry #25's "Agent said" text is the model's refusal from a turn 25 minutes earlier, so the entry reads as though the _model_ refused when in fact it attempted the read and the **gate** stopped it. The enforcement is right and the evidence field inverts the story, which is finding 257's class: a defect in what the trail says rather than in what the layer did. **274**: nothing in the build ever clears `dist/` — no clean step, no `emptyOutDir` — so a build that dies partway (the README already warns it is OOM-killed under 8 GB) leaves new chunks beside old ones and a reference that resolves to nothing; on the VPS this broke `exec` entirely with `Cannot find module .../bash-tools-O2NWTdBu.js`. **The ledger's silence was the diagnosis**: the gate records every invocation, so a governance refusal is in the chain by construction, and nothing there meant the failure happened before a tool call existed to gate. **275**: three tests in `src/agents/agent-create.test.ts` fail on the committed tree, unrelated to any change here, confirmed with the working tree fully stashed — outside the seven verification commands, which is exactly the caveat those commands carry |
