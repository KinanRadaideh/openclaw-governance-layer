...
below, **fifty-six tasks, T1–T56, of which 41 are struck and 15 open** (T1 is
...
| **T53**     | **Claude: split `governance-page.ts`, which is a recorded 700-line exception rather than a silent one.** 735 code lines against the limit. Four files were split properly on 2026-09-04 instead of suppressed (the per-agent timeout route, its dashboard row, the deployment types, the pre-sign-in gate); this one has no cheap seam, because every remaining candidate reads twenty or more private fields, so moving it relocates the lines and adds the plumbing. The real fix is two components, an outer shell and an inner signed-in view. Deliberately **not** done in the last hour before a handoff: it is a genuine refactor with regression risk on the surface T38 found four defects in                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
