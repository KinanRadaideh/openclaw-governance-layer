...
| **T54**     | **Kinan then Claude: what should a folder grant normalise against?** (finding 253.) `FolderGrantInput.cwd` is documented as the workspace root that keeps a grant's pattern in step with what the gate matches, and **no production caller passes it**, so an absolute-path grant writes an allowance and an exception that bind nothing while the panel confirms both to the operator. Relative paths work, which is why nobody hit it. **The fix is a decision, not a patch**: an agent-scoped grant could resolve against that agent's workspace, but a global grant binds agents with different workspaces and has no single root, so the options are (1) resolve per agent and refuse a global grant on a relative path, (2) refuse an absolute path at authoring time with an explanation, or (3) build the pattern position-independently with `(^                                                                                                                                                                                                                                                                                                                                                                    | /)`the way the shipped core rules already do, which matches both forms and is the smallest change. Reproduced by`docs-notes/qa-sweep-2026-09-04/gate-sweep.ts`, where it is pinned as a check that fails when it is fixed |
