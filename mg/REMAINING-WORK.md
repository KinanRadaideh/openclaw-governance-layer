...
**Open, counted from the rows of §"The numbered backlog": eleven unstruck, one of which
is T1 (not being done), so ten.** T3 (the Linux host), T13 (a read), T17 (the figures),
T18 (the report), T46 (the setup wizard's wording), T47 (the by-hand plan), T48 and T49
(two judgements), and T58 and T59 (whether `edit` is ours, and per-agent models). The
