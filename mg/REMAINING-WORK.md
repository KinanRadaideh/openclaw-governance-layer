...
| **T54**     | **Kinan then Claude: what should a folder grant normalise against?** (finding 253.) `FolderGrantInput.cwd` is documented as the workspace root that keeps a grant's pattern in step with what the gate matches, and **no production caller passes it**, so an absolute-path grant writes an allowance and an exception that bind nothing while the panel confirms both to the operator. Relative paths work, which is why nobody hit it. **The fix is a decision, not a patch**, because a global grant binds agents with different workspaces and has no single root to measure against. **Written up in plain terms for the team in `docs-notes/T54-DECISION.md`**, with a recommendation: refuse a path the control cannot express faithfully and say why (now), then resolve against the named agent's workspace (later, if absolute paths are wanted). **Not the one-line option** of building the pattern position-independently: every shipped rule that does so is a _denial_, where matching too much is safe, and this control writes an _allowance_, where matching too much hands out access nobody asked for. Reproduced by `docs-notes/qa-sweep-2026-09-04/gate-sweep.ts`, where it is pinned as a check that fails when it is fixed |
