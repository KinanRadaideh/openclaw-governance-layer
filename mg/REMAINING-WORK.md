...
code lines → **613**), split five ways along seams each of which states one
authorization rule. **Two files remain over:** `governance-page.ts` (2,412) and
`register.governance.ts` (848). **T14 closed 2026-08-24** — all three surfaces.
**One is deprioritised:** T1 — not being done.
**Eight remain:** T2, T3, T6, T7, T8, T13, T17, T18 — plus **T28**, new on
2026-08-25. (T25 closed 2026-08-25.)
...
| **T28**     | **Unreachable code in `policy-engine.ts`, found 2026-08-25.** `node node_modules/oxlint/bin/oxlint --config .oxlintrc.json src/governance/policy-engine.ts` reports `no-unreachable` at the closing `return undefined;` of `evaluateGovernancePolicy` (line ~689). **Pre-existing — present in `HEAD`, not introduced by M4/T25/T16** — and deliberately left alone rather than deleted, because the fix depends on _why_ it is unreachable and that needs the whole function's control flow read, in the file the entire security argument rests on. Two readings with opposite fixes: the escalation branch above it always returns, in which case the line is dead and should go; or a path that should reach it returns early somewhere it should not, in which case deleting the line hides a defect. **Same class as finding 112** — a branch that cannot execute in a file whose job is deciding what an agent may do. Also worth clearing the pre-existing `no-shadow` and `no-promise-executor-return` errors in `file-lock.test.ts` while in the area                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | Nothing | 2–4 h  |
