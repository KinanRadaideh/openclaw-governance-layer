...
> **If the question is "what is left and whose is it", read
> `mg/REMAINING-WORK-DASHBOARD-SWEEP.md` instead** (written 2026-09-08 (vii)).
> It carries the same outstanding work **sorted by who has to move first** —
> what Claude can do alone, what only Kinan can do, and what is blocked on a
> decision — plus findings 326–334 from the pass that drove the remaining eight
> dashboard sections. This file stays the long-form backlog and the place a
> task's reasoning lives; that one is the short answer.
>
> **Its first line is the urgent one: the startup JS budget has 112 bytes of
> headroom**, which is one short sentence for the whole product, and T64 is now
> what blocks the next operator-facing text fix rather than tidying after it.

**The authoritative outstanding list is §"The numbered backlog" immediately
below, **sixty-two tasks, T1–T62, of which 47 are struck and 15 open** (T1 is
not being done, and T41 was cancelled and is struck), **re-derived from the rows
2026-09-07 (iii)**. **A grep will tell you 48/14** — T13 carries a struck row
reading _DRAFTED_ for its drafting sub-task while T13 itself is open, which is
the same two-row trap recorded below. **Re-derived from the rows
2026-09-05, which is finding 259**: the summaries in this file and in
`HANDOFF.md` disagreed with each other and both disagreed with the table, because
**T13 has two rows** — struck where its drafting was recorded, open where the
reading still is — and each summary had taken a different one. **Count the
rows.** A §"Who can do what" triage sits in front of the table. _(This sentence has been stale three
...
**Current as of 2026-09-11: 53 struck, 11 open, across T1–T64.** 53 + 11 =
64, and the arithmetic is written out because without it this line was wrong
...
> **Current answer, re-derived 2026-09-07 (iii) from the rows.** Fifteen open,
> sorting into four categories rather than three:
>
