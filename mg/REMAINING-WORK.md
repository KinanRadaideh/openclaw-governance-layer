...
**Eight remain:** T2, T3, T6, T7, T8, T16 (one file), T17, T18 — plus **T13**,
which is drafted and waiting to be read. Only **T16** can be finished without
either a decision from you or a change in OpenClaw itself. See the triage
immediately below.
...
| #       | What                                          | Depends on                                   |
| ------- | --------------------------------------------- | -------------------------------------------- |
| **T16** | `governance-page.ts`, 2,412 lines → under 700 | Nothing. A seam has to be found, not granted |
...
| ------ | --------------------------------------------------- | --------------------------------------------------------- |
| **T6** | Lockdown does not reach a running cross-agent child | `spawnedBy` on `HookContext`                              |
| **T7** | Search tools governed at their root only            | An `after_tool_call` reporting files actually opened      |
| **T8** | Outbound messages are ungoverned                    | A fourth resource kind, separable from the `message` tool |

These three cannot be closed from this codebase and are written up as stated
limitations rather than scheduled as work. **What _is_ doable here is checking
whether upstream has moved** — each names a specific hook or field, and whether
it exists now is a twenty-minute question against the current `openclaw/openclaw`
tree. Nobody has asked it since the limitations were recorded.
