// Control UI English locale strings, minus the strings a lazily-loaded page owns.
//
// **Split out of `en.ts` on 2026-09-09, decision T64.** This module is what the
// running app loads at **startup**: `lib/translate.ts` imports it directly, so
// every sentence in it is paid for by every operator on every first page load,
// whatever page they are going to.
//
// The English catalog used to be one module and the governance dashboard's
// ~32 KB of operator-facing text (10.5 KB gzipped) was charged here, although
// the page itself is lazy and most sessions never open it. That is what
// exhausted the startup budget: 67 bytes of headroom for the whole product,
// against a ceiling that had already been raised once.
//
// **The rule this file exists to enforce.** Strings belonging to one lazy page
// live in a sibling `en-<page>.ts` that the page imports, so they travel in the
// page's own chunk. `en-governance.ts` is the first; `en-agents.ts` is the
// older precedent for splitting this file at all, though that one is still
// merged in below because its strings are used from the app shell.
//
// **`en.ts` is still the whole catalog** and is still the source of truth for
// the translation pipeline, which imports it and reads `en`. Nothing about the
// twenty translated locales changes: they were already one lazily-imported
// module each, and they still contain every key.
import type { TranslationMap } from "../lib/types.ts";
import * as agentEn from "./en-agents.ts";

export const enCore: TranslationMap = {
  common: {
    health: "Health",
    ok: "OK",
    yes: "Yes",
  ...
  execApproval: {
    followUpTitle: "Approval follow-up",
